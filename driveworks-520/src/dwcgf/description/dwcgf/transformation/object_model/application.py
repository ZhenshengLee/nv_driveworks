...
{len(pass_not_listed)} passes not listed in wcet file."
    ...
    def collect_wcet(self, schedule_name: str) -> Dict[str, int]:
        ...
        if not self._constructed:
            raise NotImplementedError("Can not dump wcet before construction.")
...
{len(pass_with_empty_wcet)} passes have no valid wcet."
    ...
    def dump_pass_dependency_graph(self, schedule_name: str) -> dict:
        ...
        if not self._constructed:
            raise NotImplementedError(
                "Can not dump pass dependency graph before construction."
            )
        if schedule_name not in self._stm_schedules:
    ...
    def update_hyperepoch_name(
        ...
        if new_hyperepoch_name == "":
            raise ValueError("New hyperepoch name should not be empty string")
    ...
    @object_model_channel.pair_self
    def update_hyperepoch_monitoring_period(
        self, schedule_name: str, hyperepoch_name: str, new_monitoring_period: int
    ) -> UndoContext:
        ...
        old_monitoring_period = schedule_definition.schedule.hyperepochs[
            hyperepoch_name
        ].monitoring_period
        schedule_definition.schedule.hyperepochs[
            hyperepoch_name
        ].monitoring_period = new_monitoring_period

        self._self_is_modified = True

        return UndoContext(self, schedule_name, hyperepoch_name, old_monitoring_period)
    ...
    def remove_process_subcomponent(
        self, process_name: str, content: List[str], allowProcessNotFound: bool
    ) -> None:
    ...
    @object_model_channel.pair
    def insert_process_epochsyncservice_pass(
        self, process_name: str, pass_name: str, pass_param: Optional[dict] = None
    ) -> UndoContext:
        """insert EpochSyncService pass to the process."""
        # The if check is for print detailed error message
        # The assert is for passing mypy static check
        if process_name not in self._processes:
            raise ValueError(f"Process name '{process_name}' not found.")
        services_instance = self._processes[process_name].services
        if services_instance is None:
            raise ValueError(f"Process: '{process_name}' has no services.")
        assert services_instance is not None
        sync_instance = services_instance.sync
        if sync_instance is None:
            raise ValueError(f"Process: '{process_name}' has no EpochSyncService.")
        assert sync_instance is not None
        # insert parameters
        if pass_param is not None:
            sync_instance.parameters[pass_name] = pass_param
        # insert pass
        sync_instance.passes.append(pass_name)
        self._self_is_modified = True

        return UndoContext(self, process_name, pass_name)

    @insert_process_epochsyncservice_pass.pair
    def remove_process_epochsyncservice_pass(
        self, process_name: str, pass_name: str
    ) -> UndoContext:
        """remove EpochSyncService pass to the process."""
        # The if check is for print detailed error message
        # The assert is for passing mypy static check
        if process_name not in self._processes:
            raise ValueError(f"Process name '{process_name}' not found.")
        ...
        assert sync_instance is not None
        # remove parameters
        pass_param_dict: Dict[str, Dict] = {}
        if pass_name is not sync_instance.parameters:
            pass_param_dict = sync_instance.parameters.pop(pass_name)
        # remove pass
        sync_instance.passes.remove(pass_name)
        self._self_is_modified = True

        return UndoContext(self, process_name, pass_name, pass_param_dict)
    ...
    def remove_unspecified_epoch_passes(self, schedule: str, content: Set[str]) -> None:
        """remove all epoch passes unspecified in the content set (undo-able)."""
        if schedule not in self._stm_schedules:
            raise ValueError(f"Schedule name '{schedule}' does not exist.")
        for hyperepoch in self._stm_schedules[schedule].schedule.hyperepochs.values():
            for epoch in hyperepoch.epochs.values():
                # For each pass, remove it from the pass list if it appears in the content set.
                for epoch_passes in epoch.passes:
                    idx = 0
                    while idx < len(epoch_passes):
                        if epoch_passes[idx] not in content:
                            self.__remove_from_list(
                                src=epoch_passes, start_at=idx, length=1
                            )
                        else:
                            idx += 1
    ...
    @object_model_channel.pair_self
    def set_schedule_priority(self, schedule_name: str, priority: int) -> UndoContext:
        """set a schedule's priority (undo-able)."""
        if schedule_name not in self._stm_schedules:
            raise ValueError(f"Schedule name '{schedule_name}' does not exist.")

        schedule = self._stm_schedules[schedule_name]
        old_priority = schedule.schedule.priority
        schedule.schedule.priority = priority

        self._self_is_modified = True

        return UndoContext(self, schedule_name, old_priority)
    ...
    def set_rcs_enabled(self, process_name: str, enable: bool) -> UndoContext:
        ...
        if process_name not in self._processes:
            raise ValueError("Can't disable as the specified process doesn't exist.")

        services = self._processes[process_name].services
        if services is None or services.rcs is None:
            raise ValueError(
                "Can't insert as the RoadCastService is not defined for the process."
            )
    ...
    def update_rcs_serialize_mask(
        ...
        if process_name not in self._processes:
            raise ValueError("Can't update as the specified process doesn't exist.")

        services = self._processes[process_name].services
        if services is None or services.rcs is None:
            raise ValueError(
                "Can't update as the RoadCastService is not defined for the process."
            )
        ...
        for mask_name, mask_value in serialize_mask.items():
            if not isinstance(mask_name, str) or not isinstance(mask_value, bool):
                raise ValueError("Invalid serialize mask.")
