...
class Component:
    ...
    def try_get_input_port(self, name: str) -> Optional[Port]:
        """Get input port of this component.

        Similar to `get_input_port(...)`, this API return None if not found.
        """
        return Component._get_port(self.input_ports, name)
    ...
    def try_get_output_port(self, name: str) -> Optional[Port]:
        """Get output port of this component.

        Similar to `get_output_port(...)`, this API return None if not found.
        """
        return Component._get_port(self.output_ports, name)
    ...
    def get_output_port(self, name: str) -> Port:
        ...
        if found is None:
            raise ValueError(
                f"Cannot find output port with name '{name}' "
                + f"in component '{self._name}', "
                + f"available ports '{self.output_ports}'"
            )
        return found
    ...
    def _insert_port_with_connection(
        ...
        if port.upstream is not None:
            raise ValueError("new port already has upstream")
