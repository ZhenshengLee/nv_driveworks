...
from dwcgf.transformation.object_model import Graphlet, Node, Port
...
class ReplaceSwitchboard(Action):
        ...
        for connection_definition in connection_definitions:
            ...
            if src == original_src:
                ...
                new_connection_definition = ConnectionDefinition(
                    src=new_src,
                    dests=new_dests,
                    params=connection_definition.params,
                )
                if len(new_dests) > 0:
    ...
    def transform_impl(self, target: Graphlet) -> None:
        ...
        if depth not in [3, 4, 5]:
            raise ValueError(
                "Expected <subcomp>.<subsubcomp>.<subsubsubcomp>.<subsubsubsubcomp>.<output>; "
...
class RemoveUnspecifiedSubcomponents(Action):
    ...
    def should_keep(self, component_name: str) -> bool:
        ...
        for keep in keeps:
            # use delimiter to strict check name to avoid return true for
            # 'roadCast' with `keep = 'road'`
            if component_name == keep:
                return True
            elif len(component_name) < len(keep) and keep.startswith(
                component_name + "."
            ):
                return True
            elif len(component_name) > len(keep) and component_name.startswith(
                keep + "."
            ):
                return True
        return False
...
class InsertParameter(Action):
    ...
    extra_attributes = (
        ...
        ActionAttribute(
            ...
            attr_type=str,
            default=None,
            description="Comment for new parameter.",
        ),
    )
    ...
    def transform_impl(self, target: Graphlet) -> None:
        ...
        definition = ParameterDefinition(
            ...
            comment=self.get_attribute("comment"),  # may return None
        )
...
class RemoveConnection(Action):
    ...
    extra_attributes = (
        ...
        ActionAttribute(
            name="allowConnectionNotFound",
            is_required=False,
            attr_type=bool,
            default=False,
            description="The bool indicating whether allow connection not found.",
        ),
    )
    ...
    def transform_impl(self, target: Graphlet) -> None:
        ...
        allow_connection_not_found: bool = self.get_attribute("allowConnectionNotFound")
        if len(dests) == 0:
            raise ValueError("dests cannot be empty")
        target.remove_connections(src, dests, allow_connection_not_found)


@ActionFactory.register("remove-connection-to-dest")
class RemoveConnectionToDest(Action):
    """class for remove-connection-to-dest action."""
    ...
    def transform_impl(self, target: Graphlet) -> None:
        """action implementation."""
        dest: str = self.get_attribute("dest")
        if len(dest) == 0:
            raise ValueError("dest cannot be empty")
        target.remove_connection_to_dest(dest)
...
class InsertConnectionNested(Action):
    ...
    def transform_impl(self, target: Graphlet) -> None:
        ...
        for dest, param in dests.items():
            ...
            if depth < 3:
                raise ValueError(
                    "ERROR! only support nesting more than three level "
                    f"currently dest is not support (incompatible dest: {dest})"
                )
...
@ActionFactory.register("replace-src-port")
class ReplaceSrcPort(Action):
    """class for replacing srource port for destination ports."""
    ...
    extra_attributes = (
        ActionAttribute(
            ...
            attr_type=str,
            default="",
            description="New source port name.",
        ),
        ActionAttribute(
            name="dests",
            is_required=True,
    ...
    def transform_impl(self, target: Graphlet) -> None:
        ...
        """
        newSrc = self.get_attribute("newSrc")
        dests: Dict = self.get_attribute("dests")
        params: Dict = self.get_attribute("params")
        ...
        if len(params) != 0:
            for i in target.connection_definitions:
                if i.src == newSrc:
                    raise ValueError(
                        f"ERROR! src: {newSrc} exists. Cannot update params!"
                    )

        connection_definition = ConnectionDefinition(
            src=newSrc, dests=dests, params=params
        )
        target.insert_connections(connection_definition)
