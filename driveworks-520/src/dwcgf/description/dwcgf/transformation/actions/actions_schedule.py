...
class RemoveEpoch(Action):
    ...
    def transform_impl(self, target: Application) -> None:
        ...
        if self.get_attribute("scheduleNames") and self.get_attribute("allSchedules"):
            raise ValueError(
                "Please do not specify schedule names with 'allSchedules' on."
            )
        elif not self.get_attribute("scheduleNames") and not self.get_attribute(
            "allSchedules"
        ):
            raise ValueError("Please specify schedule names with 'allSchedules' off.")
...
@ActionFactory.register("remove-unspecified-epoch-passes")
class RemoveUnspecifiedEpochPasses(Action):
    """class for remove-unspecified-epoch-passes action."""
    ...
    extra_attributes = (
        ActionAttribute(
            ...
            attr_type=list,
            default=None,
            description="Epoch passes to keep. Passes not specified in this list will be removed.",
        ),
        ActionAttribute(
            name="scheduleNames",
            is_required=False,
    ...
    def transform_impl(self, target: Application) -> None:
        ...
        for schedule_name in (
            ...
            target.remove_unspecified_epoch_passes(
                schedule=schedule_name,
                content=set(self.get_attribute("passesToKeep")),
            )
        ...
        for schedule_name in cast(list, self.get_attribute("scheduleNames")):
            target.insert_hyperepoch(
                ...
                hyperepoch=HyperepochDefinition(
                    ...
                    period=self.get_attribute("period"),
                    monitoring_period=self.get_attribute("monitoringPeriod"),
                    epochs=epochs,
                    resources=resources,
                ),
            )
...
class RemoveHyperepochResourceAssignmentByPassName(Action):
    ...
    def transform_impl(self, target: Application) -> None:
        ...
        for schedule_name in (
            ...
            target_hyperepoch: HyperepochDefinition = (
                schedule_definition.schedule.hyperepochs[hyperepoch_name]
            )
...
@ActionFactory.register("update-hyperepoch-monitoring-period")
class UpdateHyperepochMonitoringPeriod(Action):
    """class for update-hyperepoch-monitoring-period action."""
    ...
    def transform_impl(self, targets: Application) -> None:
        """action implementation."""
        for schedule_name in cast(list, self.get_attribute("scheduleNames")):
            targets.update_hyperepoch_monitoring_period(
                schedule_name=schedule_name,
                hyperepoch_name=self.get_attribute("hyperepochName"),
                new_monitoring_period=self.get_attribute("newMonitoringPeriod"),
            )
...
class InsertProcess(Action):
    ...
    extra_attributes = (
        ...
        ActionAttribute(
            ...
            attr_type=dict,
            default=None,
            description="Data for the process to be added.",
        ),
    )
...
class RemoveProcessArg(Action):
    """class for remove-process-argument action."""
...
@ActionFactory.register("remove-process-argument-str")
class RemoveProcessArgStr(Action):
    """class for remove-process-argument-str action."""
    ...
    extra_attributes = (
        ...
        ActionAttribute(
            ...
            attr_type=str,
            default=None,
            description="Key of the argument to be modified.",
        ),
        ActionAttribute(
            name="argumentValue",
            is_required=True,
            attr_type=str,
            default=None,
            description="Value within argument to be removed.",
        ),
    )
    ...
    def transform_impl(self, target: Application) -> None:
        ...
        if process_name not in target.processes:
            raise ValueError(f"Process name '{process_name}' not found.")
        if target.processes[process_name].argv is not None:
            if arg not in cast(dict, target.processes[process_name].argv).keys():
                raise ValueError(f"Input argument name '{arg}' dose not exist.")
        if len(arg) == 0:
            raise ValueError("Input argument name is empty.")

        dst = target.processes[process_name].argv
        assert dst is not None
        assert isinstance(dst[arg], str)

        argParts = dst[arg].split(",")
        if arg_value in argParts:
            argParts.remove(arg_value)
        if len(argParts) == 0:
            target.remove_process_arg(process_name, arg)
        else:
            new_arg = ",".join(argParts)
            target.remove_process_arg(process_name, arg)
            target.insert_process_arg(process_name, arg, new_arg)


@ActionFactory.register("append-process-argument-str")
class AppendProcessArgStr(Action):
    """class for append-process-argument-str action."""
    ...
    def transform_impl(self, target: Application) -> None:
        ...
        if process_name not in target.processes:
            raise ValueError(f"Process name '{process_name}' not found.")
        if (
            target.processes[process_name].argv is None
            or arg not in target.processes[process_name].argv.keys()
        ):
            target.insert_process_arg(process_name, arg, arg_value)
        else:
            existing_arg = target.processes[process_name].argv[arg]
            assert isinstance(existing_arg, str)
            argParts = existing_arg.split(",")
            if arg_value in argParts:
                raise ValueError(
                    f"append a duplicate arg: {arg_value} into existing arg: {existing_arg}"
                )
            new_arg = existing_arg + "," + arg_value
            target.remove_process_arg(process_name, arg)
            target.insert_process_arg(process_name, arg, new_arg)
...
class RemoveProcessSubcomponents(Action):
    ...
    extra_attributes = (
        ...
        # TODO(azou): This is a WAR for supporting l2_4 single process on X86/Drive-Linux
        # and l2_4 multi process on Drive-QNX.
        # Remove this once l2_4 multi process default on all platform.
        ActionAttribute(
            name="allowProcessNotFound",
            is_required=False,
            attr_type=bool,
            default=False,
            description="Allow process not found in the DAG and ignore this action.",
        ),
    )
    ...
    def transform_impl(self, target: Application) -> None:
        ...
        target.remove_process_subcomponent(
            process_name=self.get_attribute("processName"),
            content=self.get_attribute("subcomponentID"),
            allowProcessNotFound=self.get_attribute("allowProcessNotFound"),
        )


@ActionFactory.register("insert-process-epochsyncservice-pass")
class InsertProcessEpochSyncServicePass(Action):
    """class for insert-process-epochsyncservice-pass action."""
    ...
    extra_attributes = (
        ...
        ActionAttribute(
            ...
            attr_type=dict,
            default=None,
            description="Parameters for the pass.",
        ),
    )

    def transform_impl(self, target: Application) -> None:
        """action implementation."""
        target.insert_process_epochsyncservice_pass(
            process_name=self.get_attribute("processName"),
            pass_name=self.get_attribute("passName"),
            pass_param=self.get_attribute("passParams"),
        )


@ActionFactory.register("remove-process-epochsyncservice-pass")
class RemoveProcessEpochSyncServicePass(Action):
    """class for remove-process-epochsyncservice-pass action."""
    ...
    def transform_impl(self, target: Application) -> None:
        """action implementation."""
        target.remove_process_epochsyncservice_pass(
            process_name=self.get_attribute("processName"),
            pass_name=self.get_attribute("passName"),
        )
...
# Schedule State
####################################################################################################
@ActionFactory.register("set-schedule-priority")
class SetSchedulePriority(Action):
    """class for set-schedule-priority action."""
    ...
    extra_attributes = (
        ActionAttribute(
            ...
            attr_type=str,
            default="",
            description="The existing schedule to set priority",
        ),
        ActionAttribute(
            name="initOrder",
            is_required=True,
    ...
    def transform_impl(self, target: Application) -> None:
        """action implementation."""
        target.set_schedule_priority(
            schedule_name=self.get_attribute("scheduleName"),
            priority=self.get_attribute("initOrder"),
        )
...
class InsertSchedule(Action):
    ...
    extra_attributes = (
        ...
        ActionAttribute(
            ...
            attr_type=int,
            default=None,
            description="Priority of the schedule to be inserted.",
        ),
    )
        ...
        if isinstance(hyperepoch_json, dict):
            ...
            for specifier, value in hyperepoch_json.items():
                ...
                hyperepochs[specifier] = HyperepochDefinition(
                    ...
                    epochs=epochs,
                    resources=resources,
                    monitoring_period=value.get("monitoringPeriod"),
                )
                ...
                for epoch in epochsList:
                    ...
                    for key, resource in hyperepoch.resources.items():
                        ...
                        if "CPU" in key:
                            ...
                            # there can be only 1 VIC ldc and 1 nv2d resource per schedule
                            if len(vic_epoch) != 0 and vic_epoch != epoch.name:
                                continue
                            resource_name = key
                            vic_epoch = epoch.name
                        else:
                            continue
                    ...
                    hyperepoch_monitoring_period = None
                    if hyperepoch.monitoring_period is not None:
                        hyperepoch_monitoring_period = (
                            hyperepoch.monitoring_period / epoch.frames
                        )
                    ...
                    new_hyperepoch = HyperepochDefinition(
                        ...
                        monitoring_period=hyperepoch_monitoring_period,
                    )
                    target.insert_hyperepoch(
