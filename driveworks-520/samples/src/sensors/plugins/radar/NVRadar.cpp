...
dwStatus NVRadar::parseDataBuffer(dwRadarScan* output, const dwRadarScanType scanType)
{
    ...
    dwRadarDetection* points      = reinterpret_cast<dwRadarDetection*>(output->data);
    ...
    for (size_t i = 0; i < output->numReturns; ++i)
    {
        ...
        dwRadarDetection& point = points[i];
        point                   = {};
...
dwStatus NVRadar::getConstants(_dwSensorRadarDecoder_constants* constants)
{
    ...
    constants->properties.numPacketsPerScan[DW_RADAR_RETURN_TYPE_DETECTION][DW_RADAR_RANGE_SHORT] = getPacketsPerScan(DW_RADAR_RETURN_TYPE_DETECTION, DW_RADAR_RANGE_SHORT);
...
dwStatus NVRadar::getParameter(std::string& val, const std::string& param, const char* params)
{
...
size_t NVRadar::getMaxPointsPerPacket(dwRadarReturnType returnType, dwRadarRange range)
{
    ...
    case DW_RADAR_RETURN_TYPE_DETECTION:
    {
        ...
        {
            points = MAX_DETECTIONS_PER_SCAN;
            break;
        }
    ...
    case DW_RADAR_RETURN_TYPE_CLASSIFIER:
