...
class RadarSelfCalibrationSample : public DriveWorksSample
{
    ...
    // Maximum radar scan count: sample will exit once m_scanIndex reaches MAX_RADAR_SCAN_CNT
    static constexpr uint32_t MAX_RADAR_SCAN_CNT = 1000;
...
public:
    ...
    bool onInitialize() override
    {
        ...
        // -----------------------------------------
        {
            ...
            int32_t odometryPropertyEstimation = std::stoi(getArgument("calibrate-wheel-radii"));

            if (odometryPropertyEstimation != 0)
            {
                radarCalibrationParams.enableWheelRadiiEstimation = true;
            }
            else
            {
                radarCalibrationParams.enableWheelRadiiEstimation = false;
            }
    ...
    void initSensor()
    {
        ...
        CHECK_DW_ERROR_MSG(dwRig_getGenericVehicle(&m_vehicle, m_rigConfig), "Error reading vehicle data");
    ...
    bool runLoop()
    {
        ...
        {
            if (std::stoi(getArgument("calibrate-wheel-radii")) != 0)
            {
                dwCalibrationEngine_getVehicleWheelRadius(&m_vehicle.axleFront.wheelRadiusLeft, DW_VEHICLE_WHEEL_FRONT_LEFT, m_calibRoutine, m_calibEngine);
                dwCalibrationEngine_getVehicleWheelRadius(&m_vehicle.axleFront.wheelRadiusRight, DW_VEHICLE_WHEEL_FRONT_RIGHT, m_calibRoutine, m_calibEngine);
                dwCalibrationEngine_getVehicleWheelRadius(&m_vehicle.axleRear.wheelRadiusLeft, DW_VEHICLE_WHEEL_REAR_LEFT, m_calibRoutine, m_calibEngine);
                dwCalibrationEngine_getVehicleWheelRadius(&m_vehicle.axleRear.wheelRadiusRight, DW_VEHICLE_WHEEL_REAR_RIGHT, m_calibRoutine, m_calibEngine);
            }
        }
    ...
    void onProcess() override
    {
        ...
        if (!runLoop() || m_scanIndex >= MAX_RADAR_SCAN_CNT)
        {
            stop();
            return;
        }
    ...
    void onRender() override
    {
        ...
        if (getArgument("calibrate-wheel-radii") != "0")
        {
            ss << "wheelRadius[" << DW_VEHICLE_WHEEL_FRONT_LEFT << "] = " << m_vehicle.axleFront.wheelRadiusLeft * 1000.f << "[mm], ";
            ss << "wheelRadius[" << DW_VEHICLE_WHEEL_FRONT_RIGHT << "] = " << m_vehicle.axleFront.wheelRadiusRight * 1000.f << "[mm], ";
            ss << "wheelRadius[" << DW_VEHICLE_WHEEL_REAR_LEFT << "] = " << m_vehicle.axleRear.wheelRadiusLeft * 1000.f << "[mm], ";
            ss << "wheelRadius[" << DW_VEHICLE_WHEEL_REAR_RIGHT << "] = " << m_vehicle.axleRear.wheelRadiusRight * 1000.f << "[mm], ";
        }
    ...
    void onRelease() override
    {
        if (m_status.state == DW_CALIBRATION_STATE_ACCEPTED && m_rigOutFile.size() != 0)
        {
            ...
            if (std::stoi(getArgument("calibrate-wheel-radii")) != 0)
            {
                CHECK_DW_ERROR(dwRig_setGenericVehicle(&m_vehicle, m_rigConfig));
            }
...
int32_t main(int32_t argc, const char** argv)
{
    ...
    const std::string defRigFile =
        (dw_samples::SamplesDataPath::get() + "/samples/recordings/highway1/rig9Radars-wrong-yaw.json");
    ...
    ProgramArguments args(
        ...
        {Option("rig", defRigFile.c_str(), "Path to input rig calibration file."),
         ...
         Option("calibrate-wheel-radii",
                "1",
                "0: disabled, 1: will calibrate wheel radius")});
