...
public:
    ...
    void onRender() override
    {
        ...
        // To make calibration information observed in the render, we must keep currentHeight is > 0.
        float32_t currentHeight = getWindowHeight() - 20 * LINESPACING;
...
private:
    ...
    uint32_t getSensorIndex(dwSensorType sensorType, const std::string& sensorSelector, const std::string& sensorTypeString) const
    {
        ...
        {
            std::cout << "Error: Failed to get sensor index for sensor " << sensorSelector << " of type " << sensorType << " (" << sensorTypeString << ")" << std::endl;
            throw e;
        }
    ...
    void initializeSensorManager()
    {
        ...
        m_cameraIndex                                       = getSensorIndex(DW_SENSOR_CAMERA, getArgument("camera-sensor"), "camera");
        smParams.enableSensors[smParams.numEnableSensors++] = m_cameraIndex;

        m_imuIndex                                          = getSensorIndex(DW_SENSOR_IMU, getArgument("imu-sensor"), "IMU");
        smParams.enableSensors[smParams.numEnableSensors++] = m_imuIndex;

        m_canIndex                                          = getSensorIndex(DW_SENSOR_CAN, getArgument("can-sensor"), "CAN");
