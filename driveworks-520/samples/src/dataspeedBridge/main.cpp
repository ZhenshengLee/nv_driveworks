...
{
    dwVioDrivePositionStatus drivePositionStatus = DW_VIO_DRIVE_POSITION_STATUS_FORCE32;
    assignFromMultipleSignals((dwSignal_checkSignalValidity(actuationFeedback.validityInfo.drivePositionStatus) == DW_SUCCESS),
                              (dwSignal_checkSignalValidity(nonSafeState.validityInfo.drivePositionStatus) == DW_SUCCESS),
                              drivePositionStatus,
                              actuationFeedback.drivePositionStatus,
                              nonSafeState.drivePositionStatus);

    dwVehicleIOGear gear = DW_VEHICLEIO_GEAR_UNKNOWN;
    if (drivePositionStatus != DW_VIO_DRIVE_POSITION_STATUS_FORCE32)
    {
        switch (drivePositionStatus)
        {
        case DW_VIO_DRIVE_POSITION_STATUS_R:
            gear = DW_VEHICLEIO_GEAR_REVERSE;
            break;
        case DW_VIO_DRIVE_POSITION_STATUS_N:
            gear = DW_VEHICLEIO_GEAR_NEUTRAL;
            break;
        case DW_VIO_DRIVE_POSITION_STATUS_D:
            gear = DW_VEHICLEIO_GEAR_DRIVE;
            break;
        case DW_VIO_DRIVE_POSITION_STATUS_P:
            gear = DW_VEHICLEIO_GEAR_PARK;
            break;
        case DW_VIO_DRIVE_POSITION_STATUS_FORCE32:
        default:
...
static void encodeGenericGearReport(dwCANMessage& report, const dwVehicleIONonSafetyState& nonSafeState, const dwVehicleIOActuationFeedback& actuationFeedback)
{
    ...
    if (dwSignal_checkSignalValidity(actuationFeedback.validityInfo.drivePositionStatus) == DW_SUCCESS ||
        dwSignal_checkSignalValidity(nonSafeState.validityInfo.drivePositionStatus) == DW_SUCCESS)
    {
