...
private:
    ...
    std::unique_ptr<dwDNNTensorHandle_t[]> m_outputsHost;
...
protected:
    ...
    void initDNN()
    {
        ...
        m_outputsHost.reset(new dwDNNTensorHandle_t[m_numOutputTensors]);
        dwDNNTensorProperties inputProps;
        dwDNNTensorProperties outputProps[m_numOutputTensors];
    ...
    void doInference()
    {
        ...
        CHECK_DW_ERROR(dwDNN_infer(m_dnnOutputsDevice, m_numOutputTensors, inputs, 1U, m_dnn));
        ...
        for (uint32_t outputIdx = 0U; outputIdx < m_numOutputTensors; ++outputIdx)
        {
            ...
            CHECK_DW_ERROR(dwDNNTensorStreamer_consumerReceive(&m_outputsHost[outputIdx], 1000,
                                                               streamer));
        }
        ...
        if (m_usecuDLA)
        {
            interpretOutput(m_outputsHost[m_cvgIdx], m_outputsHost[m_bboxIdx],
                            &m_detectionRegion);
        }
        else
        {
            interpretOutput(m_outputsHost[m_cvgIdx],
                            &m_detectionRegion);
        }
    }

    void returnStreamedOutput()
    {
        // Return streamed outputs
        for (uint32_t outputIdx = 0U; outputIdx < m_numOutputTensors; ++outputIdx)
        {
            dwDNNTensorStreamerHandle_t streamer = m_dnnOutputStreamers[outputIdx];
            CHECK_DW_ERROR(dwDNNTensorStreamer_consumerReturn(&m_outputsHost[outputIdx], streamer));
