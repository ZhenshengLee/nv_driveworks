...
public:
    ...
    void printStats()
    {
        ...
        if (this->c2cEnabled())
        {
            if (accumulateLatency < 0)
            {
                std::cout << "Warning! C2C connection is enabled but the ptp is not synced! cause the accumulateLatency is abnormal!" << std::endl;
            }
        }
        ...
        if (m_hasProducer)
        {
            ...
            if (m_type == "NVSCI")
            {
                if (!m_prodStreamNames.empty())
                {
                    ...
                    params.c2cMode = (m_prodReaches.find("chip") != std::string::npos ? true : false);
                }
        ...
        for (size_t consId = 0; consId < m_numConsumers; consId++)
        {
            ...
            if (m_type == "NVSCI")
            {
                if (consId < m_consStreamNames.size())
                {
                    ...
                    params.c2cMode = (m_consReaches[consId] == "chip" ? true : false);
                }
            }
            else if (m_type == "SOCKET")
            {
        ...
        while (g_isRunning)
        {
            ...
            case ChannelEventType::GROUP_CONNECTED:
            {
                ...
                if (m_lateAttaches.size() > 0U)
                {
                    ...
                    // clear m_lateAttaches to prevent another connect
                    m_lateAttaches.clear();
                }
                break;
            }
            default:
