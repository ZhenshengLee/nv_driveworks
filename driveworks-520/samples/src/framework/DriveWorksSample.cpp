...
#ifdef DW_USE_AVTRACE
#include <dwtrace/dw/trace/GlobalTracer.hpp>
#include <dwtrace/dw/trace/Trace.hpp>
#endif
    ...
    , m_commandLineInputActive(false)
{
    ...
    m_autoExit  = atoi(getArgument("autoExit").c_str());
...
{
    OffscreenMode offscreenMode = (offscreen) ? OffscreenMode::OFFSCREEN_RENDERING : OffscreenMode::ONSCREEN;
    initializeWindow(title, width, height, offscreenMode, samples);
}

//------------------------------------------------------------------------------
void DriveWorksSample::initializeWindow(const char* title, int width, int height, OffscreenMode offscreen, int samples)
{
    // -------------------------------------------
    // Initialize GL
    // -------------------------------------------
    if (offscreen == OffscreenMode::OFFSCREEN_NO_RENDERING)
    {
        return;
    }

    m_window.reset(WindowBase::create(title, width, height, static_cast<bool>(offscreen), samples));
...
void DriveWorksSample::initDWTrace(std::string& path)
{
    ...
    dw::trace::TracerConfig traceConfig(true, path.c_str(),
                                        channelMask, true, 8192,
                                        true /*nvtx enabled */, (uint32_t)tracingLevel,
