...
public:
     ...
     * @param offscreen If ONSCREEN, window is displayed and rendering is displayed.
     *                  If OFFSCREEN_RENDERING window will not be created, but full GL initialization happens.
     *                  Rendering will happen into an offscreen window.
     *                  If OFFSCREEN_NO_RENDERING, window is not created and no rendering occurs.
     * @param samples   Specifies the desired number of samples to use for subsampling
     **/
    void initializeWindow(const char* windowTitle, int width, int height, OffscreenMode offscreen = OffscreenMode::ONSCREEN, int samples = 0);
     ...
     *                  Rendering will happen into an offscreen window.
     *                  If False window is displayed and rendering is displayed.
     * @param samples   Specifies the desired number of samples to use for subsampling
     **/
    void initializeWindow(const char* windowTitle, int width, int height, bool offscreen, int samples = 0);
...
protected:
    ...
    /**
     * @brief getProcessPeriod Get the minimum time between calls to onProcess().
     */
    inline myclock_t::duration getProcessPeriod() const { return m_processPeriod; }

    /**
     * @brief getRenderPeriod Get the minimum time between calls to onRender().
     */
    inline myclock_t::duration getRenderPeriod() const { return m_renderPeriod; }

    /**
     * @brief should_AutoExit Get the value of m_autoExit.
     */
    bool should_AutoExit() { return m_autoExit; }
