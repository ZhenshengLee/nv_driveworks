...
bool DwCGFCameraInterprocessApp::onInitialize()
{
    ...
    if (!m_isProducer)
    {
        try
        {
            ...
            if (hStr.empty() || wStr.empty())
                throw std::runtime_error("invalid encode width or height");

            int32_t const h = std::stoi(hStr.c_str());
            int32_t const w = std::stoi(wStr.c_str());
    ...
    ProgramArguments args(argc, argv,
                          {
                              ...
                              ProgramArguments::Option_t("encode-height", "", "encoded image height, it should match the NvSciBufImageAttrKey_SurfHeightBase value"),
                              ProgramArguments::Option_t("encode-width", "", "encoded image width, it should match the NvSciBufImageAttrKey_SurfWidthBase value"),
                          },
