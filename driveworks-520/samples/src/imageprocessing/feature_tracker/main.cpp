...
private:
    ...
    dwImageHandle_t m_inputImageHandle{DW_NULL_HANDLE};
...
// -----------------------------------------
        ...
        // ------------------------------------------------------------------
        {
            ...
            {
                dwImageProperties props{m_cameraImageProps};
                props.meta.flags |= DW_IMAGE_FLAGS_MAPS_CUPVA;
                CHECK_DW_ERROR(dwImage_create(&m_inputImageHandle, props, m_context));
            }
        ...
        // -----------------------------
        {
            ...
            if (pvaDetector)
            {
                CHECK_DW_ERROR(dwFeature2DDetector_registerInputNccScore(d_nccScores, maxFeatureCount, detector));
                dwFeatureArray tempFeatureArray{};
                for (uint32_t histIdx{0}; histIdx < (trackerConfig.historyCapacity); histIdx++)
                {
                    CHECK_DW_ERROR(dwFeatureHistoryArray_get(&tempFeatureArray, histIdx, &featureHistoryGPU));
                    CHECK_DW_ERROR(dwFeature2DDetector_registerFeatureArray(&tempFeatureArray, detector));
                }
                CHECK_DW_ERROR(dwFeature2DDetector_registerFeatureArray(&featuresDetected, detector));
            }

            if (pvaTracker)
            {
                CHECK_DW_ERROR(dwFeature2DTracker_registerOutputNccScore(d_nccScores, maxFeatureCount, tracker));

                CHECK_DW_ERROR(dwFeature2DTracker_registerHistoryArray(&featureHistoryGPU, tracker));
            }

#endif

            // Initialize pyramid
            dwTrivialDataType pyramidType = (m_usePvaPyramid || pvaDetector || pvaTracker) ? DW_TYPE_UINT8 : DW_TYPE_FLOAT32;
    ...
    void onRelease() override
    {
        ...
        // release feature tracker
        {
            ...
            if (m_usePvaPyramid)
            {
                if (m_inputImageHandle)
                {
                    CHECK_DW_ERROR(dwImage_destroy(m_inputImageHandle));
                }
            }
    ...
    void onProcess() override
    {
        ...
        if (1 < m_customDownscale)
        {
            CHECK_DW_ERROR(dwImageTransformation_copy(m_processImageDownscale, m_imageYUV, nullptr, nullptr, m_imageTransformer));
        ...
        if (m_usePvaPyramid)
        {
            CHECK_DW_ERROR(dwImage_copyConvert(m_inputImageHandle, trackImage, m_context));
            trackFrame(m_inputImageHandle);
        }
        else
        {
    ...
    ProgramArguments args(argc, argv,
                          {
                              ...
                              ProgramArguments::Option_t("pyramidLevel", "",
                                                         "the number of pyramid levels while tracking; use the default value if empty, default value is 5."
                                                         "If PVA pyramid is enabled, only pyramidLevel 3 to 6 is supported."
