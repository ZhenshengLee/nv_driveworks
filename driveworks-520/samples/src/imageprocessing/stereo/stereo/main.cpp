...
public:
    ...
    void mapToROI(dwImageCUDA* const dstImg, dwImageCUDA const* const srcImg, dwRect const roi)
    {
        dstImg->prop.memoryLayout = srcImg->prop.memoryLayout;
        for (uint32_t i{0U}; i < static_cast<uint32_t>(DW_MAX_IMAGE_PLANES); ++i)
        {
            dstImg->pitch[i] = srcImg->pitch[i];
        }

        dwVector2ui planeSizes[DW_MAX_IMAGE_PLANES];
        uint32_t channelSize[DW_MAX_IMAGE_PLANES] = {};
        size_t elementSize{};
        size_t planeCount{};
        uint32_t planechannelCount[DW_MAX_IMAGE_PLANES];
        dwImage_getDataLayout(&elementSize,
                              &planeCount,
                              planechannelCount,
                              planeSizes,
                              &srcImg->prop);

        for (uint32_t i{0U}; i < planeCount; ++i)
        {
            dstImg->prop        = srcImg->prop;
            dstImg->prop.height = static_cast<uint32_t>(roi.height);
            dstImg->prop.width  = static_cast<uint32_t>(roi.width);

            auto dataOut = reinterpret_cast<uint8_t**>(&dstImg->dptr[i]);
            *dataOut     = reinterpret_cast<uint8_t*>(srcImg->dptr[i]) +
                       static_cast<uint32_t>(roi.y) * srcImg->pitch[i] +
                       static_cast<uint32_t>(roi.x) * channelSize[0];
        }
    }
    // -----------------------------------------------------------------------------
    ...
    void onProcess() override
    {
        ...
        for (uint32_t side = 0; side < DW_STEREO_SIDE_COUNT; ++side)
        {
            ...
            mapToROI(cudaCrop, rectifiedCuda[side], rect);
        }
...
private:
    ...
    bool readStereoImages(dwImageHandle_t gStereoImages[DW_STEREO_SIDE_BOTH])
    {
        ...
        if (m_inputTypeCamera)
        {
            ...
            mapToROI(stereoImage[0], stereoFrame, roi[0]);
            mapToROI(stereoImage[1], stereoFrame, roi[1]);
