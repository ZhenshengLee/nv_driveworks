...
public:
    ...
    dwImageStreamerHandle_t m_imageStreamer = DW_NULL_HANDLE;
    ...
    // Variables
    // ------------------------------------------------
    dwImageHandle_t m_rectifiedImage{}; // the image will store the result of the Rectifier::warp() method.
    dwImageHandle_t m_cameraImage[16];

    dwImageHandle_t m_rgbaRectifierImage{}; // the image will store the result of the rgba pixel format of the rectifierImage.
    ...
    dwImageHandle_t m_image;           // the original image from the camera.
    dwImageHandle_t m_rgbaImageCopy{}; // copy of the original image from the camera
...
bool RectifierApp::onInitialize()
{
    ...
    if (!initCameraModels())
    {
        logError("Camera models initialization failed");
        return false;
    }
    if (!initRectifier())
    {
...
{
    // return the previous frame to camera. So it will release the related memory allocation.
    if (m_frame)
    {
    ...
    CHECK_DW_ERROR(dwSensorCamera_getImage(&m_image, DW_CAMERA_OUTPUT_NATIVE_PROCESSED, m_frame));

    if (m_image != nullptr)
    {
        // convert to rgba pixel format.
        CHECK_DW_ERROR(dwImage_copyConvert(m_rgbaImageCopy, m_image, m_context));

        CHECK_DW_ERROR(dwRectifier_apply(m_rectifiedImage, m_image, nullptr, m_rectifier));
        if (m_frameCap)
        {
            CHECK_DW_ERROR(dwFrameCapture_appendFrame(m_rectifiedImage, m_frameCap));
        }
        CHECK_DW_ERROR(dwImage_copyConvert(m_rgbaRectifierImage, m_rectifiedImage, m_context));
    }
    else
    {
...
void RectifierApp::onRender()
{
    ...
    dwImageStreamerGL_producerSend(m_rgbaImageCopy, m_imageStreamer);
    dwStatus status = dwImageStreamerGL_consumerReceive(&imageGL, 500000, m_imageStreamer);
    ...
    else
    {
        ...
        // return frame
        dwImageStreamerGL_consumerReturn(&imageGL, m_imageStreamer);
    }

    CHECK_DW_ERROR(dwImageStreamerGL_producerReturn(nullptr, 30000, m_imageStreamer));

    // Render output image
    dwImageHandle_t frameGLOut;
    dwImageStreamerGL_producerSend(m_rgbaRectifierImage, m_imageStreamer);
    status = dwImageStreamerGL_consumerReceive(&frameGLOut, 500000, m_imageStreamer);
    if (status != DW_SUCCESS)
    {
        logError("Did not receive GL frame within 500ms");
    }
    else
    {
        ...
        // return frame
        dwImageStreamerGL_consumerReturn(&frameGLOut, m_imageStreamer);
    }
...
void RectifierApp::onRelease()
{
    ...
    if (m_rgbaRectifierImage)
        dwImage_destroy(m_rgbaRectifierImage);

    if (m_rgbaImageCopy)
        dwImage_destroy(m_rgbaImageCopy);
    ...
    status = dwImageStreamerGL_release(m_imageStreamer);
    if (status != DW_SUCCESS)
    {
...
//#######################################################################################
bool RectifierApp::initCameraModels()
{
    dwStatus status;
...
bool RectifierApp::initImageStreamer()
{
    ...
    CHECK_DW_ERROR(dwImageStreamerGL_initialize(&m_imageStreamer, &props, DW_IMAGE_GL, m_context));
...
bool RectifierApp::initRectifier()
{
    ...
    dwImagePool pool{};
    pool.images     = &m_rectifiedImage;
    pool.imageCount = 1;

    CHECK_DW_ERROR(dwRectifier_registerImages(pool, m_rectifier));

    props.format = DW_IMAGE_FORMAT_RGBA_UINT8;

    CHECK_DW_ERROR(dwImage_create(&m_rgbaRectifierImage, props, m_context));
    CHECK_DW_ERROR(dwImage_create(&m_rgbaImageCopy, props, m_context));
    ...
    auto status = dwSensorCamera_getImageProperties(&cameraImageProperties,
    ...
    CHECK_DW_ERROR(dwSensorCamera_appendAllocationAttributes(&cameraImageProperties, DW_CAMERA_OUTPUT_NATIVE_PROCESSED, m_cameraSensor));
    CHECK_DW_ERROR(dwRectifier_appendAllocationAttributes(&cameraImageProperties, m_rectifier));

    dwImagePool poolCam{};

    uint32_t numImages = 16;
    for (uint32_t i = 0; i < numImages; ++i)
    {
        CHECK_DW_ERROR(dwImage_create(&m_cameraImage[i], cameraImageProperties, m_context));
    }

    poolCam.imageCount = numImages;
    poolCam.images     = m_cameraImage;
    CHECK_DW_ERROR(dwRectifier_registerImages(poolCam, m_rectifier));

    CHECK_DW_ERROR(dwSensorCamera_setImagePool(poolCam, m_cameraSensor));
