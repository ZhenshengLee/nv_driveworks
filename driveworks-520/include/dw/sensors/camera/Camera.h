...
typedef enum dwCameraSIPLNotificationData {
    ...
    // Indicates out of order image is detected.
...
/**
 * Overrides the properties of the images in the internal pool. dwCameraImageOutputType specifies which pool to override
 *
 * @note The given imgProps should be compatible with that returned by
 *       dwSensorCamra_getImageProperties API.
 * @note The imgProps are read and used to generate dwImages from given imgProps for internal image pool.
 *       by the processing pipeline (ICP/ISP0/ISP1/ISP2).
 *       The allocation attributes are read from imgProps.meta.allocAttrs. Applications do not need to free or alter the
 *       imgProps.meta.allocAttrs in any way. The imgProps.meta.allocAttrs are only used
 *       If the application alters the imgProps after calling this API, the
 *       imgProps.meta.allocAttrs may no longer be applicable to the imgProps and calls related
 *       to allocating images will fail.
 ...
 *       DW and will be owned by DW context until context is destroyed
 *       and should be used wisely as it the space is limited.
* @note Must be called after dwSAL_start() and before dwSensor_start().
*
* @param[in] imageProperties A modifiable pointer to image properties of the frames captured by the camera.
* @param[in] outputType Format of the output image to get the properties of
* @param[in] sensor Sensor handle of the camera sensor previously created with
*            dwSAL_createSensor().
*
...
*/
DW_API_PUBLIC
dwStatus dwSensorCamera_setImageAttributes(dwImageProperties* const imageProperties,
                                           dwCameraOutputType const outputType,
                                           dwSensorHandle_t const sensor);
...
#ifndef DW_IS_SAFETY
/**
 * Disable the camera link. Only supported on non-safety builds.
 * This method should only be called after dwSensor_start() and before dwSensor_stop().
 *
 * @param[in] sensor A handle to the camera GMSL
 *
 * @return DW_NOT_SUPPORTED If current SDK/PDK version is not supported, or if used on a safety build
...
/**
 * Enable the camera link. Only supported on non-safety builds.
