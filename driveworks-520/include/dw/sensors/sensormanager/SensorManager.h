...
typedef struct
{
    ...
    const dwUltrasonicFrame* ultrasonicFrame; //!< Ultrasonic frame in this event.
} dwSensorEvent;
...
typedef void (*dwSensorManagerDispatcher_t)(const dwSensorEvent*, void*, dwStatus); //!< function pointer of sensormanager dispatcher.

/**
 * @brief Creates an instance of SensorManager module.
 *
 * @param[out] sm A pointer to the sm handle will be returned here.
 * @param[in] poolSize Size of the internal event pool to be allocated. Has to be greater than 0
 * @param[in] sal SAL handle instantiated by the caller
 *
 * @retval DW_INVALID_ARGUMENT if pointer to the SM handle is NULL. <br>
 * @retval DW_INVALID_HANDLE if provided context/SAL handle is invalid. <br>
...
/**
 * @brief Create and initialize a SensorManager module, adding all
 *        sensors in the provided Rig Configuration. That means SensorManager will create all enabled sensors in the Rig Configuration
 *        and add them to SensorManager handle created by this api in order to manage them in SensorManager.
 *
 * @param[out] sm A pointer to the sm handle will be returned here.
 ...
 *
 * @param[out] sm The sensor manager handle that needs to be released. This handle is created before by
 * 'dwSensorManager_initialize/dwSensorManager_initializeFromRig/dwSensorManager_initializeFromRigWithParams'.
 *
...
/**
 * @brief Adds a sensor to the SAL instance and add it to SensorManager to manage it. All addSensor
 *        calls must be completed before the dwSensorManager_start call.
 *
 * @param[in] params Specifies the parameters for sensor creation.
 * @param[in] clientData Client data to be added for this sensor. This parameters is assigned by users and saved with sensors added.
 ...
 * @retval DW_CALL_NOT_ALLOWED addSensor cannot add sensor while running
 *
 * @note This method will perform memory allocations.
 * @par API Group
 * - Init: Yes
...
/**
 * @brief Adds a camera sensor to the SAL instance and add it to SensorManager to manage it. All addCameraSensor()
...
/**
 * @brief Gets the number of sensors in the SensorManager instantiated for a given sensor type
 *
 ...
 * @param[in] sensorIndex Index of the sensor. This index can refers to sensorIndex in dwSensorEvent.
 * @param[in] sm SM handle instantiated by the caller
 *
 ...
 *
 * @param[out] sensorIndex Pointer to location where the sensor index should be returned. This index can refers to sensorIndex in dwSensorEvent.
 * @param[in] type Type of sensor being requested
 * @param[in] sensorTypeIndex Index of the sensor as defined by the dwSensorType. This index can refers to sensorTypeIndex in dwSensorEvent.
 * @param[in] sm SM handle instantiated by the caller
 *
 ...
 * the dwSensorType should be returned. This index can refers to sensorTypeIndex in dwSensorEvent.
...
/**
 * @brief Register sensor's dispatcher function when the feature is turned on.
 * 
 * When users register a dispatcher function. Once an event is already, the function registered will be called.
 * That means SensorManager will help users to deal events using the function registered. Users needn't to call
 * dwSensorManager_acquireNextEvent to acquire event and deal it.
 *
 ...
 * @retval DW_INVALID_HANDLE if provided SM handle is invalid. <br>
 * @retval DW_SUCCESS Register successfully.
 * @par API Group
 * - Init: Yes
 * - Runtime: Yes
 * - De-Init: Yes
 ...
 * @param[in] sm SM handle instantiated by the caller
 *
 * @retval DW_INVALID_ARGUMENT SM handle is nullptr<br>
 * @retval DW_INVALID_HANDLE if provided SM handle is invalid. <br>
