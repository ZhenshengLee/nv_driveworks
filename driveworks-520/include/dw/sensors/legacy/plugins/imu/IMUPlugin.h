 ...
 *
 */
DW_DEPRECATED("dwSensorIMUPlugin_parseDataBuffer is deprecated and will be removed in next major release. Please use dwSensorIMUPlugin_parseDataBufferNew instead")
 ...
 * @param[out] frame Interpreted IMU frame
 * @param[out] consumed Number of raw bytes (including header) consumed to successfully parse the frame
 * @param[out] outDomain output timedomain for frame.timestamp_us
 * @param[in] sensor Specifies the sensor the data came from.
 *
 * @return DW_INVALID_HANDLE - if the sensor handle is NULL or invalid <br>
 *         DW_NOT_AVAILABLE - if no frame is ready for consumption
 ...
 *
 */
typedef dwStatus (*dwSensorIMUPlugin_parseDataBufferNew)(dwIMUFrame* frame, size_t* consumed, dwSensorPlugin_timeDomain* outDomain, dwSensorPluginSensorHandle_t sensor);
