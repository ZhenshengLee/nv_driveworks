...
typedef struct dwImageSensorStatistics
{
    ...
    /**
     * Holds the luminance calibration matrix for the sensor. (scalar value)
...
typedef struct dwImageTimestamps
{
    ...
    /// Specifies the TSC time, in microseconds, when the last row of the image was fully
    /// exposed in TSC time domain, i.e., the time at the end of the last row’s exposure.
 ...
 *
 * @return DW_SUCCESS if the image was created successfully<br>
 ...
 * @param[in] image A handle to the image
 *
 * @return DW_SUCCESS if operation completed successfully<br>
 ...
 *         DW_SUCCESS - if operation completed successfully<br>
 *
 **/
DW_DEPRECATED("dwImageCUDA_getPlaneAsImage has been deprecated because not compliant with the API of this module and it's not usable with other modules.")
 ...
 **/
DW_API_PUBLIC
DW_DEPRECATED("dwImageCUDA_maptToiROI has been deprecated because not compliant with the API of this module and it's not usable with other modules. Call dwImage_getCUDA and adjust the pointers in app to mimic this API.")
dwStatus dwImageCUDA_mapToROI(dwImageCUDA* const dstImg, dwImageCUDA const* const srcImg, dwRect const roi);
 ...
 *
 * @note The memory buffer lifetime is still controlled by the application. The application must take
 *       care that the buffer is not freed until the returned dwImageHandle_t is freed.
 *
 * @param[out] image A handle to the image
 * @param[in] properties The image properties.
 * @param[in] nvscibufObj The NvSciBufObj to bind to.
 * @param[in] ctx The DriveWorks context.
 * @return DW_SUCCESS if the image was created, <br>
 *         DW_NOT_SUPPORTED if the platform does not support this API <br>
 *         DW_INVALID_ARGUMENT if the given image types are invalid or the image pointer is null. <br>
 * @par API Group
 * - Init: Yes
 * - Runtime: No
 * - De-Init: No
 */
DW_API_PUBLIC
dwStatus dwImage_createAndBindNvSciBuf(dwImageHandle_t* const image,
 ...
 *
 * @param [out] nvSciBufObj pointer to output NvSciBufObj.
 * @param [in]  image A handle to the image.
 *
 * @return DW_SUCCESS if operation succeeded.
 *         DW_INVALID_ARGUMENT if nvSciBufObj or image is null.
 *         DW_NOT_AVAILABLE if image is not backed by NvSciBufObj.
 * @par API Group
 * - Init: Yes
 * - Runtime: Yes
 * - De-Init: Yes
 */
DW_API_PUBLIC
dwStatus dwImage_getNvSciBuf(NvSciBufObj* nvSciBufObj, dwImageHandle_t image);
