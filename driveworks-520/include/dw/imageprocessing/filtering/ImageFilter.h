...
/**
 * Allocates resources for an image filter of a certain type with a working CUDA stream, expecting an image of a specific size.
 * @param[out] filter Pointer to an image filter handle
 * @param[in] config Specifies the configuration parameters
 ...
 */
DW_API_PUBLIC
dwStatus dwImageFilter_initializeWithCUDAStream(dwImageFilterHandle_t* filter, const dwImageFilterConfig* config, cudaStream_t stream, dwContextHandle_t context);
