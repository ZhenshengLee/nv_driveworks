...
#ifndef DW_IMAGEPROCESSING_FEATUREDESCRIPTOR_FEATURE_DESCRIPTOR_H_
#define DW_IMAGEPROCESSING_FEATUREDESCRIPTOR_FEATURE_DESCRIPTOR_H_
...
typedef struct dwFeature2DDescriptorConfig
{
    ...
    /**
     * Data type for the descriptor
     **/
    dwTrivialDataType dataType;

    /**
     * Dimension for the descriptor
     **/
...
typedef enum dwFeature2DDescriptorStage {
    ...
    /**
     *  CPU Async stage. To be used by CPU implementation
     **/
    DW_FEATURE2D_DESCRIPTOR_STAGE_CPU_ASYNC,
    /**
     * CPU preprocess
     **/
...
*         DW_NOT_SUPPORTED if platform does not support. <br>
*         DW_NOT_AVAILABLE if module not available in platform. <br>
*         DW_SUCCESS otherwise.<br>
**/
DW_API_PUBLIC
...
**/
DW_API_PUBLIC
dwStatus dwFeature2DDescriptor_getCUDAStream(cudaStream_t* stream, dwConstFeature2DDescriptorHandle_t obj);
...
*         DW_SUCCESS otherwise.<br>
*
* @note `dwFeature2DDescriptor_bindBuffers` or `dwFeature2DDescriptor_bindInput` must be called at least once before `dwFeature2DDescriptor_processImage`.
**/
DW_API_PUBLIC
dwStatus dwFeature2DDescriptor_bindBuffers(dwImageHandle_t image,
...
* @param[inout] featureArray Features whose descriptor to be calculated.
* @param[in] obj Specifies the feature descriptor handle.
*
* @return DW_INVALID_ARGUMENT if descriptor handle, features or image are NULL.<br>
*         DW_INVALID_ARGUMENT if features are not allocated according to descriptor object. GPU for PVA and CPU for CPU implementation.<br>
*         DW_NOT_SUPPORTED if platform does not support. <br>
*         DW_SUCCESS otherwise.<br>
*
* @note `dwFeature2DDescriptor_bindInput` or `dwFeature2DDescriptor_bindBuffers`  must be called at least once before `dwFeature2DDescriptor_process`.
**/
DW_API_PUBLIC
dwStatus dwFeature2DDescriptor_bindInput(const dwPyramidImage* const pyramid,
                                         const dwFeatureArray* featureArray,
                                         dwFeature2DDescriptorHandle_t obj);

/**
* Do detections on the image bound by `dwFeature2DDescriptor_bindBuffers` or `dwFeature2DDescriptor_bindInput`, output will be written to
* `dwFeatureDescriptorArray` bound by `dwFeature2DDescriptor_bindOutput`.
*
* @param[in] stage Specifies the detecting stage
* @param[in] obj Specifies the feature descriptor handle.
*
* @return DW_INVALID_ARGUMENT if there's no output dwFeatureDescriptorArray bound to descriptor.<br>
*                             if there's no input image or features bound to descriptor.<br>
*         DW_SUCCESS otherwise.<br>
**/
DW_API_PUBLIC
dwStatus dwFeature2DDescriptor_process(dwFeature2DDescriptorStage stage,
                                       dwFeature2DDescriptorHandle_t obj);
 ...
 * @note The ownership of the stream remains by the callee.
 *
 * @param[in] stream The CUPVA stream to be used. Default is the one passed during dwFeature2DDescriptor_initialize.
 * @param[in] obj A handle to the feature Descriptor module to set CUPVA stream for.
 *
 * @return DW_INVALID_ARGUMENT if feature Descriptor handle is NULL.<br>
 *         DW_NOT_AVAILABLE if not available for platform. <br>
 *         DW_SUCCESS otherwise.<br>
 **/
DW_API_PUBLIC
dwStatus dwFeature2DDescriptor_setPVAStream(cupvaStream_t stream, dwFeature2DDescriptorHandle_t obj);
 ...
 **/
DW_API_PUBLIC
dwStatus dwFeature2DDescriptor_getPVAStream(cupvaStream_t* stream, dwConstFeature2DDescriptorHandle_t obj);

/**
 * Set descriptor rotation lock option. This turns off rotation invariance for feature descriptors.
 *
 * @param[in] orientationLock Specifies to lock orientation or not during next process operation
 * @param[in] obj Specifies the feature descriptor extractor handle.
 *
 * @return DW_INVALID_HANDLE if obj is invalid handle.<br>
 *         DW_SUCCESS otherwise.<br>
 */
DW_API_PUBLIC
dwStatus dwFeature2DDescriptor_setOrientationLock(bool const orientationLock,
...
#endif
/** @} */
#endif // DW_IMAGEPROCESSING_FEATUREDESCRIPTOR_FEATURE_DESCRIPTOR_H_