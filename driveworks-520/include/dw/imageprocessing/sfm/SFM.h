...
/**
 * @brief Dryrun the reconstructor estimation on GPU to record CUDAGraph.
 * SFM uses GPU for the estimation. The GPU implementation 
 ...
 * @param[in] obj The reconstructor object handle
 * 
 * @retval DW_INVALID_HANDLE when provided reconstructor handle is invalid, i.e., null or of wrong type
 * @retval DW_CUDA_ERROR recording failed
 * @retval DW_SUCCESS when operation succeeded
 * 
 * @note safety build MUST call this API before dwReconstructor_estimatePoseAsync, 
 *        Non-safety platform can call dwReconstructor_estimatePoseAsync directly
 * 
 * \ingroup sfm
 * @par API Group
 * - Init: Yes
 ...
 */
DW_API_PUBLIC
dwStatus dwReconstructor_dryrunEstimation(dwReconstructorHandle_t obj);
