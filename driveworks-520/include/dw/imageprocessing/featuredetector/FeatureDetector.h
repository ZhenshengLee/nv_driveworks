 ...
 *             detections are appended after old features. Must be created by `dwFeatureArray_createNew()` with
 ...
 *               The `dwFeatureArray` can either be an empty array allocated by `dwFeatureArray_createNew` or a
 ...
 * @param[in] obj Specifies the feature detector handle.
 *
 * @return DW_INVALID_ARGUMENT if detector handle or preTrackedFeatures are NULL.<br>
 *         DW_INVALID_ARGUMENT if preTrackedFeatures are not allocated on GPU.<br>
 *         DW_CALL_NOT_ALLOWED if target is safety platform and useCudaGraph is on, either outputDetections, preTrackedFeatures or d_normalizedCrossCorrelation's address changes<br>
 *         DW_SUCCESS otherwise.<br>
 *
 * @note In safety platform, if useCudaGraph is on, dwFeature2DDetector_dryrunDetectionImage must be called at least one time before 
 *       any dwFeature2DDetector_detectFromImage in INIT state, elsewise dwFeature2DDetector_detectFromImage may have unexpected behavior.
 * 
 ...
 *         DW_SUCCESS otherwise.<br>
 *
 * @note In safety platform, if useCudaGraph is on, dwFeature2DDetector_dryrunDetectionPyramid must be called at least one time before 
 *       any dwFeature2DDetector_detectFromPyramid in INIT state, elsewise dwFeature2DDetector_detectFromPyramid may have unexpected behavior.
 *      
 *       If there're N features in preTrackedFeatures,
 *       for DW_FEATURE2D_DETECTOR_TYPE_STD, the first N features in detectedFeatures are from preTrackedFeaturs,
 *       new detections are appended after old ones.
 ...
 *         DW_SUCCESS otherwise.<br>
 *
 * @note This API MUST be called at least one time in INIT state in safety platform if if useCudaGraph is on, 
 *       elsewise dwFeature2DDetector_detectFromImage may have unexpected behavior.
 *       For PVA detector, there's no such restriction.
 * 
 * @par API Group
 * - Init: Yes
 * - Runtime: Yes
...
*/
DW_API_PUBLIC
dwStatus dwFeature2DDetector_dryrunDetectionImage(dwFeatureArray* outputDetections,
                                                  dwFeatureArray* preTrackedFeatures,
                                                  const float32_t* d_normalizedCrossCorrelation,
                                                  dwFeature2DDetectorHandle_t obj);
 ...
 * @param[in] obj Specifies the feature detector handle.
 *
 * @return DW_INVALID_ARGUMENT if detector handle or preTrackedFeatures.<br>
 *         DW_INVALID_ARGUMENT if preTrackedFeatures are not allocated on GPU.<br>
 *         DW_SUCCESS otherwise.<br>
 *
 * @note This API MUST be called at least one time in INIT state in safety platform if useCudaGraph is on,
 *       elsewise dwFeature2DDetector_detectFromPyramid may have unexpected behavior.
 *       For PVA detector, there's no such restriction.
 * 
 * @par API Group
 * - Init: Yes
...
*/
DW_API_PUBLIC
dwStatus dwFeature2DDetector_dryrunDetectionPyramid(dwFeatureArray* outputDetections,
                                                    dwFeatureArray* preTrackedFeatures,
                                                    const float32_t* d_normalizedCrossCorrelation,
                                                    dwFeature2DDetectorHandle_t obj);
...
/**
* @brief Binds input ncc Score. Internally the cuda pointer to nccScore is mapped to pva address space using the cupva cuda interop api
* 
* @param[in] nccScore cuda pointer to Input NccScore
* @param[in] obj A handle to the feature detector module.
* @param[in] nccScoreArraySize Number of elements in the ncc Score Array.
*
* @return DW_NOT_SUPPORTED if the api is called for non pva platform.
*         DW_SUCCESS otherwise.<br>
* @par API Group
* - Init: Yes
* - Runtime: Yes
...
DW_API_PUBLIC
dwStatus dwFeature2DDetector_registerInputNccScore(float32_t const* nccScore, uint32_t nccScoreArraySize, dwFeature2DDetectorHandle_t obj);

/**
* @brief Binds feature Array. Internally the cuda pointer to featureArray is mapped to pva address space using the cupva cuda interop api
* 
* @param[in] inFeatures cuda pointer to Feature Array
* @param[in] obj A handle to the feature detector module.
*
* @return DW_NOT_SUPPORTED if the api is called for non pva platform.
*         DW_SUCCESS otherwise.<br>
...
DW_API_PUBLIC
dwStatus dwFeature2DDetector_registerFeatureArray(dwFeatureArray* inFeatures, dwFeature2DDetectorHandle_t obj);
