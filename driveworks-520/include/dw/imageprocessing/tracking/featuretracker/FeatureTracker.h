...
/**
* @brief Binds output ncc Score. Internally the cuda pointer to nccScore is mapped to pva address space using the cupva cuda interop api
* 
...
DW_API_PUBLIC
dwStatus dwFeature2DTracker_registerOutputNccScore(float32_t const* nccScore, uint32_t nccScoreArraySize, dwFeature2DTrackerHandle_t obj);

/**
* @brief Binds feature HistoryArray. Internally the cuda pointer to feature History Array is mapped to pva address space using the cupva cuda interop api
* 
* @param[in] historyArray cuda pointer to Complete feature History Array
* @param[in] obj A handle to the feature tracker module.
*
* @return DW_NOT_SUPPORTED if the api is called for non pva platform.
*         DW_SUCCESS otherwise.<br>
...
DW_API_PUBLIC
dwStatus dwFeature2DTracker_registerHistoryArray(dwFeatureHistoryArray* historyArray, dwFeature2DTrackerHandle_t obj);
...
DW_API_PUBLIC
dwStatus dwFeature2DTracker_registerFeatureArray(dwFeatureArray* inFeatures, dwFeature2DTrackerHandle_t obj);
