...
/**
 * @brief Type of polynomial stored in PinholeCamera.
 *
 * The PinholeCamera model distortion can either be defined
 * in forward direction following the photons from ray to pixel
 * or in backward direction from pixel to ray.
 *
 * This struct defines which of those two options a polynomial represents.
 */
typedef enum dwPinholeCameraPolynomialType {
    /**
     * Forward polynomial type,
     * mapping from normalized image coordinates to ray
     */
    DW_PINHOLE_CAMERA_POLYNOMIAL_TYPE_FORWARD = 0,

    /**
     * Backward polynomial type,
     * mapping from ray to normalized image coordinates
     */
 ...
 * `[k_1, k_2, k_3]` given in dwPinholeCameraConfig::distortion and type given in
 * dwPinholeCameraConfig::polynomialType, i.e.,
 *
...
*
 *     xn = xd / (1 + k_1 * r^2 + k_2 * r^4 + k_3 * r^6),
 *     xn = xd / (1 + k_1 * r^2 + k_2 * r^4 + k_3 * r^6),
 *
 * whereas
 *     r^2 = (xn^2 + yn^2) is iteratively estimated.
 *
...
typedef struct dwPinholeCameraConfig
{
     ...
     *  either maps in forward direction from ray to pixel
     *  or in backward direction from pixel to ray.
     */
