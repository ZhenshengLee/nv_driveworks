...
// This amounts to max(CalibrationPass) + 1, i.e. the required size of
...
typedef enum dwCalibrationPropertiesVariantType {
    ...
    DW_CALIBRATION_PROPERTY_VARIANT_SERVICE         = 4, //!< calibration values are produced by service calibration
    DW_CALIBRATION_PROPERTY_VARIANT_EOL             = 5, //!< calibration values are produced by end-of-line calibration in the factory
...
typedef struct dwCalibrationHistogramEstimatorParameters
{
    ...
    /// Fast acceptance +/- *value* envelope around reference that is being fast-accepted, 0 or negative to disable fast acceptance (signal units, FastAcceptanceRejectionEstimator only)
    float32_t fastAcceptanceThreshold;

    /// When estimating the error bound of the signal, add this as a base value to represent unmodeled/general uncertainty
    float32_t baseErrorBound;

    /// Fast rejection +/- *value* maximum allowed displacement of the short histogram peak, negative to disable fast rejection (signal units, FastAcceptanceRejectionEstimator only)
    float32_t fastRejectionShortTermPeakMovementThreshold;
