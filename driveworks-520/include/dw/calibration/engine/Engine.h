 ...
 *
 * @retval DW_INVALID_ARGUMENT when the arguments in the parameters are invalid. In particular:
 *         - if params is null,
 *         - if the sensorIndex does not correspond to a camera in the rig. Note that,in case the given index does not specify
 *           a camera, but params.calibratedCamera is nullptr, it will return DW_CANNOT_CREATE_OBJECT.
 *         - if params.cameraProperties is null
 *         - if params.method specify a value different from DW_CALIBRATION_CAMERA_METHOD_FEATURES (the only supported method at the moment)
 *         - if params.signals is not if neither DW_CALIBRATION_CAMERA_SIGNAL_DEFAULT nor DW_CALIBRATION_CAMERA_SIGNAL_PITCHYAW is set.
 *         - if params.features.maxFeatureCount is zero
 *         - if params.features.maxFeatureHistorySize is zero
 * @retval DW_CANNOT_CREATE_OBJECT if params.calibratedCamera is null and the sensorIndex specify a wrong entry in the rig that
 *         does not allow to instantiate properly a camera model
 * @retval DW_INVALID_HANDLE when provided routine, egomotion or engine handle are invalid, i.e., null or of wrong type
 ...
 *        A radar calibration routine can also estimate odometry properties (i.e. wheel radii)
 ...
 **/
DW_API_PUBLIC
DW_DEPRECATED("Vehicle Calibration has been deprecated and will be removed/replaced by the next major release")
...
/**
 * @brief Dryrun the routine estimation GPU to record CUDAGraph.
 * Some of the calibration routines uses GPU for the estimation. The GPU implementation
 ...
 *
 * @param[in] routine Specifies the handle of the calibration routine
 * @param[in] engine Specifies the handle of the calibration engine that is managing the calibration
 *
 * @retval DW_INVALID_HANDLE when provided engine handle or routine is invalid, i.e., null or of wrong type
 * @retval DW_CUDA_ERROR recording failed
 * @retval DW_SUCCESS when operation succeeded
 *
 * @note safety platform MUST call this API before dwCalibrationEngine_startCalibration with the given routine
 *       Non-safety platform can call dwCalibrationEngine_startCalibration directly
 * @par API Group
 * - Init: Yes
 * - Runtime: Yes
 * - De-Init: Yes
 */
DW_API_PUBLIC
dwStatus dwCalibrationEngine_dryrunRoutine(dwCalibrationRoutineHandle_t routine,
                                           dwCalibrationEngineHandle_t engine);
 ...
 *
 * @retval DW_INVALID_ARGUMENT when the status handle is nullptr, or calibration routine is not managed by the calibration engine
 ...
 * @note A valid calibration engine handle has to be provided.
 *
 * @retval DW_INVALID_ARGUMENT when signals handle is null, or the calibration routine is not managed by the calibration engine or supported is null
 * @retval DW_INVALID_HANDLE when any of the provided handles is invalid, i.e., null or of wrong type
 * @retval DW_SUCCESS when operation succeeded
 * @par API Group
 * - Init: Yes
 ...
 * @param[in] engine A pointer to the calibration engine handle that owns the sensor
 *
 * @retval DW_INVALID_ARGUMENT when given `radius` pointer is invalid
 * @retval DW_OUT_OF_BOUNDS when wheel is invalid
 * @retval DW_INVALID_HANDLE when any of the provided handles is invalid
 * @retval DW_NOT_AVAILABLE when calibration routine is not estimating wheel radius
 * @retval DW_SUCCESS when operation succeeded
 * @par API Group
 * - Init: Yes
 * - Runtime: Yes
