...
/// @{
/**
 * @brief Get the address of the pointer @a p, which is simply @a p.
...
/**
 * @brief Get the address of fancy pointer @c TPtr if it has custom @c pointer_traits with a @c to_address function.
 *
 * @param[in] p The pointer.
 *
 * @return The address of the pointer.
...
/**
 * @brief Get the address of fancy pointer or iterator @c TPtr by using its @c operator->.
 *
 * @note Unlike @c std::to_address, this overload only participates in cases where @c TPtr::operator-> directly returns a
 * pointer.
 *
 * @param[in] p The pointer.
 *
 * @return The address of the pointer.
*/
// While @c std::to_address recursively follows @c operator->, this implementation will not work for cases
