...
#define DW_STRINGIFY(text) #text
    ...
    #define DW_THREAD_LOCAL thread_local
    ...
    #if (__GNUC__ * 10000 + __GNUC_MINOR__ * 100) >= 90000 // gcc >= 9.0.0
        #ifdef __COVERITY__
            // coverity[autosar_cpp14_a1_1_1_violation] RFD Accepted: TID-2164
            #define DW_FALLTHROUGH
        #else
        ...
        #endif
    #else
        #define DW_FALLTHROUGH  // QNX SDP 7.1 uses GCC 8.3 as of 8/10/23
    #endif
#else
...
#else
    // CUDA_BOTH is defined as empty for non-nvcc so that we can define a CUDA_BOTH function in a header and include in both nvcc- and gcc-compiled files.
...
// If AddressSanitizer is enabled
#if defined(DW_COMPILER_IS_GCC) && defined(__SANITIZE_ADDRESS__)
#define DW_ADDRESS_SANITIZER_ON 1
#else
// Define stubs for macros defined in sanitizer/asan_interface.h

/// Marks a memory region as unaddressable. No-op without AddressSanitizer.
...
/// Marks a memory region as addressable. No-op without AddressSanitizer.
/// \param addr Start of memory region.
/// \param size Size of memory region in bytes
