...
// Simplified cast assuming input is a positive float64_t. Added only because neither SafetyResult nor
// Optional can be used due to a circular dependency.
template <typename TFloat>
int64_t floatToSignedInt(TFloat const& x)
{
    // One bit less due to sign bit.
    constexpr float64_t PRECISION{63.F};

    if (std::isnan(x) || PRECISION < std::log2(std::fabs(static_cast<TFloat>(x))))
    {
        return -1;
    }
...
// Optional can be used due to a circular dependency.
template <typename TFloat>
std::pair<uint64_t, bool> floatToUnsignedInt(TFloat const& x)
{
    // All 64 bits can be used for representation
    constexpr float64_t PRECISION{64.F};

    if (std::isnan(x) || x < 0.F || PRECISION < std::log2(std::fabs(static_cast<TFloat>(x))))
    {
        return std::make_pair(0, false);
    }

    return std::make_pair(static_cast<int64_t>(x), true);
}
...
public:
    ...
    auto operator+=(uint64_t unumber) noexcept -> StringBuffer<BufferSize, TChar, StringContainer>&
    {
        try
        {
            ...
            // Initial push needed for the case where unumber equals 0
            push_back(static_cast<TChar>(static_cast<uint8_t>(unumber % BASE) + ZERO_CHAR));
            unumber /= BASE;
    ...
    auto appendNum(float64_t fnumber, uint32_t const decimalPlaces, bool useScientific = false) noexcept -> StringBuffer<BufferSize, TChar, StringContainer>&
    {
        ...
        if (useScientific)
        {
            float64_t const flog10{std::floor(std::log10(floatVal))};

            // Casting result of flog10, so we can be sure there will be no overflow
            int64_t exponent{detail::floatToSignedInt(flog10)};
            float64_t const fpow10{this->safePowOf10(exponent)};

            floatVal /= fpow10; // floatVal is now in format a.xxxxxx where 1 <= a < 10
        }

        // Given the conditions above, floatVal is either converted to format a.xxxxxx (where 1 <= a < 10)
        // or it is 0 <= floatVal <= std::numeric_limits<uint32_t>::max()
        std::pair<uint64_t, bool> floatValAsUint{detail::floatToUnsignedInt(std::floor(floatVal))};
        uint64_t unumber{floatValAsUint.first};
        float64_t const decimal10{this->safePowOf10(decimalPlaces)};

        uint64_t floatingPart{detail::floatToUnsignedInt(std::round((floatVal - std::floor(floatVal)) * decimal10)).first};

        uint64_t decimal10AsInt{detail::floatToUnsignedInt(decimal10).first};
        if (floatingPart == decimal10AsInt)
        {
            // The decimals rounded up to the next unit
            unumber += detail::StringBufferImplValues::One<uint64_t>::value;
            floatingPart -= decimal10AsInt;
        }
        ...
        if (useScientific)
        {
            ...
            float64_t const flog10{std::floor(std::log10(fnumber))};
            // Casting result of log10, so we can be sure there will be no overflow
            int64_t const exponent{detail::floatToSignedInt(flog10)};

            if (exponent > detail::StringBufferImplValues::Zero<int64_t>::value)
            {
...
protected:
    ...
    /// return 10^y with some simplified assumptions about the exponent (limited domain errors)
    constexpr float64_t safePowOf10(int64_t exponent)
    {
        constexpr float64_t TEN_F{10.0};
        float64_t result{1.0};
        uint64_t e{exponent >= 0 ? static_cast<uint64_t>(exponent) : static_cast<uint64_t>(-exponent)};
...
// coverity[autosar_cpp14_a14_5_3_violation] Deviation Record: AV-DriveWorks_Core_Library-dwfoundation_core_base-SWSADR-0004
