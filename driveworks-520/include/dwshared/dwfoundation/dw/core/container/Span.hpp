...
public:
    ...
    /// Empty constructor
    // coverity[autosar_cpp14_a1_1_1_violation] RFD Pending: TID-2194
    CUDA_BOTH_INLINE constexpr span(std::nullptr_t) // clang-tidy NOLINT(cppcoreguidelines-pro-type-member-init, google-explicit-constructor)
...
// -----------------------------------------------------------------------------
/// copy elements for 1-dimensional spans.
template <typename Td, typename Ts>
void copyElements(dw::core::span<Td> const dst, dw::core::span<Ts> const src)
{
...
// -----------------------------------------------------------------------------
/// copy elements for n-dimensional spans.
template <typename Td, typename Ts, size_t DIMS>
void copyElements(dw::core::span<Td, DIMS> dst, dw::core::span<Ts, DIMS> src)
{
    static_assert(!std::is_const<Td>::value, "Destination span cannot be const");
    static_assert(std::is_same<Td, typename std::remove_const<Ts>::type>::value, "Span types must be the same after removing const");
...
{
    dw::core::internal::copyElements(dst, src);
}
