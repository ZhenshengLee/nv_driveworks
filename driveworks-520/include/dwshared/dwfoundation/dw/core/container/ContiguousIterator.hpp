...
#include <dwshared/dwfoundation/dw/core/meta/BoolConstant.hpp>
...
public:
    ...
    // coverity[autosar_cpp14_a1_1_1_violation] RFD Pending: TID-2194
    CUDA_BOTH_INLINE constexpr BasicContiguousIterator(BasicContiguousIterator<UValue, UContainer> const& other) noexcept // clang-tidy NOLINT(google-explicit-constructor) -- converting constructor is intentionally implicit
        : m_current(other.getCurrent())
    ...
    operator-(BasicContiguousIterator<UValue, UContainer> const& other) const noexcept
    {
        ...
        if ((m_lowerBound != other.getLowerBound()) || (m_upperBound != other.getUpperBound()))
        {
    ...
    CUDA_BOTH_INLINE pointer const& getLowerBound() const { return m_lowerBound; }
    CUDA_BOTH_INLINE pointer const& getUpperBound() const { return m_upperBound; }
    CUDA_BOTH_INLINE pointer const& getCurrent() const { return m_current; }
