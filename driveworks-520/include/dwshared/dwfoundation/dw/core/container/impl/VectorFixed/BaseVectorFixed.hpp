...
public:
    ...
    /// Conversion to const T
    /// Dummy template so this doesn't get defined for const iterators
    /// Avoid implicit conversion flagged by AUTOSAR rule A23-0-1
    template <bool IsConst_ = IsConst, typename = typename std::enable_if<!IsConst_>::type>
    Iterator<T, true> toConst() const
    {
...
public:
    ...
    {
#if DW_RUNTIME_CHECKS()
        checkIteratorInBounds(position, true);
#endif
    ...
    {
        return insert(position, std::move(static_cast<value_type>(val)));
    }

    /// Inserts element at position by moving.
    ///
    /// @return Iterator pointing to the inserted @a val
    iterator insert(size_type position, value_type&& val)
    {
        return insert(begin() + position, std::move(val));
    }

    /// Inserts copy of element at position.
    ///
    /// @return Iterator pointing to the inserted @a val
    iterator insert(size_type position, value_type const& val)
    {
        return insert(begin() + position, val);
    }

    /// Inserts @a count copies of @a val before @a position
    ///
    /// @return Iterator pointing to the first element inserted, or @a position if count == 0
    iterator insert(iterator position, size_type count, value_type const& val)
    {
        checkIteratorInBounds(position, true);
        checkNewSizeInRange(count);
        ...
        if (position == end())
        {
            push_back_range(count, val);
            return end() - count;
        }
        else
        {
            span<value_type> const newStorageSpan{m_storage.data(), size() + count};
            size_type const insertStartIdx{narrow<size_type>(position - begin())};
            // move all elements at and after position
            for (size_type i{size()}; i != insertStartIdx; i--)
            {
                iterator it{begin() + i - 1};
                new (&newStorageSpan[i + count - 1U]) value_type(std::move(*it)); // Move construct
            }

            // insert copies of elements
            for (size_type i{0U}; i < count; i++)
            {
                new (&newStorageSpan[i + insertStartIdx]) value_type(val); // Copy construct
            }
            m_storage.size() += count;

            return begin() + insertStartIdx;
        }
    }

    /// Inserts elements from range [first, last) before @a position
    ///
    /// @return Iterator pointing to the first element inserted, or @a position if first == last
    template <typename TIter = iterator>
    iterator insert(iterator position, TIter first, TIter last)
    {
        size_type const count{narrow<size_type>(last - first)};
        checkIteratorInBounds(position, true);
        checkNewSizeInRange(count);
        ...
        // insert copies of elements
        for (size_type i{insertStartIdx}; first != last; first++, i++)
        {
            new (&newStorageSpan[i]) value_type(*first); // Copy construct
        }
        m_storage.size() += count;
    ...
    /// Inserts elements from an initializer list before @a position
    ///
    /// @return Iterator pointing to the first element inserted, or @a position if @a ilist is empty
...
protected:
    ...
    /// Swap capacity hint with other @c BaseVectorFixed
    /// @note This method is needed in @c HeapVectorFixed::swap to swap the capacity hint of two vectors.
    /// Changing access control of m_capacityHint would make this a non standard-layout class, which violates
    /// [cert_exp60_cpp_violation].
    void swapCapacityHint(BaseVectorFixed& other)
    {
        std::swap(m_capacityHint, other.m_capacityHint);
    }

    /// Check if iterator is valid
    void checkIteratorInBounds(const_iterator const position, bool inclusive = false) const
    {
        if (position < begin() || (inclusive && position > end()) || (!inclusive && position >= end()))
        {
            throw OutOfBoundsException("VectorFixed: iterator is out of bounds");
        }
    }

    /// Check if this vector can hold additional @a count elements
    void checkNewSizeInRange(size_type const count) const
    {
        if (size() + count > m_storage.capacity())
        {
