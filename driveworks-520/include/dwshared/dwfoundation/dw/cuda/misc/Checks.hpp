...
/**
 * @brief Checks for cuda errors and reports it without throwing an exception in case an error is found. Executes no statements.
 *        Useful to check errors in destructor: <tt>DW_CHECK_CUDA_ERROR_NOTHROW(cudaStreamDestroy(...));</tt>
 */
#define DW_CHECK_CUDA_ERROR_NOTHROW(X)                          \
    {                                                           \
        static_cast<void>(X);                                   \
        cudaError_t const err{cudaGetLastError()};              \
        if (err != cudaSuccess)                                 \
        {                                                       \
            LOGSTREAM_ERROR() << __FILE__                       \
                              << "(" << __LINE__ << ") "        \
 ...
 */
static inline void checkError(const char* file, int32_t line)
{
