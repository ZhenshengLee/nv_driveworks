...
struct PitchPtr : public BasePitchPtr<T>
{
    ...
    */
    ...
    {
        return PitchPtr<T>(static_cast<Ti*>(p), strideInBytes / Base::SIZE_TI);
    }
};
...
public:
    ...
    DevicePitchPtrBase()
        : m_data({nullptr, 0})
    {
        static_assert(std::is_array<T>::value || std::is_same<T, void>::value || std::is_same<T, void const>::value, "DevicePitchPtr can only be used with array types");
    }
    ...
    */
    ...
    /**
     * @brief Get the number of items in the stride.
     * 
     * @return The number of items in the stride.
     */
    size_t getStrideItems() const
    {
        return m_data.getStrideItems();
    }
    ...
    *          with the stride item size automatically adjusted.
    */
    template <class TNew, std::enable_if_t<!std::is_same<TNew, void>::value, bool> = true>
    ...
    /**
    *  @brief Reinterpret cast to a DevicePitchPtr<void>.
    *
    *  @tparam TNew New data type to convert Pitch data into. Its data size might be
    *               different than that in Pitch from calling instance.
    *
    *  @return A DevicePitchPtr\<T\> instance containing original data in internal Pitch,
    *          with the stride item size is set to the original stride byte size.
    */
    template <class TNew, std::enable_if_t<std::is_same<TNew, void>::value, bool> = true>
    auto reinterpret() const -> DevicePitchPtr<TNew>
    {
