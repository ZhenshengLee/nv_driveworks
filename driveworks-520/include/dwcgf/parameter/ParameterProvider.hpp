...
public:
     ...
     * Convenience API throwing an exception instead of returning false.
     *
     * @see getOptionalWithIndex(dw::core::StringView const&, size_t const, T&)
     * @throws Exception if the parameter is not successfully retrieved
     */
    template <typename T>
    void getRequiredWithIndex(dw::core::StringView const& key, size_t const index, T& out) const
    {
        if (!getOptionalWithIndex(key, index, out))
        {
            throw ExceptionWithStatus(DW_NOT_AVAILABLE, "Required parameter not available: ", key);
        }
    }
    ...
    bool getOptional(dw::core::StringView const& key, T& out) const
    {
        ...
        {
            throw ExceptionWithStatus(e.statusCode(), "Failed to get parameter by name: ", key, " - ", e.message());
        }
    }
     ...
     * Convenience API throwing an exception instead of returning a failed status.
     *
     * @see getWithIndex(dw::core::StringView const&, size_t const, T&)
     * @throws Exception if the parameter retrieval has a failure
     */
    template <typename T>
    bool getOptionalWithIndex(dw::core::StringView const& key, size_t const index, T& out) const
    {
        try
        {
            return getWithIndex(key, index, out);
        }
        catch (ExceptionWithStatus& e)
        {
            throw ExceptionWithStatus(e.statusCode(), "Failed to get parameter by name and index: ", key, "[", index, "] - ", e.message());
        }
    }
     ...
     *
     * @see getImpl(ParameterProvider const* const, dw::core::StringView const&, const std::type_info&, const std::type_info&, T&)
     */
    template <
        typename T,
    ...
    {
        return getImpl(this, key, typeid(T), typeid(T), &out);
    }

    /**
     * Get a single non-enum parameter value from an array parameter by index.
     *
     * @see getImplWithIndex(ParameterProvider const* const, dw::core::StringView const&, const std::type_info&, const std::type_info&, size_t const, T&)
     */
    template <
        typename T,
        typename std::enable_if_t<
    ...
    {
        return getImplWithIndex(this, key, index, typeid(T), typeid(T), &out);
    }
    ...
    {
        return getImpl(this, key, typeid(std::vector<T>), typeid(std::vector<T>), &out);
    }

    /**
     * Get a fixed-size 1 dimension array of non-enum parameter values.
     *
     * @see getImpl(ParameterProvider const* const, std::vector<T>*)
     */
    template <
        typename T,
        typename std::enable_if_t<
            std::is_array<T>::value &&
            std::rank<T>::value == 1 &&
            !std::is_enum<std::remove_extent_t<T>>::value &&
            !std::is_same<std::remove_extent_t<T>, char8_t>::value>* = nullptr>
    // coverity[autosar_cpp14_a2_10_5_violation] FP: nvbugs/3907242
    bool get(dw::core::StringView const& key, T& out) const
    {
        ...
        dw::core::span<TElement> elementSpan{element, size};
        for (size_t i{0U}; i < size; ++i)
        {
            elementSpan[i] = value[i];
        }
        return true;
    }
    ...
    bool get(dw::core::StringView const& key, T& out) const
    {
        ...
        static_cast<void>(memcpy(element, value.data(), value.size()));
        dw::core::span<TElement> elementSpan{element, std::extent<T, 0>::value};
        // coverity[autosar_cpp14_a5_1_1_violation] RFD Accepted: TID-2056
        elementSpan[value.size()] = static_cast<char8_t>(0);
        return true;
    }
    ...
    void getRequiredWithIndex(dw::core::StringView const& key, size_t const index, T& out) const
    {
        if (!getOptionalWithIndex<S, T>(key, index, out))
        {
            throw ExceptionWithStatus(DW_NOT_AVAILABLE, "Required parameter not available: ", key);
        }
    }
    ...
    bool getOptional(dw::core::StringView const& key, T& out) const
    {
        ...
        catch (ExceptionWithStatus& e)
        {
            if (key.empty())
            {
                throw ExceptionWithStatus(e.statusCode(), "Failed to get unnamed parameter with mangled semantic type: ", typeid(S).name(), " - ", e.message());
            }
            else
            {
                throw ExceptionWithStatus(e.statusCode(), "Failed to get parameter with semantic by name: ", key, " - ", e.message());
            }
        }
    }
    ...
    bool getOptionalWithIndex(dw::core::StringView const& key, size_t const index, T& out) const
    {
        ...
        {
            // coverity[cert_err59_cpp_violation] RFD Pending: TID-2587
            return getWithIndex<S, T>(key, index, out);
        }
        catch (ExceptionWithStatus& e)
        {
            if (key.empty())
            {
                throw ExceptionWithStatus(e.statusCode(), "Failed to get unnamed parameter with mangled semantic type and index: ", typeid(S).name(), " ", index, " - ", e.message());
            }
            else
            {
                throw ExceptionWithStatus(e.statusCode(), "Failed to get parameter with semantic by name and index: ", key, " ", index, " - ", e.message());
            }
        }
    }
    ...
    bool get(dw::core::StringView const& key, T& out) const
    {
        ...
        return getImpl(this, key, typeid(S), typeid(T), &out);
    }
    ...
    bool get(dw::core::StringView const& key, dw::core::FixedString<N>& out) const
    {
        ...
        bool success{getImpl(this, key, getSemanticTypeInfo<S, N>(), typeid(dw::core::StringView), &str)};
        if (success)
        {
            // coverity[autosar_cpp14_a5_1_1_violation] FP: nvbugs/3364868
            if (N <= str.size())
            {
                throw ExceptionWithStatus(DW_BUFFER_FULL, "The FixedString parameter '", key, "' has a maximum capacity of N=", N, " but the value has a length of ", str.size() + 1U, "(including trailing \\0)");
            }
    ...
    bool get(dw::core::StringView const& key, T& out) const
    {
        ...
        dw::core::StringView str{};
        if (!getImpl(this, key, typeid(dw::core::StringView), typeid(dw::core::StringView), &str))
        {
            return false;
        }
        try
        ...
        {
            throw ExceptionWithStatus(e.statusCode(), "Failed to map enum name '", str, "' for parameter '", key, "' to numeric value: ", e.message());
        }
    }
     ...
     *
     * @see getImpl(ParameterProvider const* const, dw::core::StringView const&, size_t const, const std::type_info&, const std::type_info&, T&)
     */
    template <
        typename S, typename T,
        std::enable_if_t<!std::is_enum<T>::value>* = nullptr>
    ...
    bool getWithIndex(dw::core::StringView const& key, size_t const index, T& out) const
    {
        return getImplWithIndex(this, key, index, typeid(S), typeid(T), &out);
    }
    ...
    {
        dw::core::StringView str{};
        bool success{getImplWithIndex(this, key, index, getSemanticTypeInfo<S, N>(), typeid(dw::core::StringView), &str)};
        if (success)
        {
            // coverity[autosar_cpp14_a5_1_1_violation] FP: nvbugs/3364868
            if (N <= str.size())
            {
                throw ExceptionWithStatus(DW_BUFFER_FULL, "The FixedString parameter '", key, "' and index ", index, " has a maximum capacity of N=", N, " but the value has a length of ", str.size() + 1U, "(including trailing \\0)");
            }
            out.copyFrom(str.data(), str.size());
        }
        return success;
    ...
    bool getWithIndex(dw::core::StringView const& key, size_t const index, T& out) const
    {
        ...
        dw::core::StringView str{};
        if (!getImplWithIndex(this, key, index, typeid(dw::core::StringView), typeid(dw::core::StringView), &str))
        {
            return false;
        }
        try
        ...
        {
            throw ExceptionWithStatus(e.statusCode(), "Failed to map enum name '", str, "' for parameter '", key, "' and index ", index, " to numeric value: ", e.message());
        }
    }
...
private:
    // these two template specializations only exist since using a ternary expression instead
    // results in a coverity violation [identical_branches] if S is dw::core::StringView
    template <
        typename S, size_t N,
        std::enable_if_t<std::is_same<S, dw::core::FixedString<N>>::value>* = nullptr>
    static const std::type_info& getSemanticTypeInfo()
    {
        return typeid(dw::core::StringView);
    }
