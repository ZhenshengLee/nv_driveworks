...
{
    static_assert(std::is_trivially_copy_constructible<DefaultType>(), "DefaultType must be trivially copy constructible");
...
public:
    ...
    void addDefault(nlohmann::ordered_json& object) const override
    {
        ...
        object[DEFAULT_KEY] = detail::getDefaultValueForJson(m_defaultValue);
    }
...
public:
    ...
    void addDescriptors(const std::tuple<THead>& t)
    {
        ...
        auto params = std::get<0>(t).parameterDescriptors;
        // all parameters of this constructor argument
        addDescriptors(params);
    }
    ...
    void addDescriptors(const std::tuple<THead, TTail...>& t)
    {
        // coverity[autosar_cpp14_a8_5_2_violation] FP: nvbugs/3904083
        auto params = std::get<0>(t).parameterDescriptors;
        // all parameters of the first constructor argument
        addDescriptors(params);
        // remaining constructor arguments
        addDescriptors(tail_impl(std::make_index_sequence<sizeof...(TTail)>(), t));
    }

private:
    template <size_t... Ns, typename... Ts>
    auto tail_impl(std::index_sequence<Ns...>, const std::tuple<Ts...>& t) -> decltype(std::make_tuple(std::get<Ns + 1U>(t)...))
    {
...
static ParameterCollectionDescriptor createParameterCollectionDescriptor()
{
    ...
    d.addDescriptors(describeNodeParameters<NodeT>());
    return d;
}
