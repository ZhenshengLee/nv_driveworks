...
{
    static_assert(std::is_convertible<PassFunctionT, dw::core::Function<dwStatus()>>::value, "PassFunctionT must be callable without arguments and return dwStatus");
...
public:
    ...
    {
        constexpr const dw::core::StringView SUBMITTEE_SUFFIX{"_submittee"};
        constexpr const size_t MAX_NAME_LEN_WITHOUT_SUBMITTEE_SUFFIX{MAX_NAME_LEN - SUBMITTEE_SUFFIX.size()};
        if (runnableId.size() >= MAX_NAME_LEN_WITHOUT_SUBMITTEE_SUFFIX)
        {
        ...
        m_runnableIdSubmit += SUBMITTEE_SUFFIX;
    }
...
private:
    ...
    dw::core::Function<dwStatus()> m_functionInt;
    /// The runnable id.
