...
/// Add dependencies to pass descriptors.
template <
    typename NodeT, size_t Index,
    typename std::enable_if_t<!hasPassDependencies<NodeT, Index>(), void>* = nullptr>
// coverity[autosar_cpp14_a2_10_5_violation] FP: nvbugs/3907242
void addPassDescriptorDependencies(PassDescriptor& descriptor)
{
...
void addPassDescriptorDependencies(PassDescriptor& descriptor)
{
    for (size_t i{0U}; i < passDependencies<NodeT, Index>().size(); ++i)
    {
        descriptor.addDependency(passDependencies<NodeT, Index>()[i]);
    }
}
...
template <
    ...
    void addPassDescriptor(PassCollectionDescriptor& collection)
{
    PassDescriptor descriptor{passName<NodeT, Index>(), passProcessorType<NodeT, Index>(), hasPassDependencies<NodeT, Index>(), passDependencies<NodeT, Index>().size()};
    addPassDescriptorDependencies<NodeT, Index>(descriptor);
