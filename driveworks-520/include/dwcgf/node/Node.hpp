...
public:
    ...
    /**
     * @brief Collect error signals from node and module, combine and return the pointer to the error signal for this node.
     * If combination of module and node signals exceeds max amount, will return up to max amount, node first, and DW_BUFFER_FULL.
     * @param[out] errorSignal The error signal.
     * @return DW_SUCCESS, DW_NOT_AVAILABLE, DW_BUFFER_FULL
     */
    virtual dwStatus collectErrorSignals(dwGraphErrorSignal*& errorSignal) = 0;

    /**
     * @brief Get a copy of the error signals from the module that is a member of this node.
     * @param[out] errorSignal The error signal.
     * @return DW_SUCCESS, DW_NOT_AVAILABLE
     */
    virtual dwStatus getModuleErrorSignal(dwErrorSignal& errorSignal) = 0; // Will Be Deprecated upon completion of PRC2 refactor

    /**
     * @brief Get a copy of the error signals for the node.
     * @param[out] errorSignal The error signal.
     * @return DW_SUCCESS
     */
    virtual dwStatus getNodeErrorSignal(dwGraphErrorSignal& errorSignal) = 0; // Will Be Deprecated upon completion of PRC2 refactor

    /**
     * @brief Collect health signals from node and module, combine and return the pointer to the health signal for this node.
     * If combination of module and node signals exceeds max amount, will return up to max amount, node first, and DW_BUFFER_FULL.
     * @param[out] healthSignal The health signal.
     * @return DW_SUCCESS, DW_NOT_AVAILABLE, DW_BUFFER_FULL
     */
    virtual dwStatus collectHealthSignals(dwGraphHealthSignal*& healthSignal) = 0;

    /**
     * @brief Return a copy of health signals from module that is a member of node.
     * @param[out] healthSignal The health signal.
     * @return DW_SUCCESS, DW_NOT_AVAILABLE
     */
    virtual dwStatus getModuleHealthSignal(dwHealthSignal& healthSignal) = 0; // Will Be Deprecated upon completion of PRC2 refactor

    /**
     * @brief Return a copy of graph health signals for the node.
     * @param[out] healthSignal The health signal.
     * @return DW_SUCCESS
     */
    virtual dwStatus getNodeHealthSignal(dwGraphHealthSignal& healthSignal) = 0; // Will Be Deprecated upon completion of PRC2 refactor

    /**
     * @brief Clear the error signal for this node.
     * @return DW_SUCCESS
     */
    virtual dwStatus clearErrorSignal() = 0;

    /**
     * @brief Clear the health signal for this node.
     * @return DW_SUCCESS
     */
     ...
     * @return DW_SUCCESS
     **/
    virtual dwStatus updateCurrentErrorSignal(dwGraphErrorSignal& signal) = 0; // Will Be Deprecated upon completion of PRC2 refactor
     ...
     * @return DW_SUCCESS
     **/
    virtual dwStatus updateCurrentHealthSignal(dwGraphHealthSignal& signal) = 0; // Will Be Deprecated upon completion of PRC2 refactor

    /**
     * @brief A function the allows user to add an error to the error signal list.
     * @param[in] error error value to add.
     * @param[in] timestamp timestamp when error is reported.
     * return DW_SUCCESS, DW_BUFFER_FULL
    */
    virtual dwStatus addToErrorSignal(uint32_t error, dwTime_t timestamp = 0L) = 0;

    /**
     * @brief A function the allows user to add an error to the health signal list.
     * @param[in] error error value to add.
     * @param[in] timestamp timestamp when error is reported.
     * return DW_SUCCESS, DW_BUFFER_FULL
    */
     ...
     * @return DW_SUCCESS, DW_INVALID_ARGUMENT if node doesnt have input port w/port id
     */
     ...
     * @param[out] port The port associated with the port id.
     * @return DW_SUCCESS, DW_INVALID_ARGUMENT if node doesnt have output port w/port id
     */
...
public:
    ...
    /**
     * @brief Whether or not the node is enabled
     * @param isEnabled Output enabled
     * @return DW_SUCCESS
     */
