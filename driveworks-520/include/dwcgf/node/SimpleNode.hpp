...
public:
    ...
    void iterateManagedInputPorts(Func func)
    {
        iteratePorts(m_inputPorts, [&func, this](decltype(m_inputPorts)::TElement& elem) {
            if (nullptr == elem.second.get())
            {
                const char* nodeName{nullptr};
                static_cast<void>(this->getName(&nodeName));
...
protected:
    ...
    {
        // if the pass index is invalid the compilation will fail on passProcessorType() below
        // no need to check with isValidPass<NodeT>(PassIndex) at runtime

        // coverity[autosar_cpp14_a5_1_1_violation] RFD Accepted: TID-2056
        if (0U == m_passList.size() || m_passList.size() - 1U < PassIndex)
        {
            // coverity[autosar_cpp14_a5_1_1_violation] RFD Accepted: TID-2056
            m_passList.resize(PassIndex + 1U);
        }
        // coverity[autosar_cpp14_a5_1_1_violation] FP: nvbugs/3364868
        if (nullptr != m_passList[PassIndex])
        {
    ...
    {
        static_assert(DW_PROCESSOR_TYPE_GPU == passProcessorType<NodeT, PassIndex>(), "The processor type of a pass with a cuda stream must be GPU");
...
public:
    ...
    void initInputPort(Args&&... args)
    {
        ...
        std::shared_ptr<ManagedPortInput<DataType>> port{std::make_shared<ManagedPortInput<DataType>>(portName<NodeT, PortDirection::INPUT, PortIndex>(), std::forward<Args>(args)...)};
    ...
    void initInputArrayPort(size_t arrayIndex, Args&&... args)
    {
        ...
        std::shared_ptr<ManagedPortInput<DataType>> port{std::make_shared<ManagedPortInput<DataType>>(portName<NodeT, PortDirection::INPUT, PortIndex>(arrayIndex), std::forward<Args>(args)...)};
    ...
    void initOutputArrayPort(size_t arrayIndex, Args&&... args)
    {
        ...
        std::shared_ptr<ManagedPortOutput<DataType>> port{std::make_shared<ManagedPortOutput<DataType>>(portName<NodeT, PortDirection::OUTPUT, PortIndex>(arrayIndex), std::forward<Args>(args)...)};
    ...
    ManagedPortInput<decltype(portType<NodeT, PortDirection::INPUT, PortIndex>())>& getInputPort()
    {
        ...
        ManagedPortInputBase* portBase{m_inputPorts[PortIndex].get()};
        using DataType    = decltype(portType<NodeT, PortDirection::INPUT, PortIndex>());
        using PointerType = ManagedPortInput<DataType>*;
        // LCOV_EXCL_START the type is ensured by the template parameter and the exception in the defensive code can't be triggered
        // coverity[autosar_cpp14_m5_2_3_violation]
        PointerType port{dynamic_cast<PointerType>(portBase)};
        if (!port)
        {
    ...
    ManagedPortInput<decltype(portType<NodeT, PortDirection::INPUT, PortIndex>())>& getInputPort(size_t arrayIndex)
    {
        ...
        ManagedPortInputBase* portBase{m_inputPorts[PortIndex + arrayIndex].get()};
        using DataType    = decltype(portType<NodeT, PortDirection::INPUT, PortIndex>());
        using PointerType = ManagedPortInput<DataType>*;
        // LCOV_EXCL_START the type is ensured by the template parameter and the exception in the defensive code can't be triggered
        // coverity[autosar_cpp14_m5_2_3_violation]
    ...
    ManagedPortOutput<decltype(portType<NodeT, PortDirection::OUTPUT, PortIndex>())>& getOutputPort()
    {
        ...
        ManagedPortOutputBase* portBase{m_outputPorts[PortIndex].get()};
        using DataType    = decltype(portType<NodeT, PortDirection::OUTPUT, PortIndex>());
    ...
    ManagedPortOutput<decltype(portType<NodeT, PortDirection::OUTPUT, PortIndex>())>& getOutputPort(size_t arrayIndex)
    {
        ...
        ManagedPortOutputBase* portBase{m_outputPorts[PortIndex + arrayIndex].get()};
        using DataType    = decltype(portType<NodeT, PortDirection::OUTPUT, PortIndex>());
        using PointerType = ManagedPortOutput<DataType>*;
        // LCOV_EXCL_START the type is ensured by the template parameter and the exception in the defensive code can't be triggered
        // coverity[autosar_cpp14_m5_2_3_violation]
