...
public:
    ...
    ChannelConnector();
    explicit ChannelConnector(size_t numChannels);
    ...
    /// Adds a means to override the max sleep time for connection threads joining and in the waitUntilConnected call. By default without calling this we have a progressively longer timeout from ~1-512ms each iteration.
    void setWaitDelay(uint32_t const delayMs);
