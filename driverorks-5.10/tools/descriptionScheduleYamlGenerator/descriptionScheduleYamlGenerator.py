...
class STM_graph:
    ...
    def __init__(self,
        ...
        self.extern = extern
        self.frameworkPass = {}
        # The RoadCastService requries 2 framework_pass_rcs for cameraEpoch and imuEpoch.
        # The rcsPorts indicates depenencies of the framework_pass_rcs
        self.rcsPorts = {}
    ...
    def setFrameworkPass(self, frameworkPass):
        self.frameworkPass = frameworkPass

    def setRcsPorts(self, ports):
        self.rcsPorts = ports

    @typechecked
    def insertFrameworkPass(self):
        # Create framework pass specified by user to support application layer service injection
        # Now only support sync(at the beginning of a Epoch) and RCS
        if not self.frameworkPass:
            return
        ...
        for client in self.clients:
            if not client.name in self.frameworkPass:
                continue
            # create standalone framework_pass "rcs" for RoadCastService.
            # now only cameraEpoch and ImuEpoch are active for both rcs
            rcsEpochs = {}
            # create framework_pass "sync"
            ...
            # now only cameraEpoch is active for sync. No sync task needed for other epochs yet.
            syncEpochs = {}
            for epochName, passes in self.frameworkPass[client.name].items():
                if "rcs" in passes:
                    rcsEpochs[epochName] = passes["rcs"]
                if "sync" in passes:
                    syncEpochs[epochName] = passes["sync"]
            # verify if RoadCastService and frameworkPass are consistent
            # if one specified but the other is not, raise
            if bool(rcsEpochs) ^ bool(self.rcsPorts):
                raise ValueError("Framework pass and RoadCastService are inconsistent.")
            epochList = []
            for runnable in client.runnables:
            ...
            for epoch in epochList:
                ...
                if epoch.name in syncEpochs:
                    ...
                    if syncFrameworkPass.name != syncEpochs[epoch.name]:
                        raise ValueError("Framework pass name inconsistent. Specified: " + syncEpochs[epoch.name] + ", generated: " + syncFrameworkPass.name)
                ...
                if epoch.name in rcsEpochs:
                    ...
                    if rcsFrameworkPass.name != rcsEpochs[epoch.name]:
                        raise ValueError("Framework pass name inconsistent. Specified: " + rcsEpochs[epoch.name] + ", generated: " + rcsFrameworkPass.name)
    ...
    def yml(self) -> Dict:
        # insert framework pass before dump
        self.insertFrameworkPass()
        ymlTree = {}
            ...
            for outputPort in leafNode.getOutputPorts().values():
                for downstream in outputPort.peers:
                    ...
                    for finalInputPort in finalInputPorts:
                        ...
                        if (finalInputPort.parent.passes[0].epoch is not None
                            and outputPass.epoch is not None
                            and finalInputPort.parent.passes[0].epoch.name != outputPass.parent.passes[0].epoch.name):
                                listItem = (leafNode, finalInputPort.parent)
                                listName = leafNode.passes[0].epoch.name + finalInputPort.parent.passes[0].epoch.name
...
class SystemDescription:
    def __init__(self, systemFile):
        ...
        ROADCASTSERVICE = 'RoadCastService'
        FRAMEWORKPASS = 'frameworkPass'
        # RoadCastService
        self.rcs = None
        if ROADCASTSERVICE in systemJson:
            self.rcs = systemJson[ROADCASTSERVICE]
        # FrameworkPass
        self.frameworkPass = None
        if FRAMEWORKPASS in systemJson:
            self.frameworkPass = systemJson[FRAMEWORKPASS]
    ...
    def getRoadCastServiceJsonObj(self):
        return self.rcs
    def getFrameworkPassJsonObj(self):
        return self.frameworkPass
...
def main(argv=None):
    ...
    parser.add_argument('--add_inferred_edges', type = str, required = False, help = 'Add inferred edges to the graph to order runnables for determinism', default="False")
    ...
    if argvs.add_inferred_edges == "True":
        argsHelper.add_inferred_edges = True
    ...
    # roadCastService
    rcs = systemDescription.getRoadCastServiceJsonObj()
    # framworkPass
    frameworkPass = systemDescription.getFrameworkPassJsonObj()
    ...
    for schedule in systemDescription.getSchedules():
        ...
        # === frameworkPass ===
        if frameworkPass:
            stmGraph.setFrameworkPass(frameworkPass)
        # === roadCastService  ===
        if rcs:
            stmGraph.setRcsPorts(rcs["ports"])
        # === generate  ===
