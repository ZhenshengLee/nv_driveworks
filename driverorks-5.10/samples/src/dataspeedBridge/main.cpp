...
static void resetSignalValid(dwSignalValidity& validity)
{
    dwSignal_encodeSignalValidity(&validity,
...
static bool signalValid(dwSignalValidity const& validity)
{
...
static void encodeGenericSteeringReport(dwCANMessage& report, const dwVehicleIOActuationFeedback& actuationFeedback)
{
    ...
    if (signalValid(actuationFeedback.validityInfo.steeringWheelAngle))
    {
    ...
    if (signalValid(actuationFeedback.validityInfo.steeringWheelTorque))
    {
    ...
    if (signalValid(actuationFeedback.validityInfo.latCtrlDriverInterventionStatus))
    {
    ...
    if (signalValid(actuationFeedback.validityInfo.latCtrlStatus))
    {
    ...
    if (signalValid(actuationFeedback.validityInfo.latCtrlErrorStatus))
    {
...
static void encodeGenericBrakeReport(dwCANMessage& report, const dwVehicleIOSafetyCommand& safeCmd, const dwVehicleIONonSafetyState& nonSafeState, const dwVehicleIOActuationFeedback& actuationFeedback)
{
    ...
    if (signalValid(nonSafeState.validityInfo.driverBrakePedal))
    {
    ...
    if (signalValid(actuationFeedback.validityInfo.latCtrlModeStatus))
    {
    ...
    if (signalValid(actuationFeedback.validityInfo.longCtrlFaultStatus))
    {
    ...
    if (signalValid(safeCmd.validityInfo.longCtrlBrakePedalRequest))
    {
...
static void encodeGenericThrottleReport(dwCANMessage& report, const dwVehicleIOSafetyCommand& safeCmd, const dwVehicleIONonSafetyState& nonSafeState, const dwVehicleIOActuationFeedback& actuationFeedback)
{
    ...
    if (signalValid(nonSafeState.validityInfo.throttleValue))
    {
    ...
    if (signalValid(safeCmd.validityInfo.longCtrlThrottlePedalRequest))
    {
        status = dwCANInterpreter_encodef32(safeCmd.longCtrlThrottlePedalRequest, "Throttle_Report.ThrottleCMD", &report, gIntpGeneric);
        checkError(status, __LINE__);
    }

    if (signalValid(actuationFeedback.validityInfo.driverOverrideThrottle))
    {
...
dwVehicleIOGear convertToLegacyGear(const dwVehicleIONonSafetyState& nonSafeState, const dwVehicleIOActuationFeedback& actuationFeedback)
{
    ...
    assignFromMultipleSignals(signalValid(actuationFeedback.validityInfo.gearStatus), signalValid(nonSafeState.validityInfo.gearStatus),
                              gearStatus, actuationFeedback.gearStatus, nonSafeState.gearStatus);
...
static void encodeGenericGearReport(dwCANMessage& report, const dwVehicleIONonSafetyState& nonSafeState, const dwVehicleIOActuationFeedback& actuationFeedback)
{
    ...
    if (signalValid(actuationFeedback.validityInfo.gearStatus) || signalValid(nonSafeState.validityInfo.gearStatus))
    {
...
static void encodeGenericTurnSignalReport(dwCANMessage& report, const dwVehicleIONonSafetyState& nonSafeState)
{
    ...
    if (signalValid(nonSafeState.validityInfo.turnSignalStatus))
    {
...
static void encodeGenericMiscReport(dwCANMessage& report, const dwVehicleIONonSafetyState& nonSafeState)
{
    ...
    if (signalValid(nonSafeState.validityInfo.headlightState))
    {
    ...
    if (signalValid(nonSafeState.validityInfo.speedESC))
    {
...
static void processGeneric2DataspeedCommand(const dwCANMessage& genericMsg)
{
    ...
    for (uint32_t i = 0; i < nSig; i++)
    {
        ...
        if (strcmp(sigName, "Steering_Cmd.SteeringWheelAngleCmdValid") == 0)
        {
            ...
            if (iflag == 0)
                resetSignalValid(vioSafeCmd.validityInfo.latCtrlSteeringWheelAngleRequest);
            else
                setSignalValid(vioSafeCmd.validityInfo.latCtrlSteeringWheelAngleRequest);
        }
        ...
        if (strcmp(sigName, "Brake_Cmd.BrakePedalCmdValid") == 0)
        {
            ...
            dwCANInterpreter_geti32(&iflag, &unused, i, gIntpGeneric);
            if (iflag == 0)
                resetSignalValid(vioSafeCmd.validityInfo.longCtrlBrakePedalRequest);
            else
        ...
        if (strcmp(sigName, "Throttle_Cmd.ThrottlePedalCmdValid") == 0)
        {
            ...
            dwCANInterpreter_geti32(&iflag, &unused, i, gIntpGeneric);
            if (iflag == 0)
                resetSignalValid(vioSafeCmd.validityInfo.longCtrlThrottlePedalRequest);
            else
...
int main(int argc, const char** argv)
{
    ...
    while (gRun)
    {
        ...
        shouldSend |= signalValid(vioNonSafeState.validityInfo.frontSteeringTimestamp) && vioNonSafeState.frontSteeringTimestamp != oldTimestamp;
