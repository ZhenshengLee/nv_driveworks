...
public:
    ...
    bool initializeDescriptor()
    {
        ...
        CHECK_DW_ERROR(dwFeatureArray_create(&m_featuresDetectedCPU, m_featCountLevel[0], DW_MEMORY_TYPE_CPU, m_context));
        for (uint32_t i = 0; i < m_numPyramidLevels; i++)
        {

            CHECK_DW_ERROR(dwFeatureArray_create(&m_featuresDetectedGPU[i], m_featCountLevel[i],
                                                 DW_MEMORY_TYPE_CUDA, m_context));

            CHECK_DW_ERROR(dwFeatureDescriptorArray_create(&m_descriptorsGPU[i], DW_TYPE_UINT16, 16, m_featCountLevel[i], DW_MEMORY_TYPE_CUDA, m_context));
            CHECK_DW_ERROR(dwFeatureDescriptorArray_create(&m_descriptorsCPU[i], DW_TYPE_UINT16, 16, m_featCountLevel[i], DW_MEMORY_TYPE_CPU, m_context));
        }
