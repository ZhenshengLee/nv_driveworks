...
private:
    ...
    void initChannels()
    {
        ...
        if (m_hasProducer)
        {
            std::string channelParam = "role=producer,type=" + m_type + ",ip=" + m_ipAddr + ",id=" + m_portId + ",num-clients=" + std::string(getArgument("downstreams")) + ",producer-fifo=1,fifo-size=4,timeout=1000";
            if (m_type == "NVSCI")
            {
