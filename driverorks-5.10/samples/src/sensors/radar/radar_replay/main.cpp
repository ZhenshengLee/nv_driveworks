    ...
    void computeSpin()
    {
        ...
        while (!scanComplete && status == DW_SUCCESS)
        {
            ...
            if (status == DW_SUCCESS)
            {
                ...
                m_message1 = "Host timestamp " + toStr(nextPacket->hostTimestamp / 1000000ULL) + "(sec) / " + toStr(nextPacket->hostTimestamp) + "(millisecs)";
                m_message2 = "Sensor timestamp " + toStr(nextPacket->sensorTimestamp / 1000000ULL) + "(sec) / " + toStr(nextPacket->sensorTimestamp) + "(millisecs)";
