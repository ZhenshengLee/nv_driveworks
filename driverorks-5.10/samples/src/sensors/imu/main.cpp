...
void printAll(const dwIMUFrame& frame)
{
    ...
    else
    {
        ...
        // orientation
        if (isSignalValid(frame.validityInfo.orientation[0]) ||
            isSignalValid(frame.validityInfo.orientation[1]) ||
            isSignalValid(frame.validityInfo.orientation[2]))
        {
        ...
        if (isSignalValid(frame.validityInfo.orientationQuaternion))
        {
        ...
        // gyroscope
        if (isSignalValid(frame.validityInfo.turnrate[0]) ||
            isSignalValid(frame.validityInfo.turnrate[1]) ||
            isSignalValid(frame.validityInfo.turnrate[2]))
        {
            std::cout << "Gyro(";
        ...
        // heading (i.e. compass)
        if (isSignalValid(frame.validityInfo.heading))
        {
        ...
        // Acceleration
        if (isSignalValid(frame.validityInfo.acceleration[0]) ||
            isSignalValid(frame.validityInfo.acceleration[1]) ||
            isSignalValid(frame.validityInfo.acceleration[2]))
        {
        ...
        // magnetometer
        if (isSignalValid(frame.validityInfo.magnetometer[0]) ||
            isSignalValid(frame.validityInfo.magnetometer[1]) ||
            isSignalValid(frame.validityInfo.magnetometer[2]))
        {
        ...
        // Alignment status
        if (isSignalValid(frame.validityInfo.alignmentStatus))
        {
            containIMUInfo = true;
...
int main(int argc, const char** argv)
{
    ...
    ProgramArguments arguments(
        {ProgramArguments::Option_t("driver", "imu.virtual"),
         ProgramArguments::Option_t("params", (std::string("file=") + dw_samples::SamplesDataPath::get() + "/samples/sensors/imu/imu.txt").c_str()),
