...
int main(int argc, const char** argv)
{
    ...
    bool CVS_Output = false;
    if (arguments.has("csv") && arguments.get("csv") == "true")
    {
        std::cout << "Creat interpretted CAN signals in CSV format" << std::endl;
        CVS_Output = true;
    }
            ...
            if (canParser)
            {
                ...
                if (CVS_Output)
                {
                    if (status == DW_SUCCESS && num > 0)
                    {
                        ...
                        for (uint32_t i = 0; i < num; ++i)
                        {
                            if (dwCANInterpreter_getSignalName(&name, i, canParser) == DW_SUCCESS && CVS_Output)
                            {
