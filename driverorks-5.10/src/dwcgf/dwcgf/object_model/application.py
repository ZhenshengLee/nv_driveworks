...
from dwcgf.utils import find_duplicate_elements
...
class Application(Graphlet):
    ...
    def __init__(
        ...
        *,
        ...
        for process_name, process_def in processes.items():
            ...
            if process_name not in self._processes.keys():
                self._processes[process_name] = Process.from_descriptor(
                    desc=process_def,
    ...
    def framework_pass(self) -> Optional[Dict[str, Dict]]:
        """Return frameworkPass."""
        return self._framework_pass

    @framework_pass.setter
    def framework_pass(self, new_framework_pass: Optional[Dict[str, Dict]]) -> None:
        """Replace the framework_pass dict."""
        self._set_framework_pass(new_framework_pass)

    @object_model_channel.pair_self
    def _set_framework_pass(
        self, new_framework_pass: Optional[Dict[str, Dict]]
    ) -> UndoContext:
        """Set a new framework_pass dict."""

        ret = UndoContext(self, self._framework_pass)
        self._framework_pass = new_framework_pass
        return ret
    ...
    def construct(self) -> None:
        ...
        for schedule in self._stm_schedules.values():
            if schedule.schedule.wcet is not None:
                if wcet_path is None:
                ...
                elif str(wcet_path) != str(
                    self.descriptor_path.parent / schedule.schedule.wcet
                    if self.descriptor_path is not None
                    else schedule.schedule.wcet
                ):
                    raise NotImplementedError(f"Multiple wcet files not supported.")

        # distribute wcet
        # currently wcet_path is stored in stm_schedule
        if wcet_path is not None:
            self.__distribute_wcet(wcet_path)
        else:
            raise ValueError(f"Can not decide wcet file path.")
    ...
    def __distribute_wcet(self, wcet_path: Path) -> None:
        ...
        self._wcet_data = deepcopy(wcet_data)
        pass_not_listed: list = []

        def distribute_wcet_impl(root: Component) -> None:
            if isinstance(root, Node):
                passes = root.passes
                ...
                for idx, pass_ in enumerate(passes):
                    ...
                    if wcet_key in self._wcet_data.keys():
                        pass_.wcet = self._wcet_data.pop(wcet_key)
...
as {process_name} does not exist in the application."
                ...
                for component in root.subcomponents.values():
                    distribute_wcet_impl(component)
            else:
                raise TypeError(f"Unexpected subcomponent subcomponent type: {root}")

        distribute_wcet_impl(self)

        print(
            f"\nAfter wcet distribution: \
wcet loaded/unused: {len(wcet_data) - len(self._wcet_data)}/{len(self._wcet_data)}, \
{len(pass_not_listed)} passes not listed in wcet file."
        )

    def collect_wcet(self) -> Dict[str, int]:
        ...
        def collect_wcet_impl(root: Component) -> None:
            if isinstance(root, Node):
                passes = root.passes
                parent_name = root.id.replace(".", "_")
                for idx, pass_ in enumerate(passes):
                    ...
                    if pass_.wcet is not None:
                        wcet_data[wcet_key] = pass_.wcet
                    else:
                        pass_with_empty_wcet.append(wcet_key)
            elif isinstance(root, (Graphlet, Application)):
                for component in root.subcomponents.values():
                    collect_wcet_impl(component)
            else:
                raise TypeError(f"Unexpected subcomponent subcomponent type: {root}")

        collect_wcet_impl(self)

        print(
            f"\nAfter wcet collection: \
wcet collected/unused: {len(wcet_data)}/{len(self._wcet_data)}, \
{len(pass_with_empty_wcet)} passes have no valid wcet."
        )

        return (
            wcet_data
            if self._wcet_data is None
            else json_merge_patch.merge(wcet_data, self._wcet_data)
        )
    ...
    def insert_process(self, process_def: ProcessDefinition) -> None:
    ...
    @object_model_channel.pair
    def insert_multi_machine_required_sensors(
        self, new_multi_machine_required_sensors: Dict[str, str]
    ) -> UndoContext:
        """insert to multi machine required sensors(undo-able)."""

        assert isinstance(new_multi_machine_required_sensors, dict)
        assert all(
            isinstance(k, str) for k in new_multi_machine_required_sensors.keys()
        )
        assert all(
            isinstance(v, str) for v in new_multi_machine_required_sensors.values()
        )

        if not new_multi_machine_required_sensors:
            return UndoContext.NO_CHANGE

        for name in new_multi_machine_required_sensors.keys():
            if name in self.multi_machine_required_sensors:
                raise ValueError(
                    f"Find '{name}' already exists in multi machine required sensors."
                )

        self.multi_machine_required_sensors.update(
            {
                name: Path(path)
                for name, path in new_multi_machine_required_sensors.items()
            }
        )
        self._self_is_modified = True

        return UndoContext(self, list(new_multi_machine_required_sensors.keys()))

    @insert_multi_machine_required_sensors.pair
    def remove_multi_machine_required_sensors(
        self, multi_machine_required_sensors: List[str]
    ) -> UndoContext:
        """remove required sensors from multi machine required sensors(undo-able)."""
        assert isinstance(multi_machine_required_sensors, list)
        assert all(isinstance(e, str) for e in multi_machine_required_sensors)

        if len(multi_machine_required_sensors) == 0:
            return UndoContext.NO_CHANGE

        dup_elements = find_duplicate_elements(multi_machine_required_sensors)
        if dup_elements:
            raise ValueError(
                f"Find duplicated elements in multi machine required sensors: '{dup_elements}'"
            )

        for name in multi_machine_required_sensors:
            if name not in self.multi_machine_required_sensors:
                raise ValueError(
                    f"Cannot find '{name}' in multi machine required sensors."
                )

        undo_context = {}
        for name in multi_machine_required_sensors:
            path: Path = self.multi_machine_required_sensors.pop(name)
            undo_context[name] = str(path)
    ...
    @object_model_channel.pair_self
    def set_rcs_enable(self, enable: Optional[bool]) -> UndoContext:
        """Set rcs enable or disable.

        @param enable if set True, RCS will be enabled,
                      if set False or None will disable RCS.
        """
        ret = UndoContext(self, self._rcs.get("enabled", None))  # type: ignore
        if enable is not None:
            self._rcs["enabled"] = enable  # type: ignore
        else:
            if "enabled" in self._rcs:  # type: ignore
                self._rcs.pop("enabled")  # type: ignore

        return ret

    @object_model_channel.pair
    def insert_rcs_port(self, port_name: str, port_value: dict) -> UndoContext:
        """insert RoadCastService port (undo-able)."""
        if not self._rcs:
            raise ValueError(
                f"Can't insert when default RoadCastService is not defined."
            )
        if port_name in self._rcs["ports"]:
            raise ValueError(f"Port name '{port_name}' already exist.")

        self._rcs["ports"][port_name] = port_value

        self._self_is_modified = True

        return UndoContext(self, port_name)

    @insert_rcs_port.pair
    def remove_rcs_port(self, port_name: str) -> UndoContext:
        """remove RoadCastService port (undo-able)."""
        if not self._rcs:
            raise ValueError(
                f"Can't remove when default RoadCastService is not defined."
            )
        if port_name not in self._rcs["ports"]:
            raise ValueError(f"Port name '{port_name}' does not exist.")

        dst = self._rcs["ports"]
        removal = dst.pop(port_name)

        self._self_is_modified = True

        return UndoContext(self, port_name, removal)

    @object_model_channel.pair_self
    def update_rcs_serialize_mask(
        self, channel_id: str, serialize_mask: dict
    ) -> UndoContext:
        """update RoadCastService serialize mask for the specified channel."""
        if not self._rcs:
            raise ValueError(
                f"Can't update when default RoadCastService is not defined."
            )
        if channel_id not in self._rcs["params"]["channelParams"]:
            raise ValueError(f"Channel id '{channel_id}' does not exists.")

        original_mask = {}
        ref = self._rcs["params"]["channelParams"][channel_id]["serialMask"]
        ...
        return UndoContext(self, channel_id, original_mask)
