...
class SeparateEpochs(Action):
    ...
    extra_attributes = (
        ...
        # Having multiple CUDA_MUTEX_LOCKs causes in the yaml generation to
        # incorrectly associate the locks with the stream. Based on the current
        # schedule, these resources can be associated with one of the epochs.
        # Let the user decide the association.
        # TODO(ajayawardane): remove this option once the multiple mutex issue is
        # resolved.
        ActionAttribute(
            name="gpuResourceAssociation",
            is_required=True,
            attr_type=list,
            default=[],
            description="List of hyperepochs and epochs that needs gpu resources. \
                         This is captured by a list where the 0th index is a list \
                         of epoch names and the 1st index is a list of hyperepochs \
                         corresponding to the epoch list that require gpu resources",
        ),
    )

    def transform_impl(self, target: Application) -> None:
        """action implementation."""
        gpuResourceAssociation = self.get_attribute("gpuResourceAssociation")
        for schedule_name in cast(list, self.get_attribute("scheduleNames")):
            if schedule_name not in target._stm_schedules:
                ...
                for epoch in epochsList:
                    ...
                    for key, resource in hyperepoch.resources.items():
                        ...
                        if "CPU" in key:
                            ...
                            if (
                                epoch.name in gpuResourceAssociation[0]
                                and hyperepoch.name in gpuResourceAssociation[1]
                            ):
                                resource_name = key
                            else:
                                continue
                    ...
                    hyperepoch_name = hyperepoch.name + epoch.name + "Hyperepoch"
