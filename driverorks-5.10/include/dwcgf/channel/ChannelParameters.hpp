...
using ChannelType = enum ChannelType : uint8_t {
    DW_CHANNEL_TYPE_SHMEM_LOCAL  = 0, ///< local shared memory
    DW_CHANNEL_TYPE_SHMEM_REMOTE = 1, ///< remote shared memory
    DW_CHANNEL_TYPE_EGLSTREAM    = 2, ///< EGL stream
    DW_CHANNEL_TYPE_SOCKET       = 3, ///< socket
    DW_CHANNEL_TYPE_DDS          = 4, ///< Data Distribution Service (DDS)
    DW_CHANNEL_TYPE_NVSCI        = 5, ///< NvSciStream
};
...
uint16_t ParseChannelParameter(const ChannelParamStr& value)
{
    ...
    {
        throw ExceptionWithStatus(DW_INVALID_ARGUMENT, "ChannelSocketParams: port is larger than uint16_t allows!");
    }
...
public:
    ...
    uint32_t getLocalClientCount() const { return m_localClientCount; }
