...
#include <dw/core/language/UnformattedOutputStreamTraits.hpp>
#include <dw/core/base/TypeAliases.hpp>
#include <dw/core/language/ToAddress.hpp>
...
public:
    ...
    constexpr BasicStringView(const_pointer const s) // clang-tidy NOLINT(cppcoreguidelines-pro-type-member-init) -- forwarding constructor
        : BasicStringView(s, traits_type::length(s))
    {
    }
    ...
    auto copy(value_type* const dest, size_type const count, size_type const position = zero) const -> size_type
    {
        ...
        // TODO(dwplc): FP -- Coverity thinks count is a size_type* instead of the size_type lvalue it actually is. This
        //              code would not compile if that was the case, as std::min requires identical typed parameters.
        // coverity[cert_exp37_c_violation]
