...
void SSM::initialize()
{
    SSM_LOG("Initializing SM5");
    isStateMachineActive = true;
    initStateMachine();
    ENTER();
}

bool SSM::executeLockedCommand(SystemStateManager::SM5::LockSteppedCommands command)
{
    return executeLockSteppedCommand((int)command);
}

bool SSM::registerLockedCommand(SystemStateManager::SM5::LockSteppedCommands command, SSMFunctionHandler ptr)
{
...
void SSM::documentStateChangeExternally()
{
...
{
    return changeStateByString(SSM_enumToStr(e));
}

int SSM::sendDataByID(StateMachines e, void *data, int datalen, std::string dataStructID)
{
...
{
    return SSM_strToEnum(mySMPtr->getCurrentState());
}


bool SSM::getUserSpecificData(UserDataPkt &pkt)
{
