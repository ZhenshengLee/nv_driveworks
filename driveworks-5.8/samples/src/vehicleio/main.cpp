...
public:
    ...
    void resetCmd()
    {
        ...
        m_safeCmd.longCtrGearCommand               = DW_VIO_LONG_CTR_GEAR_COMMAND_IDLE;
        ...
        setSignalValidIf(false, m_safeCmd.validityInfo.longCtrGearCommand);
    }
    ...
    void handleJoystick()
    {
        ...
        if (buttons[JS_Enable])
        {
            ...
            setSignalValidIf(true, m_safeCmd.validityInfo.longCtrGearCommand);
        }
    ...
    void onProcess() override
    {
        ...
        if (now >= m_timeToSend)
        {
            ...
            m_timeToSend = now + 20000;

            logWarn("VehicleIO sample: Status\n");
        ...
        status = dwSensorManager_acquireNextEvent(&m_ev, 1000000, m_sm);
        if (status == DW_END_OF_STREAM)
        {
            logWarn("VehicleIO sample: reached EOF\n");
            reset();
            return;
        }
        else if (status != DW_SUCCESS)
        {
            logWarn("VehicleIO sample: sensor error %s\n", dwGetStatusName(status));
            return;
        }
    ...
    void onRender() override
    {
        ...
        dwRenderEngine_renderText2D(logBrakeThrottle.str().c_str(), {10.0f, 180.0f}, m_renderEngine);
        ...
        dwRenderEngine_renderText2D(logSteering.str().c_str(), {10.0f, 520.0f}, m_renderEngine);
        ...
        dwRenderEngine_renderText2D(logMisc.str().c_str(), {500.0f, 180.0f}, m_renderEngine);
        ...
        logSpeed << " wheelSpeedFrontLeft:  " << m_nonSafeState.wheelSpeed[DW_VEHICLE_WHEEL_FRONT_LEFT] << "\n";
        logSpeed << " wheelSpeedFrontRight:  " << m_nonSafeState.wheelSpeed[DW_VEHICLE_WHEEL_FRONT_RIGHT] << "\n";
        logSpeed << " wheelSpeedRearLeft:  " << m_nonSafeState.wheelSpeed[DW_VEHICLE_WHEEL_REAR_LEFT] << "\n";
        logSpeed << " wheelSpeedRearRight:  " << m_nonSafeState.wheelSpeed[DW_VEHICLE_WHEEL_REAR_RIGHT] << "\n";

        dwRenderEngine_setColor(DW_RENDER_ENGINE_COLOR_LIGHTBLUE, m_renderEngine);
        dwRenderEngine_renderText2D(logSpeed.str().c_str(), {500.0f, 520.0f}, m_renderEngine);
    ...
    void onKeyDown(int key, int /*scancode*/, int /*mods*/) override
    {
        ...
        case GLFW_KEY_UP:
            ...
            if (signalValid(m_safeCmd.validityInfo.longCtrlBrakePedalRequest))
            {
        ...
        case GLFW_KEY_DOWN:
            if (signalValid(m_safeCmd.validityInfo.longCtrlThrottlePedalRequest))
            {
        ...
        case GLFW_KEY_P:
            setSignalValidIf(true, m_safeCmd.validityInfo.longCtrGearCommand);
            m_safeCmd.longCtrGearCommand = DW_VIO_LONG_CTR_GEAR_COMMAND_P;
            break;
        ...
        default: logWarn("VehicleIO sample: No command for this key\n");
        }
    }
};
