...
public:
    ...
    bool onInitialize() override
    {
        ...
        // -----------------------------
        {
            ...
            for (size_t k = 0; k < CAMERA_COUNT; k++)
            {
                ...
                // Feature list
                CHECK_DW_ERROR(dwFeatureHistoryArray_create(&camera.featureHistoryCPU, maxFeatureCount,
                                                            FEATURE_HISTORY_SIZE, DW_MEMORY_TYPE_CPU, m_context));
                CHECK_DW_ERROR(dwFeatureHistoryArray_create(&camera.featureHistoryGPU, maxFeatureCount,
                                                            FEATURE_HISTORY_SIZE, DW_MEMORY_TYPE_CUDA, m_context));
                CHECK_DW_ERROR(dwFeatureArray_create(&camera.featureDetectedGPU, maxFeatureCount,
    ...
    dwFeatureArray trackFrame(size_t cameraIdx, const dwTransformation3f& predictedRig2World)
    {
        ...
        //Build pyramid
        {
            ...
            CHECK_DW_ERROR(dwImageCUDA_getPlaneAsImage(&planeY, img, 0));

            CHECK_DW_ERROR(dwImageFilter_computePyramid(&camera.pyramidCurrent, &planeY, 0, m_context));
        }
    ...
    void processCan(const dwSensorEvent& event)
    {
        ...
        CHECK_DW_ERROR(dwVehicleIO_getVehicleState(&vioState, vehicleIO));
        CHECK_DW_ERROR(dwEgomotion_addVehicleState(&vioState, egomotion));
    }
