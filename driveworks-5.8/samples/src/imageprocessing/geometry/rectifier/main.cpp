...
{
    dwImageHandle_t imageGL = m_camera->getFrameRgbaGL();
    ...
    dwStatus status = dwImageStreamerGL_consumerReceive(&frameGLOut, 500000, m_streamerCUDA2GL);
...
bool RectifierApp::initCameras()
{
    ...
    dwStatus status;
    status = dwSensor_start(m_cameraSensor);
    ...
    dwFThetaCameraConfigNew config{};
    dwRig_getFThetaCameraConfigNew(&config, 0, rigConf);
