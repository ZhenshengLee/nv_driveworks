...
#include <dw/imageprocessing/filtering/PyramidPVA.h>
#include "cupva_types_wrapper.h"
...
private:
    ...
    dwImageHandle_t m_processImageHalfRes{DW_NULL_HANDLE};
...
public:
    ...
    bool onInitialize() override
    {
...
#ifdef DW_SDK_BUILD_PVA
        cupvaStreamCreate(&m_cupvaStream, pvaEngineType::PVA_PVA0, pvaAffinityType::PVA_VPU_ANY);
#endif
        ...
        // ------------------------------------------------------------------
        {
            ...
            if (m_useHalfRes)
            {
                ...
                m_cameraImageProps.width /= 2;
                m_cameraImageProps.height /= 2;
                ...
                CHECK_DW_ERROR(dwImage_create(&m_processImageHalfRes, m_cameraImageProps, m_context));
            }
    ...
    void onReset() override
    {
        ...
        CHECK_DW_ERROR(dwFeatureHistoryArray_reset(&featureHistoryGPU, cudaStream_t(0)));
        CHECK_DW_ERROR(dwFeatureArray_reset(&featuresDetected, cudaStream_t(0)));
    ...
    void onRelease() override
    {
        ...
        if (m_processImageHalfRes)
        {
            CHECK_DW_ERROR(dwImage_destroy(m_processImageHalfRes));
        }
    ...
    void onProcess() override
    {
        ...
        if (m_useHalfRes)
        {
            CHECK_DW_ERROR(dwImageTransformation_copyFullImage(m_processImageHalfRes, m_imageYUV, m_imageTransformer));
            trackImage = m_processImageHalfRes;
        }
        else
        {
    ...
    void onRender() override
    {
        if (m_imageRGBA)
        {
            ...
            printf("Number of feature %d \n", *(featureHistoryCPU.featureCount));
            ...
            if (drawHistory)
            {
                ...
                for (uint32_t i = 0; i < *(featureHistoryCPU.featureCount); i++)
                {
                    ...
                    for (int32_t histIdx = static_cast<int32_t>(drawAge) - 2; histIdx >= 0; histIdx--)
                    {
                        ...
                        drawCount++;
                        if (m_curFrameIdx == 2)
                            printf("preFeature.locations[i].x ->%f, preFeature.locations[i].y -> %f i -> %d\n", preFeature.locations[i].x, preFeature.locations[i].y, i);

                        preFeature = curFeature;
                    }
                }

                sprintf(stime, "Frame: %u, number of feature: %u", m_curFrameIdx, validFeatureCount);
                CHECK_DW_ERROR(dwRenderer_setColor(DW_RENDERER_COLOR_WHITE, m_renderer));
