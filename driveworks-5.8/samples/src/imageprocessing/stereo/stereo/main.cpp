...
public:
    ...
    void onProcess()
    {
        ...
        // build pyramids
        dwImageCUDA planeImage[DW_STEREO_SIDE_BOTH] = {};
        {
            ProfileCUDASection s(getProfilerCUDA(), "Pyramid build");
            for (uint32_t side = 0; side < DW_STEREO_SIDE_BOTH; ++side)
            {
                CHECK_DW_ERROR(dwImageCUDA_getPlaneAsImage(&planeImage[side], inputImagesCUDA[side], 0));
                CHECK_DW_ERROR(dwImageFilter_computePyramid(&m_pyramids[side], &planeImage[side], 0, m_context));
            }
        }
