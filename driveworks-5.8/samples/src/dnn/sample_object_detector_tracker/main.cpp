...
public:
    ...
    bool onInitialize() override
    {
        ...
        //------------------------------------------------------------------------------
        {
            ...
            CHECK_DW_ERROR(dwFeatureHistoryArray_create(&m_featureHistoryCPU, m_maxFeatureCount,
                                                        m_historyCapacity, DW_MEMORY_TYPE_CPU, m_sdk));
            CHECK_DW_ERROR(dwFeatureHistoryArray_create(&m_featureHistoryGPU, m_maxFeatureCount,
                                                        m_historyCapacity, DW_MEMORY_TYPE_CUDA, m_sdk));
            CHECK_DW_ERROR(dwFeatureArray_create(&m_featureDetectedGPU, m_maxFeatureCount,
                                                 DW_MEMORY_TYPE_CUDA, m_sdk));
...
private:
    ...
    uint32_t trackFeatures(const dwImageCUDA* image)
    {
        ...
        // build pyramid
        CHECK_DW_ERROR(dwImageFilter_computePyramid(&m_pyramidCurrent, &planeY, m_cudaStream, m_sdk));
