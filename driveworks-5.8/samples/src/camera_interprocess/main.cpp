...
public:
    ...
    dwImageHandle_t m_appendImage                             = DW_NULL_HANDLE;
    dwImageProperties m_appendImageProperties{};
...
void DwCGFCameraInterprocessApp::onInitFrameCapture()
{
    ...
    frameParams.width             = 3848;
    frameParams.height            = 2168;
...
dw::framework::OnSetSyncAttrs DwCGFCameraInterprocessApp::getProducerSyncAttrs(size_t camIdx, bool isSignaler)
{
    if (p2cEnabled() && isSignaler)
    {
        return [this, camIdx](NvSciSyncAttrList attrList) {
            CHECK_DW_ERROR(dwSensorCamera_fillSyncAttributes(attrList, m_outputType, m_camera[camIdx]));
        };
    }
    return {};
}
...
void DwCGFCameraInterprocessApp::setSyncObj(int i)
{
    ...
    CHECK_DW_ERROR(dwSensorCamera_setSyncObject(span[0], m_outputType, m_camera[i]));
    return;
}
...
bool DwCGFCameraInterprocessApp::onInitialize()
{
    ...
    if (!m_isProducer)
    {
        ...
        props.height            = static_cast<int32_t>(2160);
        props.width             = static_cast<int32_t>(3840);
        props.format            = DW_IMAGE_FORMAT_YUV420_UINT8_SEMIPLANAR;
        props.type              = DW_IMAGE_NVMEDIA;
        m_appendImageProperties = props;

        dwImageTransformationParameters transParams{false};
        CHECK_DW_ERROR(dwImageTransformation_initialize(&m_imageTransformationEngine, transParams, context));

        CHECK_DW_ERROR(dwImageTransformation_appendAllocationAttributes(&m_appendImageProperties,
                                                                        m_imageTransformationEngine));
        CHECK_DW_ERROR(dwImage_create(&m_appendImage, m_appendImageProperties, context));
        ...
        m_inputImagePort = std::make_unique<dw::framework::PortInput<PacketT>>(getConsumerSyncAttrs(0, c2pEnabled()), getConsumerSyncAttrs(0, p2cEnabled()));
...
void DwCGFCameraInterprocessApp::onRender()
{
    ...
    if (!m_isProducer)
    {
        ...
        }
        {
            ...
            CHECK_DW_ERROR(dwImage_copyConvert(m_appendImage, *inputPtr, context));

            CHECK_DW_ERROR(dwFrameCapture_appendFrame(m_appendImage, m_frameCapture));
...
void DwCGFCameraInterprocessApp::onRelease()
{
    ...
    else
    {
        ...
        if (m_appendImage)
        {
            CHECK_DW_ERROR(dwImage_destroy(m_appendImage));
        }

        if (m_imageTransformationEngine)
        {
            CHECK_DW_ERROR(dwImageTransformation_release(m_imageTransformationEngine));
        }
