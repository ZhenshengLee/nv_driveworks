...
private:
    ...
    void initializeFeatureMatcher()
    {
        ...
        CHECK_DW_ERROR(dwFeatureHistoryArray_create(&m_matchesHistoryGPU, m_maxMatchesCount, HISTORY_CAPACITY, DW_MEMORY_TYPE_CUDA, m_context));
        CHECK_DW_ERROR(dwFeatureArray_create(&m_matchesDetectedGPU, m_maxMatchesCount, DW_MEMORY_TYPE_CUDA, m_context));
    ...
    void processImage(dwImageHandle_t imageYuv)
    {
        ...
        dwImageCUDA imageY;
        CHECK_DW_ERROR(dwImageCUDA_getPlaneAsImage(&imageY, imageYuvCUDA, 0));

        std::swap(m_pyramidCurrent, m_pyramidPrevious);
        CHECK_DW_ERROR(dwImageFilter_computePyramid(&m_pyramidCurrent, &imageY, cudaStream_t(0), m_context));
