...
public:
    ...
    void onRender() override
    {
        ...
        float32_t currentHeight = getWindowHeight() - 11 * LINESPACING;
...
private:
    ...
    void initializeFeatureTracker()
    {
        ...
        CHECK_DW_ERROR(dwFeatureHistoryArray_create(&m_featureHistoryGPU, m_maxFeatureCount,
                                                    HISTORY_CAPACITY, DW_MEMORY_TYPE_CUDA, m_context));
        CHECK_DW_ERROR(dwFeatureArray_create(&m_featureDetectedGPU, m_maxFeatureCount,
    ...
    {
        // Get Y plane from CUDA image
        dwImageCUDA planeY;
        CHECK_DW_ERROR(dwImageCUDA_getPlaneAsImage(&planeY, &imageCuda, 0));

        // Update pyramid
        std::swap(m_pyramidCurrent, m_pyramidPrevious);
        CHECK_DW_ERROR(dwImageFilter_computePyramid(&m_pyramidCurrent, &planeY,
...
public:
    ...
    void handleCAN(const dwCANMessage& canMsg)
    {
        ...
        dwVehicleIOState vehicleState{};
        CHECK_DW_ERROR(dwVehicleIO_getVehicleState(&vehicleState, m_vehicleIO));
        CHECK_DW_ERROR(dwEgomotion_addVehicleState(&vehicleState, m_egomotion));
    }
    ...
    void handleIMU(const dwIMUFrame& imuFrame)
    {
        ...
        msgIMU.timestamp_us += m_offsetTime;
