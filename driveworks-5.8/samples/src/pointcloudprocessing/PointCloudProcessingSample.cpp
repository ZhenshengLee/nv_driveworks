...
bool PointCloudProcessingSample::getSpin()
{
    ...
    while (numLidarsAccumulated < m_lidarCount)
    {
        ...
        case DW_SENSOR_CAN:
        {
            std::swap(m_prevVehicleState, m_currVehicleState);

            CHECK_DW_ERROR(dwVehicleIO_consumeCANFrame(&acquiredEvent->canFrame, 0, m_vehicleIO));
            CHECK_DW_ERROR(dwVehicleIO_getVehicleState(&m_currVehicleState, m_vehicleIO));

            if (m_prevVehicleState.steeringTimestamp < m_currVehicleState.steeringTimestamp)
            {
                CHECK_DW_ERROR(dwEgomotion_addVehicleState(&m_currVehicleState, m_egomotion));
            }
            break;
        }
