...
//################################################################################
dwStatus NVLidar::createSensor(dwSALHandle_t sal, const char* params)
{
...
dwStatus NVLidar::readRawData(const uint8_t** data,
                              size_t* size,
                              dwTime_t* timestamp,
                              dwTime_t timeout_us)
{
...
dwStatus NVLidar::returnRawData(const uint8_t* data)
{
    if (data == nullptr)
    {
...
dwStatus NVLidar::parseDataBuffer(dwLidarDecodedPacket* output, const uint64_t hostTimestamp)
{
    ...
    //Fill output
    output->hostTimestamp   = hostTimestamp; //m_currScanTimestamp;
