...
//################################################################################
dwStatus NVRadar::createSensor(dwSALHandle_t sal, const char* params)
{
    //Set flag
    m_isVirtual = false;
...
dwStatus NVRadar::readRawData(const uint8_t** data,
                              size_t* size,
                              dwTime_t* timestamp,
                              dwTime_t timeout_us)
{
...
dwStatus NVRadar::returnRawData(const uint8_t* data)
{
    if (data == nullptr)
    {
...
dwStatus NVRadar::getConstants(_dwSensorRadarDecoder_constants* constants)
{
    ...
    constants->properties.supportedScanTypes[DW_RADAR_RETURN_TYPE_DETECTION][DW_RADAR_RANGE_MEDIUM]  = false;
    constants->properties.supportedScanTypes[DW_RADAR_RETURN_TYPE_DETECTION][DW_RADAR_RANGE_LONG]    = false;
    constants->properties.supportedScanTypes[DW_RADAR_RETURN_TYPE_DETECTION][DW_RADAR_RANGE_UNKNOWN] = false;
    ...
    constants->properties.maxReturnsPerScan[DW_RADAR_RETURN_TYPE_DETECTION][DW_RADAR_RANGE_MEDIUM]  = 0;
    constants->properties.maxReturnsPerScan[DW_RADAR_RETURN_TYPE_DETECTION][DW_RADAR_RANGE_LONG]    = 0;
    constants->properties.maxReturnsPerScan[DW_RADAR_RETURN_TYPE_DETECTION][DW_RADAR_RANGE_UNKNOWN] = 0;
    ...
    for (size_t i = 0; i < DW_RADAR_RETURN_TYPE_COUNT; ++i)
    {
        for (size_t j = 0; j < DW_RADAR_RANGE_COUNT; ++j)
        {
            ...
            constants->maxPointsPerPacket[i][j] = MAX_DETECTIONS_PER_SCAN / PACKETS_PER_SCAN;
...
size_t NVRadar::getPacketsPerScan(dwRadarReturnType returnType, dwRadarRange range)
{
    ...
    {
        if (range == DW_RADAR_RANGE_SHORT)
        {
            packets = PACKETS_PER_SCAN;
        }
        else if (range == DW_RADAR_RANGE_MEDIUM)
        {
            packets = PACKETS_PER_SCAN;
        }
        else if (range == DW_RADAR_RANGE_LONG)
        {
            packets = PACKETS_PER_SCAN;
        }
        else if (range == DW_RADAR_RANGE_UNKNOWN)
        {
            packets = PACKETS_PER_SCAN;
        }
        else
        {
            throw std::runtime_error("NVRadar::getPacketsPerScan: Provided undefined range for detection");
        }
        break;
    }
    ...
    default:
        throw std::runtime_error("NVRadar::getPacketsPerScan: Trying to get properties of incorrect return range type");
    }
    return packets;
}
