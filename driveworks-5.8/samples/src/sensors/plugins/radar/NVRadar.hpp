...
public:
    ...
    dwStatus readRawData(const uint8_t** data,
                         size_t* size,
                         dwTime_t* timestamp,
                         dwTime_t timeout_us);

    dwStatus returnRawData(const uint8_t* data);
    dwStatus pushData(size_t* lenPushed, const uint8_t* data, const size_t size);
...
private:
    ...
    /**
         ...
         *
         * @return dwStatus size_t Number of packets sent for respective type/range combination.
         */
