...
auto& cameraPipelineEventHanding = cameraEventHandling;
...
public:
    ...
    {
        CHECK_DW_ERROR(dwSensor_stop(m_camera[0]));
        if (m_serializer)
        {
            dwSensorSerializer_stop(m_serializer);
    ...
    bool onInitialize() override
    {
        ...
        for (uint32_t i = 0; i < cnt; i++)
        {
            ...
            {
                CHECK_DW_ERROR(dwSensorCamera_setEventCallback(cameraDevBlkEventHandling, cameraPipelineEventHanding, m_camera[m_totalCameras]));
            }
        ...
        // -----------------------------------------
        {
            ...
            if (m_recordCamera)
            {
                ...
                newParams += std::string("format=") + format;
                ...
                CHECK_DW_ERROR(dwSensorSerializer_initialize(&m_serializer, &serializerParams, m_camera[0]));
    ...
    void onRender() override
    {
        ...
        for (uint32_t i = 0; i < m_totalCameras; ++i)
        {
            ...
            {
                CHECK_DW_ERROR(dwSensorSerializer_serializeCameraFrameAsync(frame[i], m_serializer));
            }
