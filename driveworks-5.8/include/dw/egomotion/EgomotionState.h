...
/**
 * Create empty state from the given egomotion module. The call is equivalent to calling `dwEgomotionState_createEmpty()` with
 * the right selection of parameters.
 *
 * @param[out] state Return here state handle of an empty state.
 ...
 *
 * @note Ownership of the state goes back to caller. The state has to be released with `dwEgomotionState_release()`.
 */
DW_API_PUBLIC
 ...
 *
 * @param[in,out] state Preallocated state to be filled with latest state
...
/**
 * Returns the capacity of the history, i.e. same parameter as passed with `dwEgomotionParameters.historySize'
 * or default value used instead. Returned capacity can be used by `dwEgomotionState_createEmpty()` for initialization
 * of the empty state, if same size of the history is desired as egomotion does uses internally.
 *
 * @param[out] capacity A pointer where to return the capacity of the history.
 ...
 *       `dwEgomotion_getState()` will fill out this state's only with as much data as can fit, dropping oldest entries.
 *       This in turn would mean that calls to `dwEgomotionState_computeRelativeTransformation()` might not succeed if
 ...
 *
 * @param[out] bufferSize Size of the buffer required to hold serialized state
 * @param[in] state State handle to be serialized
 *
 * @return DW_INVALID_ARGUMENT - if the provided `bufferSize` is nullptr. <br>
 *         DW_INVALID_HANDLE - if provided handle is invalid <br>
 *         DW_SUCCESS - if the call was successful. <br>
 */
DW_API_PUBLIC
dwStatus dwEgomotionState_getMaxNumBytes(size_t* bufferSize, dwConstEgomotionStateHandle_t state);

/**
 * Serialize the state out into the provided buffer.
 *
 * @param[out] numBytes number of bytes used to dump out the state. The number is always lower or equal to `dwEgomotionState_getMaxNumBytes()`
 * @param[out] buffer Buffer where the state will be serialized to
 * @param[in] bufferSize Size of the buffer provided, i.e. must be at least `dwEgomotionState_getMaxNumBytes()` large
 * @param[in] state State handle to be serialized
 *
 ...
 */
DW_API_PUBLIC
dwStatus dwEgomotionState_serialize(size_t* numBytes, uint8_t* buffer, size_t bufferSize, dwConstEgomotionStateHandle_t state);
 ...
 *
 * @return DW_INVALID_ARGUMENT - if the provided `buffer` is nullptr. <br>
 *         DW_OUT_OF_BOUNDS - if the provided `bufferSize` isn't enough to hold the state <br>
 ...
 *
 * @param[out] state Handle of the preallocated egomotion state.
 * @param[in] source Handle of the existing egomotion state.
 *
