 ...
 *
 * @return DW_INVALID_ARGUMENT - if pointer to the engine handle is invalid \n
 * @return DW_INVALID_HANDLE - if provided context handle is invalid, i.e., null or of wrong type \n
 * @return DW_SUCCESS - no error
 */
DW_API_PUBLIC
 ...
 *
 * @return DW_INVALID_ARGUMENT - if the arguments in the parameters are invalid, if the egomotion handle is invalid or if the routine handle is invalid \n
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid, i.e., null or of wrong type \n
 * @return DW_BUFFER_FULL - if the number of sensors need to be calibrated exceeds \n
 * @return DW_CALIBRATION_MAXROUTINES value \n
 * @return DW_SUCCESS - no error
 */
DW_API_PUBLIC
 ...
 *
 * @return DW_INVALID_ARGUMENT - if the arguments in the parameters are invalid or if the routine handle is invalid \n
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid, i.e., null or of wrong type, or the given indices do point to
 *                             appropriate entries in the rig file \n
 * @return DW_BUFFER_FULL - if the number of sensors need to be calibrated exceeds the DW_CALIBRATION_MAXROUTINES value \n 
 * @return DW_OUT_OF_BOUNDS - if the given indices point outside the rig file \n
 * @return DW_SUCCESS - no error
 */
DW_API_PUBLIC
 ...
 *
 * @param[out] routine A pointer to the new calibration routine handle associated with this engine
 * @param[in]  lidarIndex The index of the lidar to be calibrated. This is the same
 *             as the sensor index in the rig module
 * @param[in]  canIndex Index of the CAN bus from which speed information will be provided as dwVehicleIOState
 ...
 * @return DW_INVALID_ARGUMENT - if the arguments in the parameters are invalid or if the routine handle is invalid \n
 * @return DW_INVALID_HANDLE   - if provided engine handle is invalid,i.e. null or of wrong type \n
 * @return DW_BUFFER_FULL      - if the number of sensors need to be calibrated exceeds DW_CALIBRATION_MAXROUTINES value. \n
 * @return DW_SUCCESS - no error
 */
DW_API_PUBLIC
 ...
 *
 * @param[out] routine A pointer to the new calibration routine handle associated with this engine
 * @param[in] radarIndex The index of the radar we are creating a calibration for. This is the same
 *            as the sensor index in the rig module
 * @param[in] canIndex Index of the CAN bus from which speed information will be provided as dwVehicleIOState
 * @param[in] params Specifies the parameters that are used to in calibrating the radar
 * @param[in] engine A pointer to the calibration engine handle that owns the sensor
 *
 * @return DW_INVALID_ARGUMENT - if the arguments in the parameters are invalid or if the egomotion handle is invalid \n
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid, i.e., null or of wrong type \n
 * @return DW_BUFFER_FULL - if the number of sensors need to be calibrated exceeds DW_CALIBRATION_MAXROUTINES value \n
 * @return DW_SUCCESS - no error
 */
 ...
 *
 * @param[out] routine A pointer to the new calibration routine handle associated with this engine
 * @param[in] vehicleSensorIndex The index of the vehicle sensor. This is the same
 *             as the sensor index in the rig configuration module
 * @param[in] leftSensorIndex The index of the left camera sensor. This is the same
 *             as the sensor index in the rig configuration module
 * @param[in] rightSensorIndex The index of the right camera sensor. This is the same
 *             as the sensor index in the rig configuration module
 ...
 *       - none
 *
 * @return DW_INVALID_ARGUMENT - if the arguments in the parameters are invalid \n
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid \n
 * @return DW_BUFFER_FULL - if the number of sensors need to be calibrated exceeds DW_CALIBRATION_MAXROUTINES value \n
 * @return DW_SUCCESS - no error
 */
DW_API_PUBLIC
 ...
 *
 * @param[out] routine A pointer to the new calibration routine handle associated with this engine.
 * @param[in] sensorIndex The index of the sensor that provides vehicle i/o data. This should be the same
 *            as the number in the rig configuration module
 * @param[in] params Specifies the parameters that are used to in calibrating the vehicle parameters
 * @param[in] egoMotion Specifies the ego motion module that holds the motion of the vehicle, 
                        needs to be of DW_EGOMOTION_IMU_ODOMETRY motion type
 * @param[in] vehicle A pointer to a vehicle parametere data.
 * @param[in] engine A pointer to the calibration engine handle that owns the sensor.
 *
 ...
 *
 * @return  DW_INVALID_ARGUMENT - if the arguments in the parameters are invalid or wrong egomotion type is passed \n
 * @return  DW_INVALID_HANDLE - if provided engine handle is invalid \n
 * @return  DW_BUFFER_FULL - if the number of sensors need to be calibrated exceeds DW_CALIBRATION_MAXROUTINES value \n
 * @return  DW_SUCCESS - no error
 **/
 ...
 * @param[in] engine Specifies the handle of the calibration routine that is managing the calibration
 *
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid, i.e., null or of wrong type or if the calibration routine is not managed by the calibration engine \n
 * @return DW_SUCCESS - no error
 */
DW_API_PUBLIC
 ...
 * 
 * @return DW_INVALID_ARGUMENT - if the calibration routine is not managed by the calibration engine \n
 * @return DW_INVALID_HANDLE   - if any of the provided handles is invalid, i.e., null or of wrong type \n
 * @return DW_SUCCESS - no error
**/
DW_API_PUBLIC
 ...
 * 
 * @return DW_INVALID_HANDLE   - if any of the provided handles is invalid, i.e., null or of wrong type \n
 * @return DW_INVALID_ARGUMENT - if the calibration routine is not managed by the calibration engine or supported is null \n
 * @return DW_SUCCESS - no error
 */
DW_API_PUBLIC
 ...
 * @return DW_INVALID_HANDLE   - if any of the provided handles is invalid, i.e., null or of wrong type \n
 * @return DW_INVALID_ARGUMENT - if the calibration routine is not managed by the calibration engine \n
 * @return DW_NOT_SUPPORTED    - if the calibration routine is not estimating a sensorToRigTransformation \n
 * @return DW_SUCCESS - no error
**/
DW_API_PUBLIC
 ...
 * @param[in] engine Specifies the calibration engine module we are checking against
 *
 * @return DW_INVALID_HANDLE   - if any of the provided handles is invalid \n
 * @return DW_INVALID_ARGUMENT - if the calibration routine is not managed by the calibration engine 
 *                               or if the calibration routine is not estimating a sensorToSensorTransformation \n
 * @return DW_NOT_AVAILABLE    - if the sensors indices provided are not related to the calibration routine \n
 * @return DW_SUCCESS - no error
**/
DW_API_PUBLIC
 ...
 * @param[in] engine A pointer to the calibration engine handle that owns the sensor
 *
 * @return DW_INVALID_ARGUMENT - if given `odometrySpeedFactor` is null \n
 * @return DW_INVALID_HANDLE - if any of the provided handles is invalid, i.e., null or of wrong type \n 
 * @return DW_NOT_SUPPORTED - if calibration routine is not performing an odometry speed factor estimation \n
 * @return DW_SUCCESS - no error
 *
 ...
 * @param[in] engine A pointer to the calibration engine handle that owns the sensor
 *
 * @return DW_INVALID_ARGUMENT - if given `radius` is null <br>
 *         DW_INVALID_HANDLE - if any of the provided handles is invalid <br>
 *         DW_NOT_SUPPORTED - if calibration routine is not estimating wheel radius <br>
 *         DW_SUCCESS - no error
 */
DW_API_PUBLIC
 ...
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid \n 
 * @return DW_INVALID_ARGUMENT - if the calibration routine is not managed by the calibration engine \n
 * @return DW_NOT_SUPPORTED - if calibration routine is not estimating wheel radius \n
 * @return DW_SUCCESS - no error
 *
 **/
DW_API_PUBLIC
 ...
 * @param[in] engine            Specifies the calibration engine module we are checking against
 *
 * @return DW_INVALID_ARGUMENT - if the d_featureCount, d_ages, or d_locationHistory pointer is invalid \n
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid, i.e., null or of wrong type \n
 * @return DW_INTERNAL_ERROR - if an internal unrecoverable error was detected \n
 * @return DW_SUCCESS - no error
 **/
DW_API_PUBLIC
 ...
 *
 * @return DW_INVALID_ARGUMENT - if the imuFrame pointer is invalid \n
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid, i.e., null or of wrong type \n
 * @return DW_INTERNAL_ERROR - if an internal unrecoverable error was detected \n
 * @return DW_SUCCESS - no error
 **/
 ...
 * @note A valid calibration engine handle has to be provided.
 * 
 * @return DW_INVALID_ARGUMENT - if the lidarPoints pointer is invalid \n
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid, i.e., null or of wrong type \n
 * @return DW_INTERNAL_ERROR - if an internal unrecoverable error was detected \n
 * @return DW_SUCCESS - no error
 **/
 ...
 *            If set to NULL, deltaPoseLidarTimeAToTimeB is used to approximate deltaPoseRigTimeAToTimeB for
 *            the algorithm.
 ...
 * @param[in] engine Specifies the calibration engine module we are checking against
 *
 * @return DW_INVALID_ARGUMENT - if the radarPoints pointer is invalid \n
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid, i.e., null or of wrong type \n
 * @return DW_INTERNAL_ERROR - if an internal unrecoverable error was detected \n
 * @return DW_SUCCESS - no error
 *
 **/
DW_API_PUBLIC
dwStatus dwCalibrationEngine_addRadarDopplerMotion(dwRadarDopplerMotion const* radarMotion,
                                                   uint32_t sensorIndex,
                                                   dwCalibrationEngineHandle_t engine);
 ...
 *
 * @return DW_INVALID_ARGUMENT - if matches are invalid \n
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid \n
 * @return DW_INTERNAL_ERROR - if an internal unrecoverable error was detected \n
 * @return DW_SUCCESS - no error
 **/
 ...
 * @note A valid calibration engine handle has to be provided.
 * 
 * @return DW_INVALID_ARGUMENT - if the vioState pointer is invalid \n
 * @return DW_INVALID_HANDLE - if provided engine handle is invalid \n
 * @return DW_INTERNAL_ERROR - if an internal unrecoverable error was detected \n
 * @return DW_SUCCESS - no error
 **/
 ...
 */
DW_API_PUBLIC
dwStatus dwCalibrationEngine_addVehicleIONonSafetyState(dwVehicleIONonSafetyState const* vioNonSafetyState,
                                                        uint32_t sensorIndex,
                                                        dwCalibrationEngineHandle_t engine);
 ...
 */
DW_API_PUBLIC
dwStatus dwCalibrationEngine_addVehicleIOActuationFeedback(dwVehicleIOActuationFeedback const* vioActuationFeedback,
                                                           uint32_t sensorIndex,
                                                           dwCalibrationEngineHandle_t engine);
