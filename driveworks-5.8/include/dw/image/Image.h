...
// Forward defines from GL
// Note: GL defines the specific bitdepth of these types
//       so it is safe to declare then here without including GL.h
...
#endif //#if VIBRANTE_PDK_DECIMAL < 6000400
#include <nvscibuf.h>
#endif
...
typedef struct dwImageSensorStatistics
{
    ...
    /// Value range: [1, DEVBLK_CDI_MAX_NUM_TEMPERATURES]. The DEVBLK_CDI_MAX_NUM_TEMPERATURES is defined in NvSIPLCDICommon.h of NvSIPL
 ...
 *
 * @param[out] image A handle to the image
 * @param[in] nvmImage The pointer to an NvMediaImage.
 *
 * @return DW_SUCCESS if the image was created, <br>
 *         DW_INVALID_ARGUMENT if the given image types are invalid or the image pointer is null, <br>
 */
DW_API_PUBLIC
DW_DEPRECATED("dwImage_createAndBindNvMedia is deprecated. Since NvMediaImage is entirely removed this functionality is entirely removed. .")
dwStatus dwImage_createAndBindNvMedia(dwImageHandle_t* image, NvMediaImage* nvmImage);
#endif
#endif
...
/**
 * Retrieves the dwImageCPU of a dwImageHandle_t. The image must have been created as a DW_IMAGE_CPU type
...
/**
 * Retrieves the dwImageCUDA of a dwImageHandle_t. The image must have been created as a DW_IMAGE_CUDA type.
 * Note that any modification to the image retrieved will modify the content of the original handle
 *
...
#ifdef VIBRANTE
/**
 * Retrieves the dwImageNvMedia of a dwImageHandle_t. The image must have been created as a DW_IMAGE_NVEMDIA type
 * Note that any modification to the image retrieved will modify the content of the original handle
 *
