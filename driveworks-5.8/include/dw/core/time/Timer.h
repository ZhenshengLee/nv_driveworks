...
// Forward declaration of dwContextHandle_t to not have circular dependency
// @note: The API using the dwContextHandle_t is deprecated and will be removed with next major release
 ...
 * @param[in] context Specifies the handle to the context under which the timer should
 *                    be created.
 *
 * @return DW_NOT_AVAILABLE The method is not available and has been deprecated
 */
DW_API_PUBLIC
DW_DEPRECATED("This method is replaced by dwTimer_initializeFromSource() which require a dwTimeSourceHandle_t which can be retrieved from dwContext")
dwStatus dwTimer_initialize(dwTimerHandle_t* const timer, char8_t const* const timerName,
