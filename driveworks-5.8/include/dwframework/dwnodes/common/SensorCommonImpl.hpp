        ...
        if (find == "video=")
        {
            ...
            if (start != FixedString1024::NPOS)
            {
                ...
                if (commonPrefix == FixedString1024::NPOS)
                {
                    ...
                    FRWK_LOGW << "adjustPath: [sensorId: " << sensorId << " with param:" << paramString << "] Failed to extract rig directory path, so that cannot adjust timestamp path" << Logger::State::endl;
                    commonPrefix = 0;
                }
