...
public:
    ...
    RingBuffer()
        ...
        , m_size(0U)
    {
        static_assert(CapacityAtCompileTime_ != 0, "RingBuffer: this container has been defined as using dynamically allocated storage (CapacityAtCompileTime==0). This constructor only applies to static storage. Call one of the other constructors instead, e.g. RingBuffer(MAX_SIZE).");
    }
     ...
     **/
    // Note: Current implementation is missing emplace_back_maybe() for simplicity.
    // This results in one extra copy-constructor called.
    // This is optimized out for T with default empty constructors.
    inline bool push_back_maybe(T&& element)
    {
        ...
        if (m_size < capacity())
        {
            new (&(span<T>(m_storage.data(), m_backEndIdx + 1U)[m_backEndIdx])) T(std::move(element));
    ...
    // TODO(dwplc): FP - constructor/destructor marked as C-style cast
    // coverity[autosar_cpp14_a5_2_2_violation]
    inline bool push_back_maybe(const T& element) { return push_back_maybe(std::move(T(element))); }
    ...
    inline auto push_back(T&& element) -> T&
    {
        if (capacity() == 0U)
        {
            // TODO(dwplc): FP - ExceptionWithStackTrace is not casting to non-const
            // coverity[cert_str30_c_violation]
            throw BufferFullException("RingBuffer::push_back: cannot push_back into an RingBuffer "
        ...
        {
            static_cast<void>(push_back_maybe(std::move(element)));
        }
    ...
    // TODO(dwplc): FP - constructor/destructor marked as C-style cast
    // coverity[autosar_cpp14_a5_2_2_violation]
    auto push_back(const T& element) -> T& { return push_back(std::move(T(element))); }

    // -----------------------------------------------------------------------------
    /**
     * Push new element at the front.
     * @return If buffer is full this method will return false and will not add the element.
     * @note Current implementation is missing emplace_front_maybe() for simplicity. This results in one extra copy-constructor called.
     *       This is optimized out for T with default empty constructors.
     **/
    inline bool push_front_maybe(T&& element)
    {
        ...
        new (&m_storage[newFrontIdx]) T(std::move(element));
    ...
    // TODO(dwplc): FP - constructor/destructor marked as C-style cast
    // coverity[autosar_cpp14_a5_2_2_violation]
    inline bool push_front_maybe(const T& element) { return push_front_maybe(std::move(T(element))); }
    ...
    inline auto push_front(T&& element) -> T&
    {
        if (capacity() == 0U)
        {
            throw BufferFullException("RingBuffer::push_front: cannot push_back into an RingBuffer "
                                      "with zero capacity. Call allocate() first before pushing "
                                      "objects into the container.");
        }
        ...
        {
            static_cast<void>(push_front_maybe(std::move(element)));
        }
    ...
    auto push_front(const T& element) -> T& { return push_front(std::move(static_cast<T>(element))); }
