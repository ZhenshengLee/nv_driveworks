...
#include <limits>
#include <functional>
#include <dw/core/utility/Hash.hpp>
...
/// Default hash that can handle enums
template <class T>
using DefaultHash = typename std::conditional<std::is_enum<T>::value, EnumHash<T>, std::hash<T>>::type;
...
protected:
    ...
    BaseHashContainer(TStorage&& storage, BaseHashContainer<TKey, TElement, TKeyFromElement, THash, TKeyEqual, S2>&& other)
        : m_storage(std::move(storage))
    {
        insert_range_move(make_span(other));
        other.clear();
    }
    ...
    bool eraseAtIndex(size_t eraseIndex)
    {
        ...
        while (maxMoveCount > 0U) // can't use a for-loop due to AUTOSAR A6-5-1
        {
            ...
            while (maxIndexIterationCount > 0U) // can't use a for-loop due to AUTOSAR A6-5-1
            {
                ...
                Entry& nextEntry = m_storage[nextIndex];
