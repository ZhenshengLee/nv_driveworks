...
public:
    ...
    span(span&& other)      = default;      ///< Move constructor
    span& operator=(const span&) = default; ///< Copy operator
    span& operator=(span&&) = default;      ///< Move operator
    ...
    {
        return safeMul(m_size, sizeof(T)).value_or(util::numeric_limits<size_t>::max());
    }
...
public:
    ...
    CUDA_BOTH_INLINE BaseSpan(BaseSpan&& other)                 = default; ///< Move constructor
    CUDA_BOTH_INLINE BaseSpan& operator=(BaseSpan&& other) = default;      ///< Move operator
        ...
        , m_size(sizeIn)
    {
        ...
        bool bOverflow = true;
        if (util::numeric_limits<size_t>::max() / sizeof(T) >= sizeIn[0])
        {
            bOverflow    = false;
...
#endif
            {
                ...
                if (util::numeric_limits<size_t>::max() / sizeof(T) < count)
                {
                    bOverflow = true;
                }
            }
...
#else
            LOGSTREAM_ERROR(nullptr) << "BaseSpan(): Overflow error" << Logger::State::endl;
#endif
...
public:
    ...
    span(span&& other)                 = default; ///< Move constructor
    span& operator=(span&& other) = default;      ///< Move operator
    ~span()                       = default;      ///< Destructor
