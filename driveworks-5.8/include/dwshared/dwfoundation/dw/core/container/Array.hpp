...
/// be allocated. It differes in semantics based on it's aggregate initialization, and can be used in
...
public:
    ...
    CUDA_BOTH_INLINE
    auto operator[](size_t const i) -> T&
    {
...
{
    return util::equal(first.begin(), first.end(), second.begin());
}
...
CUDA_BOTH_INLINE bool operator<(Array<T, N> const& first, Array<T, N> const& second) noexcept
{
    return util::lexicographicalCompare(first.begin(), first.end(), second.begin(), second.end());
}
