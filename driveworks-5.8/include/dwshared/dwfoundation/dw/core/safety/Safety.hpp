...
#include <dw/core/utility/CheckedIntegerCast.hpp>
...
struct Add<Ta, Tb, true, true>
{
    ...
    {
        Ta const left = util::numeric_limits<Ta>::max() - a;
        if (b >= left)
        {
...
struct Add<Ta, Tb, false, false>
{
    CUDA_BOTH_INLINE static auto impl(const Ta a, const Tb b) -> SafetyResult<Ta>
    {
        if (a > 0)
        {
            if (b > 0)
            {
                if (b > util::numeric_limits<Ta>::max() - a)
                {
                    return SafetyResult<Ta>(SafetyIssueType::OverflowError);
                }
                return SafetyResult<Ta>(a + b);
            ...
            else // b <= 0
            {
                ...
                if (b < util::numeric_limits<Ta>::lowest() && b - util::numeric_limits<Ta>::lowest() > a)
                {
                    return SafetyResult<Ta>(SafetyIssueType::UnderflowError);
                }
        ...
        else // a <= 0
        {
            if (b > 0)
            {
                ...
                // coverity[result_independent_of_operands]
                if (b > util::numeric_limits<Ta>::max() && b - util::numeric_limits<Ta>::max() > std::abs(a))
                {
                    return SafetyResult<Ta>(SafetyIssueType::OverflowError);
                }
                return SafetyResult<Ta>(a + b);
            ...
            {
                if (b < util::numeric_limits<Ta>::lowest() - a)
                {
                    return SafetyResult<Ta>(SafetyIssueType::UnderflowError);
                }
                return SafetyResult<Ta>(a + b);
...
{
    return cuda::min(hi, cuda::max(v, lo));
}
...
CUDA_BOTH_INLINE auto safeMul(TInteger const& a, meta::TypeIdentityT<TInteger> const& b) -> SafetyResult<TInteger>
{
    ...
    constexpr TInteger MAX = util::numeric_limits<TInteger>::max();
    constexpr TInteger MIN = util::numeric_limits<TInteger>::min();
...
CUDA_BOTH_INLINE auto safeDiv(T const& a, T const& b) -> SafetyResult<T>
{
    ...
    else if (a == util::numeric_limits<T>::min() && b == -1)
    {
...
CUDA_BOTH_INLINE bool clampedAddition(T& sum, const T a, const T b) noexcept
{
    ...
    if (a >= 0)
    {
        min = util::numeric_limits<T>::min();
        // TODO(dwplc): FP - Complains cast char to larger integer size. But all variables are type T. No cast here.
        // coverity[cert_str34_c_violation]
        max = util::numeric_limits<T>::max() - a;
    }
    else
    {
        // TODO(dwplc): FP - Complains cast char to larger integer size. But all variables are type T. No cast here.
        // coverity[cert_str34_c_violation]
        min = util::numeric_limits<T>::min() - a;
        max = util::numeric_limits<T>::max();
    }
...
CUDA_BOTH_INLINE bool clampedSubtraction(T& diff, const T a, const T b) noexcept
{
    ...
    if (a >= 0)
    {
        // TODO(dwplc): FP - Complains cast char to larger integer size. But all variables are type T. No cast here.
        // coverity[cert_str34_c_violation]
        min = std::is_signed<T>::value ? (a - util::numeric_limits<T>::max()) : 0;
        // TODO(dwplc): FP - Complains cast char to larger integer size. But all variables are type T. No cast here.
        // coverity[cert_str34_c_violation]
        max = std::is_signed<T>::value ? util::numeric_limits<T>::max() : a;
    }
    else
    {
        min = util::numeric_limits<T>::min();
        // TODO(dwplc): FP - Complains cast char to larger integer size. But all variables are type T. No cast here.
        // coverity[cert_str34_c_violation]
        max = a - util::numeric_limits<T>::min();
    }
...
CUDA_BOTH_INLINE bool clampedMultiplication(T& mult, const T a, const T b) noexcept
{
    ...
    else if (a < 0)
    {
        ...
        min = util::numeric_limits<T>::max() / a;
        // TODO(dwplc): FP - Complains cast char to larger integer size. But all variables are type T. No cast here.
        // coverity[cert_str34_c_violation]
        max = a < static_cast<T>(-1) ? (util::numeric_limits<T>::min() / a) : util::numeric_limits<T>::max();
    }
    else // a == 0
    {
        min = util::numeric_limits<T>::min();
    ...
    if (b > max)
    {
        // TODO(dwplc): FP - Complains cast char to larger integer size. But all variables are type T. No cast here.
        // coverity[cert_str34_c_violation]
        mult = a > 0 ? util::numeric_limits<T>::max() : util::numeric_limits<T>::min();
        return false;
    }
    else if (b < min)
    {
        // TODO(dwplc): FP - Complains cast char to larger integer size. But all variables are type T. No cast here.
        // coverity[cert_str34_c_violation]
        mult = a > 0 ? util::numeric_limits<T>::min() : util::numeric_limits<T>::max();
        return false;
    }
    else
    {
...
{
    auto clamped = detail::clamp(x, static_cast<T>(0), util::numeric_limits<T>::max());
...
{
    auto clamped = detail::clamp(x, static_cast<T>(1), util::numeric_limits<T>::max());
    // TODO(dwplc): FP - "Values are clamped to valid input domain"
    // coverity[autosar_cpp14_a0_4_4_violation]
