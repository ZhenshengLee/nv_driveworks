...
public:
    ...
    struct LoggerMessageInfo
    {
        ...
        FixedString<TAG_SIZE> tag{};
        SourceCodeLocation loc = SourceCodeLocation("", 0);
        mutable FixedString<TID_SIZE> threadId{};
    };
    ...
    static void appendTimestamp(FixedString<N>& buffer)
    {
        timespec time{};
     ...
     *
     */
    static void setThreadId(FixedString<TID_SIZE> const& tid);
    ...
    static FixedString<TID_SIZE> getThreadId();
    ...
    public:
        ...
        FixedString<LOG_BUFFER_SIZE>& getBuffer() { return m_buffer; }
        explicit LoggerStream(Logger* const logger, LoggerMessageInfo&& info);
        FixedString<LOG_BUFFER_SIZE> m_buffer;
...
private:
    ...
    static thread_local FixedString<TID_SIZE> m_threadId; //!< identifier for the current thread, defaults to an atomically incrementing integer
...
Logger::LoggerStream& operator<<(Logger::LoggerStream& stream, FixedString<S> const& s)
{
...
// coverity[autosar_cpp14_a13_2_2_violation]
template <std::size_t S>
inline Logger::LoggerStream& operator<<(Logger::LoggerStream&& stream, FixedString<S> const& v)
{
    return stream << v;
}
