...
//////////////////////////////////////////////
/**
 * Mapped host memory, alias of UniqueCudaHostPtr for easy using
 *
 * @tparam T Data type of managed mapped memory blocks by unique pointer
 */
template <typename T>
using UniqueMappedPtr = UniqueCudaHostPtr<T>;
 ...
 *
 * @return An unique pointer(#UniqueMappedPtr<T>) to allocated mapped memory
 *         for data type @p T
 */
template <typename T>
UniqueMappedPtr<T> makeUniqueMapped()
{
    return makeUniqueCudaHost<T>(cudaHostAllocMapped);
}
...
{
    return makeUniqueCudaHost<T>(itemCount, cudaHostAllocMapped);
}
}
} // namespace dw
