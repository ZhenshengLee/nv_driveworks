...
private:
    ...
    dw::core::VectorFixed<struct pollfd> m_pfds;
    // Counter for packet drop
    uint64_t m_dropCount = 0U;

    bool isBroadcastPossible();
    bool isBroadcastConnectionValid(std::shared_ptr<SocketConnection> connection, struct pollfd const& pfd);
};
...
public:
    ...
    SocketServer(uint16_t const port, size_t const connectionPoolSize, bool const reusePort, dw::core::FixedString<8> const& sockPrefix);
...
private:
    ...
    dw::core::FixedString<8> m_sockPrefix;
};
