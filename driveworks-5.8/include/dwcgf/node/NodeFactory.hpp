...
void registerNode(const char* className)
{
    ...
    if (!metaObject)
    {
        throw Exception(DW_BAD_ALLOC, "NodeFactory: cannot allocate meta object");
    }
    ...
    {
        throw Exception(DW_INVALID_ARGUMENT, "registerNode() repeatedly called for the same class name: ", className);
    }
    else
    {
