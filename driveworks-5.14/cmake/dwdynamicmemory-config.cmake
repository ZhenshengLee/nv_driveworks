...
set(NAME dwdynamicmemory)
set(LIBS dwdynamicmemory)
...
foreach(LIB ${LIBS})
    ...
    find_path(${LIB}_INCLUDE_DIR
      NAMES dw/core/DynamicMemory.h
      PATHS ${PATH}
      NO_DEFAULT_PATH
    )

    if (NOT ${LIB}_INCLUDE_DIR)
        message(FATAL_ERROR "Cannot find 'dw/core/DynamicMemory.h' for ${LIB} at ${PATH}")
    else()
        message(STATUS "Found 'dw/core/DynamicMemory.h' in ${${LIB}_INCLUDE_DIR}")
    endif()
    ...
    add_library(dwdynamicmemory::${LIB}-lib SHARED IMPORTED)

    set_target_properties(dwdynamicmemory::${LIB}-lib PROPERTIES
      IMPORTED_LOCATION "${${LIB}_LIBRARY}"
    )
    target_link_libraries(${NAME} INTERFACE dwdynamicmemory::${LIB}-lib)
