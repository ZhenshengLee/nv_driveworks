...
set(NAME dwframework)
set(LIBS dwpbwire dwcgf dwframework_dwnodes dwframework_dwnodes_channel_packets dwroadcast-frontend)
...
foreach(LIB ${LIBS})
    ...
    find_path(${LIB}_DWFRAMEWORK_INCLUDE_DIR
      NAMES dwframework/dwnodes/common/factories/DWChannelFactory.hpp
      PATHS ${PATH}
      NO_DEFAULT_PATH
    )

    if (NOT ${LIB}_DWFRAMEWORK_INCLUDE_DIR)
        message(FATAL_ERROR "Cannot find 'dwframework/dwnodes/common/factories/DWChannelFactory.hpp' for ${LIB} at ${PATH}")
    else()
        message(STATUS "Found 'dwframework/dwnodes/common/factories/DWChannelFactory.hpp' in ${${LIB}_DWFRAMEWORK_INCLUDE_DIR}")
    endif()

    target_include_directories(${NAME} INTERFACE "${${LIB}_DWFRAMEWORK_INCLUDE_DIR}")
    ...
    add_library(dwframework::${LIB}-lib SHARED IMPORTED)

    set_target_properties(dwframework::${LIB}-lib PROPERTIES
      IMPORTED_LOCATION "${${LIB}_LIBRARY}"
    )

    target_link_libraries(${NAME} INTERFACE dwframework::${LIB}-lib)
