...
protected:
    explicit ManagedPortOutputBase(const dw::core::StringView& name, ConstructProperties props, GenericDataReference&& ref);
...
struct vectorIterable
{
    ...
    // coverity[cert_dcl51_cpp_violation]
    iterator<const T> begin() const { return iterator<const T>(m_allBuffers.begin()); }

    // coverity[cert_dcl51_cpp_violation]
    iterator<const T> end() const { return iterator<const T>(m_allBuffers.end()); }
...
public:
    ...
    ManagedPortOutput(const dw::core::StringView& name, ConstructProperties props, SpecimenT& ref)
        : ManagedPortOutputBase(name, std::move(props), make_specimen<T>(&ref))
    {
    }
    ...
    ManagedPortOutput(const dw::core::StringView& name, SpecimenT& ref)
        : ManagedPortOutputBase(name, {}, make_specimen<T>(&ref))
    {
    }
    ...
    ManagedPortOutput(const dw::core::StringView& name, ConstructProperties props)
        : ManagedPortOutputBase(name, std::move(props), make_specimen<T>(nullptr))
    {
    }
    ...
    ManagedPortOutput(const dw::core::StringView& name)
        : ManagedPortOutputBase(name, {}, make_specimen<T>(nullptr))
    {
    }
...
public:
    ...
    ManagedPortInput(const dw::core::StringView& name)
        : ManagedPortInputBase(name, {}, make_specimen<T>(nullptr))
    {
    }
    ...
    auto getBufferIfAvailableAndValid() -> T*
    {
        if (isBufferAvailable())
        {
            ...
            const bool isValid{!valid.has_value() || (valid->validity == DW_VALIDITY_VALID)};
            if (isValid)
            {
                return getBuffer();
            }
