...
public:
    ...
    */
    ...
    {
        if (m_props.isGPUMapped)
        {
            throw ExceptionWithStatus(DW_NOT_SUPPORTED, "TensorNvSciPacketBase: pinned memory not supported in nvscibuf");
        }
    ...
    size_t getPitch() const
    {
        ...
        if (m_props.tensorLayout == DW_DNN_TENSOR_LAYOUT_NCHW || m_props.tensorLayout == DW_DNN_TENSOR_LAYOUT_NHWC)
        {
            ...
            {
                pitch *= m_props.dimensionSize[idx];
            }
        }
        else
        {
