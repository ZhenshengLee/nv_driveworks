...
static_assert(DW_CALIBRATION_MAXROUTINES >= 0, "DW_CALIBRATION_MAXROUTINES must be non-negative");
static constexpr uint32_t DW_CALIBRATION_MAX_SENSORS{static_cast<uint32_t>(DW_CALIBRATION_MAXROUTINES)};
...
struct dwRoadCastNodeCalibrationIMUIntrinsicsArray
{
    dwRoadCastCalibratedIMUIntrinsics intrinsics[SELF_CALIBRATION_NODE_MAX_IMUS];
    uint32_t count;
};
