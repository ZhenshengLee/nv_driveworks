...
public:
    ...
    static std::unique_ptr<dwSelfCalibrationIMUNode> create(ParameterProvider& provider);
    ...
    static constexpr auto describeParameters()
    {
        // TODO(csketch): RFD --  user defined literal being interpreted as c style cast.
        // coverity[autosar_cpp14_a5_2_2_violation] RFD Pending: TID-1983
        return describeConstructorArguments<dwSelfCalibrationIMUNodeParam, dwContextHandle_t>(
            describeConstructorArgument(
                DW_DESCRIBE_UNNAMED_PARAMETER(
                    dwConstRigHandle_t,
                    &dwSelfCalibrationIMUNodeParam::rigHandle),
                DW_DESCRIBE_PARAMETER(
                    uint32_t,
                    "sensorIndex"_sv,
                    &dwSelfCalibrationIMUNodeParam::sensorIndex),
                DW_DESCRIBE_PARAMETER(
                    bool,
                    "enableCalibration"_sv,
                    &dwSelfCalibrationIMUNodeParam::enableCalibration),
                DW_DESCRIBE_PARAMETER(
                    uint32_t,
                    "histogramMaxOutlierFailureCount"_sv,
                    &dwSelfCalibrationIMUNodeParam::histogramMaxOutlierFailureCount),
                DW_DESCRIBE_PARAMETER(
                    uint32_t,
                    "channelFifoSize"_sv,
                    &dwSelfCalibrationIMUNodeParam::channelFifoSize)),
            describeConstructorArgument(
                DW_DESCRIBE_UNNAMED_PARAMETER(
                    dwContextHandle_t)));
    }

    dwSelfCalibrationIMUNode(dwSelfCalibrationIMUNodeParam const& param, dwContextHandle_t const ctx);
};
