 ...
 * @param[out] tensorHandle A tensor handle
 * @param[in] properties Pointer to the tensor properties
 * @param[in] extMem Pointer to external memory resources.
 * @param[in] stride Stride size of external memory resources.
 *
 * @return DW_SUCCESS if the tensor was created successfully, <br>
 *         DW_INVALID_ARGUMENT if any of the given parameters is invalid. <br>
 */
DW_API_PUBLIC
dwStatus dwDNNTensor_createWithExtMem(dwDNNTensorHandle_t* const tensorHandle,
                                      dwDNNTensorProperties const* const properties,
                                      uint8_t* const extMem,
                                      size_t const stride);

/**
 * Destroys the tensor handle and frees any memory created by dwDNNTensor_create()
 * or dwDNNTensor_createWithExtMem().
 *
 ...
 *
 * @param[out] time A pointer to the timestamp
 * @param[in] tensorHandle A tensor handle
 *
 * @return DW_SUCCESS if the timestamp is successfully retrieved, <br>
 *         DW_INVALID_ARGUMENT if the given tensor pointer or tensor handle is invalid. <br>
 */
DW_API_PUBLIC
dwStatus dwDNNTensor_getTimestamp(dwTime_t* const time, dwDNNTensorHandle_t const tensorHandle);

/**
 * Set timestamp of a dwDNNTensorHandle_t.
 *
 * @param[in] time Input timestamp
 * @param[in] tensorHandle A tensor handle
 *
 * @return DW_SUCCESS if the timestamp is successfully set, <br>
 *         DW_INVALID_ARGUMENT if the given tensor pointer or tensor handle is invalid. <br>
 */
DW_API_PUBLIC
dwStatus dwDNNTensor_setTimestamp(dwTime_t time, dwDNNTensorHandle_t const tensorHandle);
