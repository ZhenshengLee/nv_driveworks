...
class ArgsHelper():
    ...
    def __init__(self):
        ...
        self._add_inferred_edges = False
    ...
    @property
    def add_inferred_edges(self) -> bool:
        return self._add_inferred_edges
    ...
    @add_inferred_edges.setter
    def add_inferred_edges(self, other: bool):
        if not isinstance(other, bool):
            raise TypeError("Incorrect add_inferred_edges type")
        self._add_inferred_edges = other
...
class TimingHelper():
    ...
    def add_to_group(module_list):
        ...
        count = 0
        while True:
            resCount = {CPU: 0, GPU: 0, DLA: 0, VPI: 0, VPU: 0}
            for g in groupDict:
                if len(g):
                    resCount[g[0]['type']]+=1
...
class Resource:
    ...
    def hyperepoch(self, other: 'HyperEpoch'):
        if self.hyperepoch is not None:
            raise ValueError(f"Resource already added to another HyperEpoch: {self.hyperepoch.name}'")
...
class VPU(Resource):
    """
    Class to represent VPU resources.
    example instantiation in timing file
        VPU0 = VPU("VPU0")
    """
    name = 'VPU'
...
class PVAStream(Serializer):
    """
    Class to represent PVA Streams.
    example instantiation in timing file
        pva_stream_1 = PVAStream("PVA_STREAM1", pva0)
        # where pva0 is an already instantiated PVA
    This creates a cupva stream resource and associates it
    with a particular PVA resource. This association
    is used for serialization of runnables.
    """
    name = 'PVA_STREAM'
    @typechecked
    def __init__(self, name: str, vpu: VPU):
        super().__init__(name)
        self.resource_type = "PVA_STREAM"
        self.serializee = vpu
...
class VulkanQueue(Serializer):
    """
    Class to represent Vulkan Queues.
    vulkan_queue_0 = VulkanQueue("VK_QUEUE0", GPU0)
        # where GPU0 is an already instantiated GPU
    This creates a Vulkan Queue resource and associates it
    with a particular GPU resource. This association
    is used for serialization of runnables.
    """
    name = "VULKAN_QUEUE"
    @typechecked
    def __init__(self, name: str, gpu: GPU):
        super().__init__(name)
        self.resource_type = "VULKAN_QUEUE"
        self.serializee = gpu
...
class Runnable:
    ...
    @property
    def name(self) -> str:
        if self._name.startswith("External."):
            return self._name
        if self.parent is None: # ssm pass in Client and service passes
            return self._name
    ...
    def epoch(self, epoch: 'Epoch'):
        if self.epoch is not None:
            raise ValueError("runnable already added to another epoch", self.name, self.epoch.name, epoch.name)
        self._epoch = epoch

    @property
    def global_id(self) -> str:
        if self.client is None:
            raise ValueError(f"{self.name} client is not set")
    ...
    def depends(self, other: 'Runnable'):
        if isinstance(other, GPUSubmitter) and self is not other.submittee:
            ...
            self._dependencies.add(other.submittee)
        elif isinstance(other, VPUSubmitter) and self is not other.submittee:
            self._dependencies.add(other.submittee)
        else:
    ...
    @property
    def dependencies(self) -> List['Runnable']:
    ...
    def yml(self) -> Dict[str, Dict]:
        ...
        ymlDict["Resources"] = sorted(resource.name for resource in self.resources) if not self._extern else sorted(self.resources)
        if self._priority:
...
class VPUSubmitter(Submitter):
    """
    Class to represent VPU Submitters
    """
    def __init__(self, name: str):
        super().__init__(name)
        self.submittee = VPUSubmittee(self._name + "_submittee")
        self.submittee.submitter = self
        self.submittee.depends(self)
        self.serializerType = PVAStream

    @property
    def handle(self) -> PVAStream:
        return super().serializer

    @handle.setter
    def handle(self, other: PVAStream):
        self.serializer = other

class VulkanSubmitter(Submitter):
    """
    Class to represent Vulkan Submitters
    """
    def __init__(self, name: str):
        super().__init__(name)
        self.submittee = VulkanSubmittee(self._name + "_submittee")
        self.submittee.submitter = self
        self.submittee.depends(self)
        self.serializerType = VulkanQueue

    @property
    def handle(self) -> VulkanQueue:
        return super().serializer

    @handle.setter
    def handle(self, other: VulkanQueue):
        self.serializer = other
...
class VPUSubmittee(Submittee):
    def __init__(self, name: str):
        super().__init__(name)
        self._resourceType = VPU

class VulkanSubmittee(Submittee):
    def __init__(self, name: str):
        super().__init__(name)
        self._resourceType = GPU
...
class Client:
    ...
    def ymlResources(self) -> Dict:
        ...
        for resource in sorted(self.resources, key=lambda x: x.resource_type):
            ...
            elif resource.resource_type == "PVA_STREAM":
                resourceTree[resource.resource_type].append(resource.yml())
            elif resource.resource_type == "CUDA_STREAM":
                resourceTree[resource.resource_type].append(resource.yml())
            elif resource.resource_type == "VULKAN_QUEUE":
                resourceTree[resource.resource_type].append(resource.yml())
            else:
...
class HyperEpoch:
    ...
    def yml(self) -> Dict:
        ...
        if self.period is not None:
            ret[self.name]["Period"] = str(self.period)
        if self.monitor is not None and self.monitor.ns > 0:
            ret[self.name]["Monitor"] = str(self.monitor)
        return ret
...
class STM_graph:
    ...
    def __init__(self,
        ...
        if require_wcet and wcet_file is None:
            raise ValueError(f"WCET is required yet no wcet file provided for schedule {name}")
        ...
        if wcet_file:
            ...
            if not self.wcets:
                print("Skip wcet set with empty wcet file: ", wcet_file)
            else:
                for runnable in self.runnables:
                    if runnable.global_id in self.wcets:
                        runnable.wcet = TimeInterval(ns=self.wcets[runnable.global_id])
                    elif runnable.global_id.replace(".", ":", 1).replace(".", "_").replace(":", ".", 1) in self.wcets:
                        # only temporarily needed for services passes until dots are used by all fully qualified pass names instead of underscores
                        runnable.wcet = TimeInterval(ns=self.wcets[runnable.global_id.replace(".", ":", 1).replace(".", "_").replace(":", ".", 1)])
                    elif runnable.epoch is not None:
                        # No epoch set on a runnable indicates it won't be used in the schedule.
                        missing_wcets.append(runnable.global_id)
                    if isinstance(runnable, Submitter):
                        ...
                        elif runnable.submittee.epoch is not None:
                            missing_wcets.append(runnable.submittee.global_id)

        if (not self.wcets or missing_wcets) and require_wcet:
            msg = "All runnables" if not self.wcets else missing_wcets
            raise ValueError(f"WCET must be provided for all runnables. {msg} are missing a WCET in {wcet_file} for schedule {name}")
    ...
    def yml(self) -> Dict:
        ...
        if not self.extern:
            ...
            for client in self.clients:
                ...
                ymlGraph["Runnables"] = []
                for runnable in sorted(client.runnables, key=lambda x: x.name):
                    ymlGraph["Runnables"].append(runnable.yml())
                    if isinstance(runnable, Submitter):
                        ymlGraph["Runnables"][-1][runnable.name]["Submits"] = ymlGraph["Runnables"][-1][runnable.name]["Submits"].split(".")[1]
                        ymlGraph["Runnables"].append(runnable.submittee.yml())
...
class FileValidation:
    ...
    def validate(fileName, tag0) -> str:
        if not isinstance(fileName, str) or not fileName.endswith(tag0 + '.json'):
            raise ValueError('Wrong descriptionType for generating Description: ' + fileName + " tag0: " + tag0)
...
class HardwareResources:
    def __init__(self, resourceDict, soc):
        ...
        self.resourcesDict['VPU'] = [] if not 'VPU' in resourceDict else [VPU(x, soc) for x in resourceDict['VPU']]
...
class ScheduleDescription:
    # const string
    FRAMEWORKPASSSERVICES = 'services'
    FRAMEWORKPASS_RCS = 'RoadCastService'
    FRAMEWORKPASS_SYNC = 'EpochSyncService'
    ...
    def __init__(self, baseDir, scheduleJson, scheduleKey, identifier, isDefaultSchedule, systemDescription):
        ...
        # wcet file
        self.wcetFile = None
        if WCET_FILE in self.scheduleJson and self.scheduleJson[WCET_FILE]:
            self.wcetFile = os.path.join(self.baseDir, self.scheduleJson[WCET_FILE])
        else:
            print("Skip wcet set as wcet is not specified")
        # reference to system DAG
        # why instantiate GraphletDescription&NodeDescription and connects them inside ScheduleDescription?
        # because there are multiple schedules specified for an app. Each will assign runnables with epoch and clients info
        # if do these in SystemDescription, re-assign will throw. Now, each ScheduleDescription will instantiate the whole clean system once
        self.systemDescription = systemDescription
        self.subcomponents = {}
        for graphletId, grapheletFile in systemDescription.getGraphletFiles().items():
            if grapheletFile.endswith(DescriptionType.GRAPHLET.name.lower() + '.json'):
                self.subcomponents[graphletId] = GraphletDescription(graphletId, grapheletFile, self)
            elif grapheletFile.endswith(DescriptionType.NODE.name.lower() + '.json'):
                self.subcomponents[graphletId] = NodeDescription(graphletId, grapheletFile, self)
            else:
                raise ValueError('Subcomponents of Graphlet are required to be Node or Graphlet: ' + grapheletFile)
        self.connections = systemDescription.getConnections()
        # connect all connections. The same as what GraphletDescription has done
        self.connectSubcomponent()
        # framework pass runnables dict. Instatiate runnables in self.parseClients as different schedules need different instances.
        self.frameworkPassRunnables = {}

        clientInfo = ScheduleDescription.__gatherClientInfo(self.systemDescription.getProcesses())
        externalRunnableClientsSOC = {}
        externalRunnables = self.systemDescription.getExternalRunnables()
        for externalRunnable in externalRunnables:
            if self.findExternalRunnableSOC(externalRunnable) == SoC.TegraA:
                externalRunnableClientsSOC.update({externalRunnable._name[len("External."):]: "machine0"})
            elif self.findExternalRunnableSOC(externalRunnable) == SoC.TegraB:
                externalRunnableClientsSOC.update({externalRunnable._name[len("External."):]: "machine1"})
        resourcesConfig = ScheduleDescription.__parseResources(clientInfo, self.scheduleJson[HYPEREPOCHS], externalRunnableClientsSOC)
        ...
        if argsHelper.add_inferred_edges:
            self.addInferredEdges()
    ...
    def __determineResourceType(resourceName):
        ...
        if resourceName.startswith("CPU"):
            type = "CPU"
        elif resourceName.startswith("iGPU"):
            type = "GPU"
        elif resourceName.startswith("dGPU"):
            type = "GPU"
        elif resourceName.startswith("DLA"):
            type = "DLA"
        elif resourceName.startswith("VPI"):
            type = "VPI"
        elif resourceName.startswith("PVA_STREAM"):
            type = "PVA_STREAM"
        elif resourceName.startswith("VPU"):
            type = "VPU"
            ...
            type = "VPI_HANDLE"
        elif resourceName.startswith("VULKAN_QUEUE"):
            type = "VULKAN_QUEUE"
        else:
            type = "MUTEX"
        return type
        ...
        for hyperepoch, hyperepochDesc in hyperepochs.items():
            for res in hyperepochDesc.get("resources", {}).keys():
                ...
                if ids[0] == machineName:
                    ...
                    if not resType in ("CPU", "GPU", "VPI", "DLA", "VPU"):
                        raise RuntimeError("Only hardware resources can be specified with machine name", ids)
                    if resType in ret:
                        if resourceName in ret[resType]:
                            raise RuntimeError("The same resource is specified for two hyperepochs: " + resourceName)
                        else:
        ...
        for hyperepoch, hyperepochDesc in hyperepochs.items():
            for res, runnables in hyperepochDesc.get("resources", {}).items():
                ...
                if ids[0] == clientName:
                    ...
                    if resType in ("CPU", "GPU", "DLA", "VPI", "VPU"):
                        raise RuntimeError("Only software resources can be spcified with client name", ids)
                    if resType in ("CUDA_STREAM", "CUDLA_STREAM", "DLA_HANDLE", "VPI_HANDLE", "PVA_STREAM", "VULKAN_QUEUE"):
                        if not ":" in resourceNameWithMapping:
                            raise RuntimeError("CUDA_STREAM, CUDLA_STREAM, DLA_HANDLE, VPI_HANDLE, PVA_STREAM, VULKAN_QUEUE resource instance need to be mapped to a hardware resource")
                    elif resType == "MUTEX":
                        pass
                    else:
    ...
    @staticmethod
    def __getHyperepochResource(hyperepochs, clientInfo, hyperepochName, externalRunnableClientsSOC={}):
        ret = []
        ...
        for res in hyperepochDesc.get("resources", {}).keys():
            ...
            elif ids[0] in externalRunnableClientsSOC.keys():
                # external runnable resources
                ret.append(ids[0] + "." + externalRunnableClientsSOC[ids[0]] + "." + ids[1].split(":")[0])
            else:
                ret.append(res) # hardware resources
        return ret

    @staticmethod
    def __parseResources(clientInfo, hyperepochs, externalRunnableClientsSOC={}):
        ...
        clientResources = {
            ...
            for clientName in list(clientInfo.keys()) + list(externalRunnableClientsSOC.keys())
        }
        # in the form of {"hyperepoch": [<resource name>}
        hyperepochResources = {
            hyperepochName: ScheduleDescription.__getHyperepochResource(hyperepochs, clientInfo, hyperepochName, externalRunnableClientsSOC)
    ...
    def parseHyperepochResourcePrefix(self, hyperEpoch: dict) -> set:
        ...
        Example:
             For cameraHyperepoch, resource keys are:
                 ...
                 camera_master.CUPVA_STREAM0:VPU0
                 camera_master.CUPVA_STREAM1:VPU1
                 camera_master.VULKAN_QUEUE0:iGPU
                 machine0.VPU0
                 machine0.VPU1
                 machine0.CPU1
                 machine0.CPU2
    ...
    def parseClients(self, clientsDict, clientResources, hyperepochsDict):
        ...
        for externalRunnable in externalRunnables:
            ...
            externalRunnableResourcesDict         = clientResources[externalRunnable._client].get('resources', {})
            externalRunnableResources     = []
            for resourceType, resourceList in externalRunnableResourcesDict.items():
                for resourceParam in resourceList:
                    if resourceType == 'MUTEX':
                        resource = MUTEX(resourceParam, soc)
                        externalRunnableResources.append(resource)
                        ...
                        raise KeyError('SoftwareResource only support type "MUTEX", "CUDASTREAM, "PVA_STREAM" and "CUDLA_STREAM": ' + resourceType)
            self.softwareResources[externalRunnable._client] = externalRunnableResources
            clients.append(Client(
                name      = externalRunnable._client,
                soc       = soc,
                runnables = [],
                externalRunnables = [externalRunnable],
                resources = externalRunnableResources
            ))
        ...
        for clientName, clientConfig in clientsDict.items():
            # TODO(xingpengd) remove hard coded machine name machine0 and machine1
            # TODO(xingpengd) remove hard coded mapping between machine name and SoC name
            soc = None
            ...
            # instantiate framemwork pass runnables
            # the framework pass runnable name is start with clientName. and it is unique for each client. so it
            # is ok to instantiate inside the for loop and assigned to self.frameworkPassRunnables with the name
            if ScheduleDescription.FRAMEWORKPASSSERVICES in clientConfig:
                for frameworkPassType, frameworkPassConfig in clientConfig[ScheduleDescription.FRAMEWORKPASSSERVICES].items():
                    if frameworkPassType == ScheduleDescription.FRAMEWORKPASS_RCS:
                        rcsJsonObj = None
                        rcsFile = os.path.join(self.baseDir, frameworkPassConfig['parameters']['configurationFile'])
                        rcsFile = FileValidation.validate(rcsFile, '')
                        with open(rcsFile, 'r') as rcsfd:
                            rcsJsonObj = json.load(rcsfd)
                        if not 'ports' in rcsJsonObj:
                            raise KeyError('Invalid RoadCastService configuration file: ', rcsFile)
                        rcsPorts = rcsJsonObj['ports']
                        for passName in frameworkPassConfig["passes"]:
                            rcsFrameworkPassName = ScheduleDescription.FRAMEWORKPASS_RCS + '.' + clientName + '.' + passName
                            self.frameworkPassRunnables[rcsFrameworkPassName] = {"runnable": CPURunnable(rcsFrameworkPassName), "config": rcsPorts}
                    elif frameworkPassType == ScheduleDescription.FRAMEWORKPASS_SYNC:
                        for passName in frameworkPassConfig["passes"]:
                            syncFrameworkPassName = ScheduleDescription.FRAMEWORKPASS_SYNC + '.' + clientName + '.' + passName
                            self.frameworkPassRunnables[syncFrameworkPassName] = {"runnable": CPURunnable(syncFrameworkPassName), "config": ''}
                    else:
                        raise TypeError("Unsupported framework pass type: ", frameworkPassType)

            if len(runOnMachines) > 1:
                # TODO(yel): wrong assumeption that hyperepoch can be used to distinguish TA and TB
                        ...
                        for epochId, epochContent in hyperepochDict['epochs'].items():
                            ...
                            for epochRunnables in epochRunnablesLists:
                                for runnablePath in epochRunnables:
                                    # append framework pass runnables
                                    if runnablePath in self.frameworkPassRunnables:
                                        runnables.append(self.frameworkPassRunnables[runnablePath]["runnable"])
                                        continue
                                    # extend DAG runnables
                                    layers = runnablePath.split('.')
                ...
                # append framework pass runnables
                for fpName, fpDict in self.frameworkPassRunnables.items():
                    runnables.append(fpDict["runnable"])
            ...
            for resourceType, resourceList in resourcesDict.items():
                for resourceParam in resourceList:
                    if resourceType == 'MUTEX':
                        ...
                        softwareResources.append(resource)
                        softwareResourcesDict[layers[0]] = resource
                    elif resourceType == 'PVA_STREAM':
                        layers      = resourceParam.split(':')
                        pvaResource = [d for d in self.getHardwareResourceSoc(soc).getResourceByType('VPU') if d.name == layers[1]]
                        if (len(pvaResource) == 0):
                            raise KeyError('pvaResource is missing')
                        resource = PVAStream(layers[0], pvaResource[0])
                        softwareResources.append(resource)
                        softwareResourcesDict[layers[0]] = resource
                    elif resourceType == 'VULKAN_QUEUE':
                        layers      = resourceParam.split(':')
                        gpuResource = [g for g in self.getHardwareResourceSoc(soc).getResourceByType('GPU') if g.name == layers[1]]
                        resource    = VulkanQueue(layers[0], gpuResource[0])
                        softwareResources.append(resource)
                        softwareResourcesDict[layers[0]] = resource
                    else:
                        raise KeyError('SoftwareResource only support type "MUTEX", "CUDASTREAM, PVA_STREAM, VULKAN_QUEUE and "CUDLA_STREAM": ' + resourceType)
                ...
                for runnablePath in runnableList:
                    ...
                    if not runnablePathArr[-1].isupper(): # runnablePath is a component ID
                        ...
                        for runnable in self.getSubComponent(componentName).getRunnables():
                            if runnable.enumName == runnablePathArr[-1] or runnable.enumName == runnablePathArr[-1]:
                                ...
                                elif isinstance(resource, PVAStream):
                                    runnable.handle = resource
                                elif isinstance(resource, VulkanQueue):
                                    runnable.handle = resource
                                else:
            ...
            for runnable in client.runnables:
                for resource in runnable.resources:
                    if 'CUDA_STREAM' in resource.name or 'CUDLA_STREAM' in resource.name:
                        ...
                        if cuda_mutex is not None:
                            runnable.resources = [self.cpu, cuda_mutex]

        return clients
    def parseHyperepochs(self, hyperepochsDict, hyperepochResources):
        hyperepochs = {}
        # hyperepoch identifiers are created and stored in sorted order
        for identifier, (hyperepochId, hyperepochDict) in enumerate(sorted(hyperepochsDict.items())):
            # resources
            ...
            for epochId, epochContent in epochsDict.items():
                ...
                for phaseIndex, phaseRunnables in enumerate(epochRunnables): # Get the phaseIndex and apply to each runnables
                    ...
                    for runnablePath in phaseRunnables:
                        if runnablePath.startswith("External."):
                            ...
                            externalRunnables.append(externalRunnable)
                        elif runnablePath in self.frameworkPassRunnables:
                            fpRunnable = self.frameworkPassRunnables[runnablePath]["runnable"]
                            fpRunnable.pipelinePhase = PipelinePhase(phaseIndex)
                            runnables.append(fpRunnable)
                        else:
                ...
                if 'aliasGroups' in epochContent.keys():
                    groups: List = []
                    for groupName, groupComp in epochContent['aliasGroups'].items():
                        groups.extend(TimingHelper.add_to_group([self.getSubComponent(comp) for comp in groupComp], groupName))
            ...
            monitor = None
            if 'monitoringPeriod' in hyperepochDict:
                monitor = TimeInterval(ns=hyperepochDict['monitoringPeriod'])
        ...
        return hyperepochs
    def parseHyperepochsGetRunnables(self, layers, subComponents, pipelinePhase = PipelinePhase.PIPELINE_PHASE_UNCHANGED):
        if len(layers) < 1:
            raise RuntimeError('Failed traverse module path for scheduleDescription. Empty layer!')
        ...
        # When reaching the last layer, returns all passes
        # When node is not the last layer, means a certain pass is specified, only returns the specified pass
        if len(layers) == 1 or (len(layers) == 2 and subComponent.getDescriptionType() == DescriptionType.NODE):
            subComponent.setPipelinePhase(pipelinePhase)
            if len(layers) == 1:
                # Graphlet and Node assignment, all passes in the graphlet/node are included
                return subComponent.getRunnables()
            else:
                # Pass assignment, only the specified pass is included
                res = []
                for runnable in subComponent.getRunnables():
                    runnablePath = layers[1]
                    if runnablePath == runnable.enumName:
                        # For separate pass assignment, the implicit dependency of previous pass no longer applies (e.g. pass_2 inserted but pass_1 doesn't exist).
                        # User needs to explicitly assign pass dependencies in this case
                        runnable.dependencies.clear()
                        res.append(runnable)
                return res
        else:
            return self.parseHyperepochsGetRunnables(layers[1:], subComponent.getSubComponents(), pipelinePhase)

    # The function will skip all indirect consumers
    def returnRealConsumerPort(self, port):
            ...
            if port.downstreams == []:
                ...
                for index in range(len(port.downstreams)):
                    if port.indirectFlags[index]:
                        continue
                    ports.extend(self.returnRealConsumerPort(port.downstreams[index]))
                return ports
    ...
    # Return all consumer ports including indirect ones
    def returnRealConsumerPortWithIndirect(self, port, indirect):
        if port.type == PortType.VIRTUAL:
            if port.downstreams == []:
                return []
            else:
                ports = []
                for index in range(len(port.downstreams)):
                    if port.downstreams[index].type == PortType.INPUT:
                        ports.extend(self.returnRealConsumerPortWithIndirect(port.downstreams[index], indirect))
                    elif port.indirectFlags[index]:
                        ports.extend(self.returnRealConsumerPortWithIndirect(port.downstreams[index], True))
                    else:
                        ports.extend(self.returnRealConsumerPortWithIndirect(port.downstreams[index], False))
                return ports
        elif port.type == PortType.INPUT:
            return [[port, indirect]]
        else:
            raise TypeError('Invalid downstream type: ' + port.type.name)
    ...
    def generateDependency(self):
        ...
        3. When pipeline enabled, set dependency on sensor sync for all phase 1 and phase 2 nodes
        '''
        def setDependency(srcNode, dstNode, srcPort, destPort, dependsAsync, printDetails = False):
            ...
            if dstNode.getPipelinePhase() == PipelinePhase.PIPELINE_PHASE_2 and srcNode.getPipelinePhase() == PipelinePhase.PIPELINE_PHASE_1:
                ...
                return
            elif dstNode.getPipelinePhase() == PipelinePhase.PIPELINE_PHASE_1 and srcNode.getPipelinePhase() == PipelinePhase.PIPELINE_PHASE_2:
                raise RuntimeError(f"Pipelining Error: backwards edge from phase 2 to phase 1. {srcNode.name}/{srcNode.id} to {dstNode.name}/{dstNode.id}. Src Port {srcPort.name}")
        ...
        # For dataset replay, to avoid the case that phase 2 nodes runs while phase 1 nodes are in
        # blocking wait for sensorSync
        def setDependencyForPipeline(leafNodes, cameraEpochSyncNode):
            if not cameraEpochSyncNode:
                return
            for leafNodeId, leafNode in leafNodes.items():
                if cameraEpochSyncNode == leafNode:
                    continue
                if leafNode.getPipelinePhase() != PipelinePhase.PIPELINE_PHASE_UNCHANGED:
                    leafNode.passes[0].depends(cameraEpochSyncNode.passes[-1])

        def getAllRcsUpstreams(rcsPorts, rcsUpstreams, virtualPort2Node):
            for portFullName in rcsPorts.keys():
                if portFullName in virtualPort2Node:
                    rcsUpstreams.add(virtualPort2Node[portFullName])

        def parsePassAndDependencyDict(passDict, dependencyDict, namePrefix, subcomponents):
            for subcomponentId, subcomponent in subcomponents.items():
                if subcomponent.getDescriptionType() == DescriptionType.NODE:
                    for passObj in subcomponent.getRunnables():
                        currName = namePrefix + '.' + subcomponentId + '.' + passObj.enumName
                    ...
                    parsePassAndDependencyDict(passDict, dependencyDict, namePrefix + '.' + subcomponentId, subcomponent.getSubComponents())

        cameraEpochSyncNode = None
        leafNodes = self.getLeafNodes()
        for leafNodeId, leafNode in leafNodes.items():
            if leafNodeId == "top.cameraEpochSync.sensorSyncNode":
                cameraEpochSyncNode = leafNode
            ...
            for outputPort in leafNode.getOutputPorts().values():
                for downstream in outputPort.peers:
                    if downstream in outputPort.indirect:
                        continue
                    finalInputPorts = self.returnRealConsumerPort(downstream)
                    dependsAsync = False
        ...
        # Add pipeline dependency between phase 2 nodes and cameraEpochSyncNode for dataset replay
        setDependencyForPipeline(leafNodes, cameraEpochSyncNode)

        # Add dependency for framework pass runnables
        # get virtualPortName to dwNode dict from graph
        virtualPort2Node = {}
        moduleSet = {node.parent for nodeId, node in self.getLeafNodes().items() if node.parent is not None}
        for module in moduleSet:
            for _, portInstance in module.outputPorts.items():
        ...
        # traverse client
        for client in self.clients:
            fpRunnablesEpochDict = {}
            for fpName, fpDict in self.frameworkPassRunnables.items():
                clientAndPassNames = fpName.split(".", 1)[1]
                if clientAndPassNames.startswith(client.name + "."):
                    if fpDict["runnable"].epoch is None:
                        raise ValueError("The service is specified but not epoch assigned for it", fpName)
                    # use name for better print(debug)
                    if fpDict["runnable"].epoch.name in fpRunnablesEpochDict:
                        fpRunnablesEpochDict[fpDict["runnable"].epoch.name].append(fpDict)
                    else:
                        fpRunnablesEpochDict[fpDict["runnable"].epoch.name] = [fpDict]
            if not fpRunnablesEpochDict:
                continue
            # collects all epochs
            epochList = []
            for runnable in client.runnables:
            ...
            # set dependency for each epoch
            for epoch in epochList:
                if not epoch or not epoch.name in fpRunnablesEpochDict:
                    continue
                fpRunnables = fpRunnablesEpochDict[epoch.name]
                ...
                for runnable in epoch.runnables:
                    ...
                    if ScheduleDescription.FRAMEWORKPASS_RCS in runnable.name or ScheduleDescription.FRAMEWORKPASS_SYNC in runnable.name:
                        continue
                    moduleName = runnable.parent.id
                    if moduleName in checked_module:
                        continue
                    checked_module.add(moduleName)
                    # dependency for each framework pass runnable
                    for fpItem in fpRunnables:
                        fpRunnable = fpItem['runnable']
                        if ScheduleDescription.FRAMEWORKPASS_SYNC in fpRunnable.name:
                            ...
                            recvPass = client.findRunnable(runnable.parent.passes[0])
                            # None means the runnable doesn't belong to the client
                            if recvPass is None:
                                continue
                            recvPass.depends(fpRunnable)
                        elif ScheduleDescription.FRAMEWORKPASS_RCS in fpRunnable.name:
                            rcsPorts = fpItem['config']
                            rcsUpstreams = set()
                            getAllRcsUpstreams(rcsPorts, rcsUpstreams, virtualPort2Node)
                            # add all producers to rcsFrameworkPass dependency
                            # don't add dependency if no connection between RCS and the node
                            if not moduleName in rcsUpstreams:
                                continue
                            ...
                            if 'cameraNode' in runnable.parent.id:
                                ...
                                outputPass = runnable.parent.passes[1]
                            sendPass = client.findRunnable(outputPass)
                            # None means the runnable doesn't belong to the client
                            if sendPass is None:
                                continue

                            if sendPass.pipelinePhase == PipelinePhase.PIPELINE_PHASE_2 and "LOW_FREQUENCY_PHASE_TWO" in fpRunnable.name or sendPass.pipelinePhase == PipelinePhase.PIPELINE_PHASE_UNCHANGED:
                                # In the unchanged case either there is no pipelining enabled, or this is an epoch that has
                                # a single roadcast pass, such as highFrequency.
                                fpRunnable.depends(sendPass)
                            elif 'cameraNode' in runnable.parent.id and "LOW_FREQUENCY_PHASE_ONE" in fpRunnable.name:
                                # Camera timestamps are used to index some roadcast data and is updated by the cameraNodes.
                                # This dependency avoids a race condition of camera ts update and roadcast push.
                                fpRunnable.depends(sendPass)

                def find_fp_by_name(name, fpRunnables):
                    for fp in fpRunnables:
                        if name in fp['runnable'].name:
                            return fp['runnable']

                # PhaseTwo to depend on PhaseOne for correct timestamp updating
                phaseOne = find_fp_by_name("LOW_FREQUENCY_PHASE_ONE", fpRunnables)
                phaseTwo = find_fp_by_name("LOW_FREQUENCY_PHASE_TWO", fpRunnables)
                if phaseOne is not None and phaseTwo is not None:
                    phaseTwo.depends(phaseOne)

                    # Add dependency from all roadcast passes onto sync nodes
                    syncRunnable = find_fp_by_name(ScheduleDescription.FRAMEWORKPASS_SYNC, fpRunnables)
                    if syncRunnable is not None:
                        phaseOne.depends(syncRunnable)
                        phaseTwo.depends(syncRunnable)
        ...
        if self.extraDeps:
            ...
            # Top level has no namePrefix. So extent the travese for this layer
            for subcomponentId, subcomponent in self.getSubComponents().items():
                if subcomponent.getDescriptionType() == DescriptionType.NODE:
                    for passObj in subcomponent.getRunnables():
                        currName = subcomponentId + '.' + passObj.enumName
                        if currName in passDict.keys():
                            passDict[currName] = passObj
                        if currName in dependencyDict.keys():
                            dependencyDict[currName] = passObj
                elif subcomponent.getDescriptionType() == DescriptionType.GRAPHLET:
                    parsePassAndDependencyDict(passDict, dependencyDict, subcomponentId, subcomponent.getSubComponents())
            # Add dependency
            for passName, dependencyNameList in self.extraDeps.items():
                for dep in dependencyNameList:
                    if passDict[passName] and dependencyDict[dep]:
                        ...
                        warnings.warn(f"The dependency object {dep} for pass object {passName} is not available and ignored here, plz check why!")

        # Strict check for SensorSync dependency at the end
        # Some user may add indirect from SensorSyncNode to SensorNode. This dangerous mistake will
        # totally break the functionality of SensorSync.
        leafNodes = self.getLeafNodes()
        for leafNodeId, leafNode in leafNodes.items():
            if ".sensorSyncNode" not in leafNodeId:
                continue
            for outputPort in leafNode.getOutputPorts().values():
                for downstream in outputPort.peers:
                    throwFlag = False
                    ...
                    if downstream in outputPort.indirect:
                        throwFlag = True
                        downstreamId = downstream.parent.id # maybe virtual port
                    else:
                        finalInputPorts = self.returnRealConsumerPortWithIndirect(downstream, False)
                        # Check pass dependency directly instead of indirct flag on port
                        for fip in finalInputPorts:
                            # epoch valid && in the same epoch && dep missing
                            if (fip[0].parent.passes[0].epoch is not None
                                and fip[0].parent.passes[0].epoch.name == leafNode.passes[2].epoch.name
                                and leafNode.passes[2] not in fip[0].parent.passes[0].dependencies):
                                throwFlag = True
                                downstreamId = fip[0].parent.id
                    if throwFlag:
                        raise ValueError(f"Indirect from SensorSyncNode to SensorNode is forbiden. SensorSyncNode: '{leafNodeId}', downstream: '{downstreamId}'")

    # almost the same as what GraphletDescription has done. No inputPorts and outputPorts
    def connectSubcomponent(self):
        def connectInstance(connectionFromStr, connectionToStr, indirectFlag, singletonFlag = False):
            # skip INBOUND
            if connectionFromStr.startswith("EXTERNAL:"):
                return
            # skip OUTBOUND
            if connectionToStr.startswith("EXTERNAL:"):
                return
            # get instance
            connectionFrom = connectionFromStr.split('.')
            connectionTo   = connectionToStr.split('.')
            if len(connectionFrom) == 1:
                raise KeyError(f"No port at system level. Is this a INBOUND? connection from: '{connectionFromStr}'")
                ...
                if not connectionFrom[1] in upComOutPorts:
                    raise KeyError(f"Cannot find 'src' port '{connectionFromStr}'")
            ...
            if len(connectionTo) == 1:
                raise KeyError(f"No port at system level. Is this a OUTBOUND? connection to: '{connectionToStr}'")
    ...
    def addInferredEdges(self):
        '''
        1. Find all cross-epoch edges
        2. Use the Floyd-Warshall algorithm to store if there is a path between any two nodes.
        3. Classify all cross-epoch edges based on the source and destination.
           For example, all edges from hyperepoch1 → hyperepoch2 would be in one list while all
           edges from hyperepoch2 → hyperepoch1 would be in another.
        4. Take all opposite facing lists (Ex: hyperepoch1 → hyperepoch2 and hyperepoch2 → hyperepoch1),
           compare each element with all elements in the other list, and add the required edges
        5. Recompute a subset of the Floyd-Warshall algorithm matrix once an edge is added.
        6. Repeat this step for all elements in the list

        Additionally, add dependencies to order indirect edges before direct ones.
        '''

        def updateMatrix(start, end):
            nonlocal graphMatrix
            graphMatrix += graphMatrix + np.outer(graphMatrix[:,start], graphMatrix[end,:])

        # Convert graph to a matrix
        def generateTranslationDict():
            nonlocal leafNodes
            nonlocal translationDict
            for counter, i in enumerate(leafNodes.values()):
                translationDict[i.id] = counter

        def pathFromAtoB(nodeA, nodeB):
            nonlocal graphMatrix
            nonlocal translationDict
            return graphMatrix[translationDict[nodeA.id]][translationDict[nodeB.id]]

        def floydWarshall(leafNodes):
            nonlocal graphMatrix
            nonlocal translationDict
            for i in leafNodes.values():
                for j in leafNodes.values():
                    graphMatrix[translationDict[i.id]][translationDict[j.id]] = (translationDict[i.id] == translationDict[j.id])

            for node in leafNodes.values():
                for nodePass in node.passes:
                    for dep in nodePass.dependencies:
                        # special case for framework_pass which doesn't belong to any node
                        if ScheduleDescription.FRAMEWORKPASS_RCS in dep.name or ScheduleDescription.FRAMEWORKPASS_SYNC in dep.name:
                            continue
                        graphMatrix[translationDict[dep.parent.id]][translationDict[node.id]] = True

            for node in leafNodes.values():
                for outputPort in node.getOutputPorts().values():
                    for downstream in outputPort.peers:
                        if downstream in outputPort.indirect:
                            continue
                        finalInputPorts = self.returnRealConsumerPort(downstream)
                        for port in finalInputPorts:
                            graphMatrix[translationDict[node.id]][translationDict[port.parent.id]] = True

            for k, val in enumerate(leafNodes.values()):
                updateMatrix(k, k)

        def addEdgeToGraph(nodeA,nodeB):
            nonlocal graphMatrix
            nonlocal translationDict
            if not graphMatrix[translationDict[nodeA.id]][translationDict[nodeB.id]]:
                graphMatrix[translationDict[nodeA.id]][translationDict[nodeB.id]] = True
                updateMatrix(translationDict[nodeA.id], translationDict[nodeB.id])

        def getCyclePath(source,path, visited):
            nonlocal graphMatrix
            nonlocal translationDict
            if source.id in path:
                path.append(source.id)
                return (True, path)
            elif source.id not in visited:
                visited.append(source.id)
                for outputPort in source.getOutputPorts().values():
                    for downstream in outputPort.peers:
                        if downstream in outputPort.indirect:
                            continue
                        finalInputPorts = self.returnRealConsumerPort(downstream)
                        for finalInputPort in finalInputPorts:
                            ...
                            newPath.append(source.id)
                            res = getCyclePath(finalInputPort.parent,newPath, visited)
                            if res[0]:
                                return res
            return (False, [])
        ...
        leafNodes = self.getLeafNodes()
        epochPairEdgeList = defaultdict(list)
        translationDict = dict()
        graphMatrix = np.zeros((len(leafNodes.values()), len(leafNodes.values())), dtype=np.bool)

        generateTranslationDict()
        floydWarshall(leafNodes)

        # Detect cycles
        for outerNode in leafNodes.values():
            for innerNode in leafNodes.values():
                if outerNode.id != innerNode.id:
                    if pathFromAtoB(outerNode, innerNode) and pathFromAtoB(innerNode, outerNode):
                        res = getCyclePath(innerNode, [], [])
                        if res[0]:
                            cyclePath = ""
                            for index, node in enumerate(res[1]):
                                cyclePath += node
                                if index is not len(res[1]) - 1:
                                    cyclePath += " -> "
                            raise RuntimeError("Cycle detected between " + str(outerNode.id) + " and " + str(innerNode.id) + " with the path: " + cyclePath +
                                               " Please remove the cycle or remove it in *_deterministic_replay.trans.json if necessary " +
                                               "or not possible to remove, such as in a subcomponent.")
                        else:
                            raise RuntimeError("Cycle detected between " + str(outerNode.id) + " and " + str(innerNode.id) + " but couldn't find path. " +
                                               "Please remove the cycle or remove it in *_deterministic_replay.trans.json if necessary " +
                                               "or not possible to remove, such as in a subcomponent.")

        # Find all cross-epoch edges and classify them based on src and destination
        for nodeA in leafNodes.values():
            for nodeB in leafNodes.values():
                if nodeA.id != nodeB.id:
                    if pathFromAtoB(nodeA, nodeB):
                            if (nodeA.passes[0].epoch is not None
                                and nodeB.passes[0].epoch is not None
                                and nodeA.passes[0].epoch.name != nodeB.passes[0].epoch.name):
                                listItem = (nodeA, nodeB)
                                listName = nodeA.passes[0].epoch.name + nodeB.passes[0].epoch.name
                                epochPairEdgeList[listName].append(listItem)

        # Check for cycles and add the inferred edges
        visitedKeys = set()
        for key1, listItem1 in epochPairEdgeList.items():
            if key1 in visitedKeys:
                continue
            visitedKeys.add(key1)
            # Re-order src and destination names to get the index of the corresponding list
            nameOfKey = listItem1[0][1].passes[0].epoch.name + listItem1[0][0].passes[0].epoch.name

            for key2, listItem2 in epochPairEdgeList.items():
                if key2 in visitedKeys:
                    continue
                # Find the corresponding key
                if key2 == nameOfKey:
                    visitedKeys.add(nameOfKey)
                    # Go through both lists
                    for edge1 in listItem1:
                        for edge2 in listItem2:
                            # Check for a path from B->A before adding A->B to avoid cycles
                            if not pathFromAtoB(edge2[1], edge1[0]) and edge1[0].id != edge2[1].id:
                                edge2[1].passes[0].depends(edge1[0].passes[-1])
                                addEdgeToGraph(edge1[0], edge2[1])
                            elif edge1[0].id != edge2[1].id:
                                edge1[0].passes[0].depends(edge2[1].passes[-1])
                                addEdgeToGraph(edge2[1], edge1[0])

                            if not pathFromAtoB(edge1[1], edge2[0]) and edge1[1].id != edge2[0].id:
                                edge1[1].passes[0].depends(edge2[0].passes[-1])
                                addEdgeToGraph(edge2[0], edge1[1])
                            elif edge1[1].id != edge2[0].id:
                                edge2[0].passes[0].depends(edge1[1].passes[-1])
                                addEdgeToGraph(edge1[1], edge2[0])

        # For deterministic replay, nodes with indirect edges must consume data first.
        # Add additional dependencies to execute those first
        epochPairEdgeList = defaultdict(list)
        for leafNode in leafNodes.values():
            for outputPort in leafNode.getOutputPorts().values():
                for downstream in outputPort.peers:
                    # Classify the cross-epoch edges into src and destination epoch and
                    # identify whether an edge is indirect or not.
                    if downstream in outputPort.indirect:
                        inputPorts = self.returnRealConsumerPortWithIndirect(downstream, True)
                    else:
                        inputPorts = self.returnRealConsumerPortWithIndirect(downstream, False)
                    for inputPort in inputPorts:
                        outputPass = leafNode.passes[1] if (
                            argsHelper.check_optional
                            and (leafNode.name == 'dw::framework::dwCameraNode'
                            and ('IMAGE_NATIVE_PROCESSED' == outputPort.name
                            or 'IMAGE_TIMESTAMP' == outputPort.name))
                        ) else leafNode.passes[-1]

                        # Create a list[list] based on the src and destination
                        # Skip same epoch edges as well as ones without an epoch
                        if (inputPort[0].parent.passes[0].epoch is not None
                            and outputPass.epoch is not None
                            and inputPort[0].parent.passes[0].epoch.name != outputPass.parent.passes[0].epoch.name):
                                # src node, dest node, indirect
                                listItem = (leafNode, inputPort[0].parent, inputPort[1])
                                listName = leafNode.passes[0].epoch.name + inputPort[0].parent.passes[0].epoch.name
                                epochPairEdgeList[listName].append(listItem)

        for key, listOfEdges in epochPairEdgeList.items():
            for edge1 in listOfEdges:
                # Skip none indirect edge
                if not edge1[2]:
                    continue

                # Check for undesired indirect edges
                if not pathFromAtoB(edge1[1], edge1[0]):
                    raise RuntimeError("Indirect edges that don't create cycles are undesired in deterministic replay. " +
                                       "Undesired indirect edge added between " + str(edge1[0].id) + " and " + str(edge1[1].id) +
                                       ". Please add an action to *_deterministic_replay.trans.json to disable the indirect edge (\"indirect\": false). " +
                                       "You can also remove the indirect flag from the offending connection to resolve this error.")

                for edge2 in listOfEdges:
                    # Skip indirect edge
                    if edge2[2]:
                        continue
                    # Add a dependency so that nodes with indirect edges get executed first to avoid deadlock
                    if not pathFromAtoB(edge1[0], edge2[0]) and edge2[0].id != edge1[0].id:
                        edge1[0].passes[0].depends(edge2[0].passes[-1])
                        addEdgeToGraph(edge2[0], edge1[0])
                    elif edge2[0].id != edge1[0].id:
                        edge2[0].passes[0].depends(edge1[0].passes[-1])
                        addEdgeToGraph(edge1[0], edge2[0])

                    if not pathFromAtoB(edge2[1], edge1[1]) and edge2[1].id != edge1[1].id:
                        edge2[1].passes[0].depends(edge1[1].passes[-1])
                        addEdgeToGraph(edge1[1], edge2[1])
                    elif edge2[1].id != edge1[1].id:
                        edge1[1].passes[0].depends(edge2[1].passes[-1])
                        addEdgeToGraph(edge2[1], edge1[1])
    ...
    def getLeafNodes(self):
        leafNodes = {}
        for graphlet in self.subcomponents.values():
            if graphlet.getDescriptionType() == DescriptionType.NODE:
                leafNodes[graphlet.id] = graphlet
            elif graphlet.getDescriptionType() == DescriptionType.GRAPHLET:
                leafNodes.update(graphlet.getLeafNodes())
            else:
                raise ValueError('Subcomponents of Graphlet are required to be Node or Graphlet: ' + graphlet.getDescriptionType())
        return leafNodes
    def getSubComponent(self, componentId):
        componentIdList = componentId.split('.')
        if not componentIdList[0] in self.subcomponents.keys():
            # Runables with "External" subcomponent name is a stmExternalRunnable
            if componentIdList[0] == "External":
                return None
            else:
                raise ValueError('componentId invalid. graphlet id: ', self.subcomponents.keys(), ' componentId: ', componentIdList)
    ...
    def getRunnables(self):
        runnables = []
        for graphlet in self.subcomponents.values():
            runnables.extend(graphlet.getRunnables())
        return runnables
    def isExtern(self):
        return self.extern
...
class NodeDescription:
        ...
        for index in range(len(passList)):
            ...
            if dependencyNames is not None:
                for dependencyName in dependencyNames:
                    dependencyPass = [r for r in passes if r.enumName == dependencyName]
    ...
    def generateRunnable(self, resources, index):
        ...
        hardware_set = {'CPU', 'GPU', 'DLA', 'VPI', 'CUDLA', 'PVA'}
        ...
        elif 'DLA' in hardware or 'CUDLA' in hardware:
        ...
        elif 'PVA' in hardware:
            return VPUSubmitter('pass_' + str(index))
        else:
            raise ValueError('Runnable requires hardware CPU/GPU/DLA/CUDLA/VPI but ' + str(resources))
    # TODO(yel): move this check logic to yaml generation
...
class GraphletDescription:
    def __init__(self, graphletId, graphletFile, parent):
        ...
        self.parent = parent
        graphletFile = FileValidation.validate(graphletFile, self.type.name.lower())
    ...
    def parseSubComponents(self, subcomponentsDict):
        ...
        for componentId in subcomponentsDict.keys():
            ...
            if componentType.endswith(DescriptionType.GRAPHLET.name.lower() + '.json'):
                component = GraphletDescription(componentFullId, fixPath(componentType, self.baseDir), self)
            elif componentType.endswith(DescriptionType.NODE.name.lower() + '.json'):
    ...
    def getLeafNodes(self):
        leaves = {}
        for _, subcomponent in self.subcomponents.items():
            if subcomponent.getDescriptionType() == DescriptionType.NODE:
                leaves[subcomponent.id] = subcomponent
            elif subcomponent.getDescriptionType() == DescriptionType.GRAPHLET:
                leaves.update(subcomponent.getLeafNodes())
            else:
...
class SystemDescription:
    def __init__(self, systemFile):
        ...
        rootGraphletIDs = list(systemJson[SUBCOMPONENTS].keys())
        if len(rootGraphletIDs) < 1:
            raise ValueError('At least one subcomponent in application description but none')
        self.graphletFiles = {}
        for rootGraphletID in rootGraphletIDs:
            keyValidation([COMPTYPE], systemJson[SUBCOMPONENTS][rootGraphletID], self.type.name.lower(), systemFile)
            self.graphletFiles[rootGraphletID] = fixPath(systemJson[SUBCOMPONENTS][rootGraphletID][COMPTYPE], self.baseDir)
        self.connections = {} if CONNECTIONS not in systemJson else systemJson[CONNECTIONS]
        ...
        if stmExternalRunnables in systemJson:
            ...
            for key, externalRunnable in externalRunnables.items():
                ...
                runnable._resourceType = externalRunnable["processorTypes"]
                runnable._resources = externalRunnable["processorTypes"]
    ...
    def getGraphletFiles(self):
        return self.graphletFiles
    def getDescriptionType(self):
        return self.type
    def getSchedules(self):
        return self.schedules
    def generateExternalRunnable(self, name, resources=[]):
...
def main(argv=None):
    ...
    parser.add_argument('--output', type = str, nargs='+', required = False, help = 'output files containing path for schedule yaml file.', default="/tmp/demo.yaml")
    parser.add_argument('--check_optional', action="store_true", help = 'check optional.')
    ...
    parser.add_argument('--add_inferred_edges', action="store_true", help = 'Add inferred edges to the graph to order runnables for determinism')
    parser.add_argument('--require_wcet', action="store_true", help = "enables a check that each runnable has a WCET value")
    ...
    outputFiles = []
    for output in argvs.output:
        if output.startswith('/'):
            outputFiles.append(output)
        else:
            outputFiles.append(os.path.abspath(os.path.join(currentPath, output)))
    ...
    for file in outputFiles:
        if os.path.split(file)[0] != outputDir:
            print('All output yamls generated together must share a base directory.') # This restriction is needed to associate output file with schedule below
            return
    outputDir = os.path.join(outputDir, '')                         #adds a / at the end if it is not there

    argsHelper.check_optional = argvs.check_optional
    argsHelper.add_inferred_edges = argvs.add_inferred_edges
    ...
    for schedule in systemDescription.getSchedules():
        if schedule.isExtern():
            currentFile = f"{outputDir}{appFileName}_sub__{schedule.getScheduleName()}.yaml"
        else:
            currentFile = f"{outputDir}{appFileName}__{schedule.getScheduleName()}.yaml"

        # Only need to generate files that are returned as fn output
        if currentFile not in outputFiles:
            continue
        ...
        stmGraph = STM_graph(
            ...
            require_wcet = argvs.require_wcet
        )
        # === generate  ===
