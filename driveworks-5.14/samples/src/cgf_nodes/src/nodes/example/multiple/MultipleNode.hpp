...
struct MultipleNodeParams
{
    dw::core::FixedString<64> name;
};

class MultipleNode : public ExceptionSafeProcessNode
{
public:
    static constexpr auto describeInputPorts()
    {
        using namespace dw::framework;
        return describePortCollection(DW_DESCRIBE_PORT(DW_STRING_TYPE, "VALUE_0"_sv, PortBinding::REQUIRED),
    ...
    static std::unique_ptr<MultipleNode> create(ParameterProvider& provider);

    static constexpr auto describeParameters()
    {
        return describeConstructorArguments<MultipleNodeParams, dwContextHandle_t>(
            describeConstructorArgument(
                DW_DESCRIBE_PARAMETER(
                    dw::core::FixedString<64>,
                    "name"_sv,
                    &MultipleNodeParams::name)),
            describeConstructorArgument(
                DW_DESCRIBE_UNNAMED_PARAMETER(
                    dwContextHandle_t)));
    }

    MultipleNode(const MultipleNodeParams& params, const dwContextHandle_t ctx);
};
