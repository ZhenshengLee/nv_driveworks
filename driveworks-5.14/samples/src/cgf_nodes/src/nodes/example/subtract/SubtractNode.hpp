...
struct SubtractNodeParams
{
    dw::core::FixedString<64> name;
};

class SubtractNode : public ExceptionSafeProcessNode
{
public:
    ...
    static std::unique_ptr<SubtractNode> create(ParameterProvider& provider);

    static constexpr auto describeParameters()
    {
        return describeConstructorArguments<SubtractNodeParams, dwContextHandle_t>(
            describeConstructorArgument(
                DW_DESCRIBE_PARAMETER(
                    dw::core::FixedString<64>,
                    "name"_sv,
                    &SubtractNodeParams::name)),
            describeConstructorArgument(
                DW_DESCRIBE_UNNAMED_PARAMETER(
                    dwContextHandle_t)));
    }

    SubtractNode(const SubtractNodeParams& params, const dwContextHandle_t ctx);
};
