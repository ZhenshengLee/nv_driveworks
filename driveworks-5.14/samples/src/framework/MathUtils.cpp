...
dwTransformation3f rigidTransformation(dwQuaternionf const& quaternion, dwVector3f const& translation)
{
    auto ret         = DW_IDENTITY_TRANSFORMATION3F;
    float32_t* array = ret.array;

    float32_t q[4] = {quaternion.x, quaternion.y, quaternion.z, quaternion.w};
    {
        float32_t dist2 = q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3];
        if (std::fabs(dist2 - 1) > 1e-6)
        {
            float32_t dist = std::sqrt(dist2);
            q[0] /= dist;
            q[1] /= dist;
            q[2] /= dist;
            q[3] /= dist;
    ...
    // rotation
    array[0] = 1 - 2 * q[1] * q[1] - 2 * q[2] * q[2];
    array[1] = 2 * (q[0] * q[1] + q[2] * q[3]);
    array[2] = 2 * (q[0] * q[2] + q[1] * q[3]);

    array[4] = 2 * (q[0] * q[1] - q[2] * q[3]);
    array[5] = 1 - 2 * q[0] * q[0] - 2 * q[2] * q[2];
    array[6] = 2 * (q[1] * q[2] + q[0] * q[3]);

    array[8]  = 2 * (q[0] * q[2] + q[1] * q[3]);
    array[9]  = 2 * (q[1] * q[2] - q[0] * q[3]);
    array[10] = 1 - 2 * q[0] * q[0] - 2 * q[1] * q[1];
...
void quaternionToRotationMatrix(dwMatrix3d& mat, dwQuaterniond const& q)
{
    float64_t const* quat = &q.x;
...
{
    return dwVector2f{imageSize.x / (2.0f * std::tan(fov.x * 0.5f)),
                      imageSize.y / (2.0f * std::tan(fov.y * 0.5f))};
}

// see https://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles
void quaternionToEulerAngles(dwQuaternionf const& quat, float32_t& roll, float32_t& pitch, float32_t& yaw)
{
    float32_t const* q = &quat.x;
...
{
    return {T.array[0 + 3 * 4], T.array[1 + 3 * 4], T.array[2 + 3 * 4]};
}
...
float32_t getRotationMagnitude(const dwTransformation3f& T)
{
    ...
    return std::acos(0.5f * (traceR - 1.0f));
}
...
void getRotationMatrix(dwMatrix3f* R, float32_t rollInDegrees, float32_t pitchInDegrees, float32_t yawInDegrees)
{
    ...
    const float32_t croll  = std::cos(roll);
    const float32_t sroll  = std::sin(roll);
    const float32_t cpitch = std::cos(pitch);
    const float32_t spitch = std::sin(pitch);
    const float32_t cyaw   = std::cos(yaw);
    const float32_t syaw   = std::sin(yaw);

    // note that dwMatrix3f is defined item(row,col) = _array[row + col*3].
    // which means the matrices, as written down by hand, would be the
    // transpose of what appears in the {} brackets below
    // for example, Rx, below, if written out as a matrix, would be
    //      [1     0       0      ]
    // Rx = [0     croll   -sroll ]
    //      [0     sroll   croll  ]
...
void computeHomography(dwMatrix3f* homographyOut, dwTransformation3f transformationIn, dwMatrix3f camOutRotationMatrix,
