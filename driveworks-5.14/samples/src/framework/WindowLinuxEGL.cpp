...
// -----------------------------------------------------------------------------
WindowLinuxEGL::WindowLinuxEGL(int32_t width, int32_t height, bool offscreen, int32_t samples)
    : WindowEGL(width, height, offscreen)
{
    // Load Linux-specific EGL Extensions
    eglStreamConsumerOutputEXT = (PFNEGLSTREAMCONSUMEROUTPUTEXTPROC)
        eglGetProcAddress("eglStreamConsumerOutputEXT");
    eglGetOutputLayersEXT = (PFNEGLGETOUTPUTLAYERSEXTPROC)
        eglGetProcAddress("eglGetOutputLayersEXT");

    if (!eglStreamConsumerOutputEXT ||
        !eglGetOutputLayersEXT)
    {
        throw std::runtime_error("WindowLinuxEGL: Cannot load EGL extensions");
    }

    EGLBoolean status;

    EGLint deviceCount  = 8;
...
// -----------------------------------------------------------------------------
WindowLinuxEGL::~WindowLinuxEGL()
{
}

// -----------------------------------------------------------------------------
bool WindowLinuxEGL::initDrm(const char* name, EGLAttrib* layerAttribs, int& dispWidth, int& dispHeight)
{
