...
void RectifierApp::onProcess()
{
    ...
    if (m_image != nullptr)
    {
        ...
        dwImageNvMedia* inImage;
        dwImage_getNvMedia(&inImage, m_image);

        // Rectify image
        dwImageNvMedia* rectifiedImage;
        dwImage_getNvMedia(&rectifiedImage, m_rectifiedImage);
        CHECK_DW_ERROR(dwRectifier_warpNvMedia(rectifiedImage, inImage, m_rectifier));
        if (m_frameCap)
        {
